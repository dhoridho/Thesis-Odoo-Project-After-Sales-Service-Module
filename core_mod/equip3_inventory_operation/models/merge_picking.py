...
class MergePicking(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if active_ids:
            ...
            if any(pick.state != 'assigned' for pick in picks):
                raise Warning('Merging only on records that are ready.')
        return super(MergePicking, self).default_get(fields)
    ...
    def merge_pickings(self):
        for res in self:
            ...
            action = mod_obj.xmlid_to_object(
            ...
            if len(pickings) < 2:
                raise Warning(
                    'Please select multiple picking to merge in the list view.')
            ...
            for pick in pickings:
                ...
                if not pick_type[1:] == pick_type[:-1]:
                    raise Warning(
                        'Merging is only allowed on Pickings of same Types.')
            ...
            new_pick = pick_obj.create(
                {'picking_type_id': type_new_pick, 'location_dest_id': location_dest_id, 'location_id': location_id, 'state': 'draft'})
            ...
            for pick in pickings:
                ...
                if not pick_state[1:] == pick_state[:-1]:
                    raise Warning(
                        'Merging is only allowed on Pickings of same State.')

                if not pick_partner[1:] == pick_partner[:-1]:
                    raise Warning(
                        'Merging is only allowed on Pickings of same Partner.')

                else:
                    cancel_list.append(pick)
                    # merge_ids = move_obj.search([('picking_id', '=', pick.id)])
                    merge_ids = pick.move_ids_without_package
                ...
                if not invoice_status[1:] == invoice_status[:-1]:
                    raise Warning(
                        'Merging is only allowed on Pickings of same Invoice Status.')
                msg_origin = ""
                origin_list = []
                ...
                new_pick.write(
                    {'state': pick.state, 'partner_id': pick.partner_id and pick.partner_id.id or False, 'origin': origin})
                if pick_ids:
                    new_id = pick.id
                ...
                pick.merge_info = 'This operation has been merged to <b>%s</b>' % (
                    new_pick.name)
                pick.action_cancel()
            if new_pick.picking_type_id.code == 'incoming':
                msg_body = _(
                    "This transfer has been created from: <b>%s</b>") % (origin)
            else:
                msg_body = _(
                    "This delivery order has been created from: <b>%s</b>") % (origin)
