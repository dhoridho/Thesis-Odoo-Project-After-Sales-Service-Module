odoo.define('equip3_inventory_operation.StockPickingBarcodeScanner', function(require){
    "use strict";
    ...
    var BarcodeFormRenderer = FormRenderer.extend({
        ...
        _renderView: function(){
            ...
            return this._super.apply(this, arguments).then(function(){
                ...
                if ($testTriggerScanButton.length){
                    self.testBarcode = $testTriggerScanButton.attr('data-barcode');
        ...
        _onBarcodeScanned: function(barcode){
            this.trigger_up('field_changed', {
                ...
                changes: {sh_stock_barcode_mobile: barcode}
            });

            if (this.state.data && this.state.data.sh_stock_bm_is_cont_scan === 'True'){
                this.trigger_up('field_changed', {
                    dataPointID: this.allFieldWidgets[this.state.id][0].dataPointID,
                    changes: {sh_stock_barcode_mobile: ""}
                });
            }
        },
    ...
    var BarcodeFormController = FormController.extend({
        _onFieldChanged: function (ev) {
            if (ev.data && ev.data.changes && 'sh_stock_barcode_mobile' in ev.data.changes){
                var $mark = $('body').find('.mark_barcode_change');
                if (!$mark.length) {
                    $mark = $('<div class="mark_barcode_change"/>');
                    $mark.prependTo('body');
                }
            }
        ...
        _barcodeScanned: function (barcode, target) {
            var $barcodeMobile = this.renderer.$el.find('input[name="sh_stock_barcode_mobile"]');
    ...
    viewRegistry.add('stock_picking_barcode_scanner', BarcodeFormView);
