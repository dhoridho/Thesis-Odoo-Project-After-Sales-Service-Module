...
class Database(http.Controller):
    ...
    @http.route('/multi_database/api/auth/login', auth='none', type='json', methods=['POST'])
    def auth_login_multi_database(self, **kw):
        body = request.jsonrequest
        try:
            uid = request.session.authenticate(body['database'], body['login'], body['password'])
        except AccessDenied as e:
            return self.get_response(401, '401', {"code":401, "message": "Invalid Password"})
        
        return self.get_response(200, '200', {"code":200, "message": "Login Successful"})
    
    @http.route('/multi_database/list', auth='none', type='http', methods=['GET'])
    def get_database_list(self, **kw):
        odoo = odoorpc.ODOO(kw['host'], port=kw['port'])
        return self.get_response(200, '200', {"code":200, "message": "get data Successful",'data':odoo.db.list()})
...
class Session(http.Controller):
    ...
    def account(self):
        ...
        return 'https://accounts.hashmicro.com/oauth2/auth?' + url_encode(params)
