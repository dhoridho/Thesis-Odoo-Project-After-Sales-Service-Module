...
class PosOrderReportInherit(models.Model):
    ...
    product_cancel_id = fields.Many2one('product.cancel', string='Session', readonly=True)

    def _select(self):
        return super(PosOrderReportInherit, self)._select() + ',s.product_cancel_id as product_cancel_id'

    def _group_by(self):
        return super(PosOrderReportInherit, self)._group_by() + ',s.product_cancel_id'
    ...
    #             LEFT JOIN product_cancel pc ON (s.product_cancel_id=pc.id)
    ...
    #             s.pricelist_id, s.account_move, s.create_date, s.session_id, s.product_cancel_id,
    #             l.product_id,
