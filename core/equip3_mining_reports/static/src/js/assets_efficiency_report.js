odoo.define('equip3_mining_reports.assets_efficiency_report', function (require) {
    'use strict';
    ...
    var AssetsEfficiencyReport = AbstractAction.extend({
        template: 'AssetsEfficiencyReportTemplate',
        ...
        init: function(parent, action) {
            ...
            this.filterSite = 'all';
            this.filterPit = 'all';
            this.searchModelConfig.modelName = 'mining.assets.efficiency.report'
        },
        ...
        start: async function() {
            ...
            var self = this;
            this._rpc({
                model: 'mining.assets.efficiency.report',
                method: 'create',
                args: [{
        ...
        loadData: function (initial_render) {
            ...
            self._rpc({
                ...
                if (initial_render) {
                    var $header = QWeb.render('AssetsEfficiencyReportHeader', {
                        filters: result['filters']
                    })
                    self.$('.o_mining_assets_efficiency_report_header').html($header);
                }
                if (!result['data'].length){
                    self.$('.o_mining_assets_efficiency_report_content').html(QWeb.render('AssetsEfficiencyNoContentHelper'));
                } else {
                    self.$('.o_mining_assets_efficiency_report_content').html(QWeb.render('AssetsEfficiencyReportTable', {
        ...
        printPdf: function(e) {
            e.preventDefault();
            return this.do_action('equip3_mining_reports.action_print_mining_assets_efficiency_report', {
                additional_context: {
                    'active_id': this.wizard_id,
                    'active_ids': [this.wizard_id],
                    'active_model': 'mining.assets.efficiency.report'
                }
            });
        },
    ...
    core.action_registry.add("mining_assets_efficiency_report", AssetsEfficiencyReport);
    return AssetsEfficiencyReport;
});