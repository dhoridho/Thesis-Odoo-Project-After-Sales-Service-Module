...
class ReportMiningFuelRatio(models.AbstractModel):
    _name = 'report.equip3_mining_reports.mining_fuel_ratio_report'
    _description = 'Report Mining Fuel Ratio'

    @api.model
    def _get_report_values(self, docids, data=None):
        return {
            'doc_ids': docids,
            'docs': self.env['mining.fuel.ratio.report'].browse(docids),
            'doc_model': 'mining.fuel.ratio.report',
        }


class MiningFuelRatioReport(models.TransientModel):
    _name = 'mining.fuel.ratio.report'
    _description = 'Fuel Ratio Report'
    ...
    def create(self, vals):
        ...
        return super(MiningFuelRatioReport, self).create(vals)
    ...
    def get_asset_values(self):
        ...
        for operation in operation_ids:
            ...
            asset_ids = record_ids.mapped('fuel_ids').mapped('asset_id')

            asset_uoms = {}
            for fuel in record_ids.mapped('fuel_ids'):
                if fuel.asset_id.id not in asset_uoms:
                    asset_uoms[fuel.asset_id.id] = [fuel.product_id.uom_id]
                else:
                    asset_uoms[fuel.asset_id.id] += [fuel.product_id.uom_id]
            ...
            for asset in asset_ids:
                child = {
                    ...
                    'uom': asset_uoms[asset.id][0].display_name,
                    'months': []
                }
                for month in range(1, 13):
                    ...
                    fuel_usage = sum(act_month_ids.mapped('fuel_ids').filtered(lambda f: f.asset_id == asset).mapped('qty_done'))
                    child['months'] += [{
                        'production': production,
                        'fuel_usage': fuel_usage,
                        'fuel_ratio': production / fuel_usage if fuel_usage else production
                    }]
    ...
    def print_xlsx_report(self):
        file_name = 'Fuel Ratio Report.xlsx'
        ...
        row = 1
        worksheet.write(row, 0, '%s: %s' % (self.company_id.name, _('Fuel Ratio Report')), title_format)
        ...
        header = ['Operation'] + (['Production', 'Fuel Usage', 'Fuel Ratio'] * 12)
        ...
        for operation in data:
            ...
            for child in operation['children']:
                ...
                for month in child['months']:
                    for key, uom in zip(
                        ['production', 'fuel_usage', 'fuel_ratio'],
                        [operation['uom'], child['uom'], '%s/%s' % (operation['uom'], child['uom'])]
                    ):
                        value = '%s %s' % (format_float(month[key], digits), uom)
