...
class MPSDetail(models.TransientModel):
    _name = 'mps.detail'
    _description = 'Master Production Schedule Detail'

    @api.model
    def create(self, vals):
        records = super(MPSDetail, self).create(vals)
        for record in records:
            record.mps_id.bom_id = record.bom_id.id
        return records

    def write(self, vals):
        result = super(MpsDetail, self).write(vals)
        for record in self:
            record.mps_id.bom_id = record.bom_id.id
        return result
    ...
    mps_id = fields.Many2one('equip.mrp.production.schedule', string='MPS', required=True, ondelete='cascade')
    ...
    material_ids = fields.One2many('mps.detail.material', 'detail_id', string='Materials')

    period = fields.Selection(selection=_get_period_selection, string='Period', default='0')
    ...
    def _onchange_forecasted_data(self):
        if not self.forecasted_data or not self.period:
            return
        forecasted = json.loads(self.forecasted_data)[int(self.period)]
        self.date_from = forecasted['date_from']
        self.date_to = forecasted['date_to']

        self.to_produce_qty = forecasted['to_replenish_qty']
...
class MPSDetailMaterial(models.TransientModel):
    _name = 'mps.detail.material'
    ...
    @api.depends('bom_line_id', 'detail_id', 'detail_id.to_produce_qty')
    def _compute_to_consume(self):
        for record in self:
            to_consume_qty = 0.0
            if record.bom_line_id and record.detail_id:
                to_consume_qty = (record.detail_id.to_produce_qty / record.bom_line_id.bom_id.product_qty) * record.bom_line_id.product_qty
    ...
    detail_id = fields.Many2one('mps.detail', string='MPS Detail', required=True, ondelete='cascade')
