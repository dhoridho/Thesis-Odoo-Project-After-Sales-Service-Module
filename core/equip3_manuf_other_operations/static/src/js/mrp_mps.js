odoo.define('equip3_manuf_other_operations.MrpMps', function (require) {
'use strict';
    ...
    var MrpMps = AbstractAction.extend({
        template: 'MPSAction',
        ...
        events: {
            ...
            'click .o_view_details': '_onClickViewDetails'
        },
        ...
        willStart: function () {
            ...
            var user_allowed_companies = _.map(session.user_companies.allowed_companies, o => o[0]);
            var def_warehouses = this._rpc({
                model: 'stock.warehouse',
                method: 'search_read',
                domain: [['company_id', '=', session.company_id], ['company_id', 'in', user_allowed_companies]],
                fields: ['id', 'name'],
                context: this.context,
            ...
            return Promise.all([def_warehouses]).then(function(results){
                var warehouseIds = results[0];
                self.warehouseId = warehouseIds[0].id;
                var def_domain = self._updateDomain(self.domain, [['warehouse_id', '=', self.warehouseId]]);
                var def_content = self._getState();
                return Promise.all([def_domain, def_content, def_control_panel, def_periods]).then(function () {
        ...
        start: async function () {
            ...
            return this._super(...arguments).then(async () => {
                if (self.state.length == 0) {
                    self.$el.find('.o_mrp_mps').replaceWith($(QWeb.render('equip3_mrp_mps_nocontent_helper')));
                }
                await self.update_cp();
                await self.renderPager();
        ...
        _actionReplenish: function (productionScheduleId) {
            ...
            var reloadMainContent = function(){
                return self._getState().then(function (state) {
                    if (state.length){
                        ...
                        var $content = $(QWeb.render('equip3_mrp_mps_nocontent_helper'));
                    }
            ...
            this.mutex.exec(function () {
                return self.do_action('equip3_manuf_other_operations.equip3_action_mrp_mps_wizard', {
        ...
        _createProduct: function () {
            var self = this;
            var exitCallback = function (a, b, c) {
                return self._rpc({
                    model: 'equip.mrp.production.schedule',
                    method: 'search_read',
                    domain: [['warehouse_id', '=', self.warehouseId]],
                    fields: ['id'],
                    limit: 1,
            ...
            this.mutex.exec(function () {
                return self.do_action('equip3_manuf_other_operations.equip3_action_mrp_mps_form_view', {
        ...
        _onClickViewDetails: function(ev){
            ev.preventDefault();
            var productionScheduleId = $(ev.target).closest('.o_mps_content').data('id');
            var state;
            for (var i=0; i < this.state.length; i++){
                if (this.state[i].id === productionScheduleId){
                    state = this.state[i];
                }
            }
            if (!state){
            ...
            var workcenterIds = [];
            for (var i=0; i < state.workcenter_ids.length; i++){
                workcenterIds.push(state.workcenter_ids[i][0]);
            }
            return this.do_action({
                name: _t("Master Production Schedule"),
                res_model: 'mps.detail',
                views: [[false, 'form']],
                view_mode: ['form'],
                ...
                context: {
                    create: false,
                    default_mps_id: productionScheduleId,
                    default_bom_id: state.bom_id[0],
                    default_warehouse_id: state.warehouse_id[0],
                    default_product_id: state.product_id[0],
                    default_company_id: state.company_id[0],
                    default_resource_id: state.resource_id[0],
                    default_workcenter_ids: [[6, 0, workcenterIds]],
                    default_period: '0',
                    default_forecasted_data: JSON.stringify(state.forecast_ids)
                },
                on_close: this._reloadContent()
            });
        }
    });

    core.action_registry.add('equip3_mrp_mps_client_action', MrpMps);
