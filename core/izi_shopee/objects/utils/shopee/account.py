...
class ShopeeAccount(object):
    ...
    def __init__(self, partner_id, partner_key, host="base", api_version="v2", **kwargs):
        ...
        self.host = host
        if self.host == "live":
            self.endpoints = ShopeeEndpoint(self, host="live", api_version=api_version)
        else:
    ...
    def get_auth_url_v2(self):
        ...
        if self.host == "live":
            credentials = '%(host)s%(path)s?partner_id=%(partner_id)s&redirect=%(redirect)s&timestamp=%(timestamp)s&sign=%(sign)s' % {
                'host': ShopeeEndpoint.HOSTS.get('live'),
