...
from .tools import validate_response, sanitize_response, pagination_get_pages, pagination_date_range, pagination_datetime_range
from .endpoint import ShopeeEndpoint


class ShopeeAPI(object):

    def __init__(self, sp_account, host="base", api_version="v2", ** kwargs):
        ...
        self.host = host
        if self.host == "live":
            self.endpoints = ShopeeEndpoint(sp_account, host="live", api_version=api_version)
        else:
            self.endpoints = ShopeeEndpoint(sp_account, host="base", api_version=api_version)
        ...
        self.pagination_datetime_range = pagination_datetime_range
