...
class SaleOrder(models.Model):
    ...
    def _add_rec_mp_field_mapping(cls, mp_field_mappings=None):
        ...
        mp_field_mapping = {
            ...
            'mp_buyer_id': ('buyer_user_id', lambda env, r: str(r) if r else None),
        ...
        def _get_time_slot(env, data):
            if data:
                slot_time = data[0]['time_slot_list'][0]['date']
                slot_time_format = datetime.fromtimestamp(time.mktime(time.gmtime(slot_time))).strftime(DEFAULT_SERVER_DATETIME_FORMAT)
        ...
        mp_field_mapping.update({
            ...
            'mp_delivered_deadline': ('ship_by_date', _convert_timestamp_to_datetime),
            'sp_package_number': ('package_list', _get_package_number),
            'mp_package_id': ('package_list', _get_package_number),
            ...
            'mp_pickup_done_time': ('pickup_done_time', _convert_timestamp_to_datetime),
            'mp_pickup_time_slot': ('shipping_paramater/pickup/address_list', _get_time_slot),
            # preorder
            'is_preorder': ('days_to_ship', _handle_preorder),
            'branch_id': ('branch_id', _get_branch_id)
        })
    ...
    def shopee_generate_delivery_line(self):
        ...
        for order in self:
            ...
            sp_order_pack_list = json_digger(sp_order_raw, 'package_list')
            if sp_order_pack_list:
                ...
                if not sp_logistic:
                    raise ValidationError('Please define logistic channel on "%s"' % sp_logistic_name)
                ...
                shipping_fee = json_digger(sp_order_raw, 'order_income/buyer_paid_shipping_fee',
                                           default=0)
        ...
        for order in self:
            ...
            if total_adjustment > 0:
                if not adjustment_line:
                    ...
                    for line in adjustment_line:
                        line['price_unit'] = total_adjustment
            ...
            if not shopee_coins_product:
                raise ValidationError(
                    'Please define shopee coins product on'
                    ' this marketplace account: "%s"' % order.mp_account_id.name)
            ...
            if total_coins > 0:
                if not shopee_coins_line:
                    ...
                    for line in shopee_coins_line:
                        line['price_unit'] = -total_coins
            order._reset_sequence()

    def shopee_generate_service_fee_line(self):
        for order in self:
            sp_order_raw = json.loads(order.raw, strict=False)
            shopee_services_line = order.order_line.filtered(lambda l: l.is_services)
            services_product = order.mp_account_id.services_product_id
            if not services_product:
                raise ValidationError(
                    'Please define services product on'
                    ' this marketplace account: "%s"' % order.mp_account_id.name)
            services_fee = json_digger(sp_order_raw, 'order_income/service_fee',
                                       default=0)
            # creditcard_transaction_fee = json_digger(sp_order_raw, 'order_income/credit_card_transaction_fee',
            #                            default=0)
            total_services = services_fee + 1000
            if total_services > 0:
                if not shopee_services_line:
                    order.write({
                        'order_line': [(0, 0, {
                            'sequence': 999,
                            'product_id': services_product.id,
                            'product_uom_qty': 1,
                            'price_unit': total_services,
                            'is_services': True
                        })]
                    })
                else:
                    for line in shopee_services_line:
                        line['price_unit'] = total_services
                order._reset_sequence()
    ...
    def shopee_get_awb(self):
        return self.shopee_print_label()
    ...
    def shopee_print_label(self):
        ...
        for order in self:
            if order.mp_awb_datas:
                ...
                pass
        if not order_list:
            raise UserError('Can not get label from the orders')
        if order.mp_account_id.default_awb_action == 'download':
            return {
                ...
                'target': 'new',
                'url': base_url + '/web/binary/shopee/download_pdf?order_ids=%s' % (','.join(order_list)),
            }
            ...
            return {
                ...
                'type': 'ir.actions.act_url',
                'target': 'new',
                'url': base_url + '/web/binary/shopee/open_pdf?order_ids=%s' % (','.join(order_list)),
            }
    ...
    def shopee_pickup(self):
        return self.shopee_request_pickup()
        ...
        for order in self:
            if order.mp_package_id:
                ...
                if sp_account:
                    ...
                    awb_data = sp_order_v2.download_shipping_document(order_sn=order.mp_invoice_number, package_number=order.mp_package_id, tracking_number=order.mp_awb_number)
                    if awb_data:
                        if 'error' in awb_data:
                            raise ValidationError(awb_data.get('message'))
                        else:
                            datas = base64.b64encode(awb_data.content)
                            name = 'shopee_shipping_document_awb_%s' % order.mp_awb_number
                            values = {
                                'name': name,
                                'type': 'binary',
                                'datas': datas,
                            ...
                            attachments = self.env['ir.attachment'].sudo().search([('name', '=', name)], limit=1)
                            if attachments:
    ...
    def action_cancel(self):
        if self.mp_account_id and self.mp_account_id.marketplace == 'shopee':
            return self.shopee_reject_order()
        else:
            return super(SaleOrder, self).action_cancel()

    def cancel_marketplace_sale_order_records(self):
        self.ensure_one()
        if self.state != 'cancel' and self.state != 'done':
            ...
            if picking_ids and any(picking.state not in ('done', 'cancel') for picking in picking_ids):
                ...
                name = " Cancelled by %s at %s. Reason: Cancel from %s" % (
                    user.name, datetime.now().strftime(DEFAULT_SERVER_DATETIME_FORMAT), self.marketplace)
