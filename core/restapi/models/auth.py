...
class AuthAuth(models.Model):
    ...
    def ir_cron_create_auth(self):
        user_ids =  self.env['res.users'].search([])
        if user_ids:
            for data in user_ids:
                auth =  self.env['auth.auth'].search([('user_id','=',data.id)])
                if not auth:
                    new_auth = self.env['auth.auth'].create({'user_id':data.id,'name':f"{data.name} Auth"})
    ...
    def revoke_access(self):
        ...
        """
        for rec in self:
            token = self.env['auth.access.token'].sudo().search([('access_token','=',rec.access_token)],limit=1)
            if token:
                token.sudo().unlink()
            rec.access_token = ''
...
class AuthAuthCode(models.Model):
    ...
    access_token_validity = fields.Datetime('Access Token Validity', domain=[('access_token_validity', '>', fields.Datetime.now())])
    auth_id = fields.Many2one('auth.auth', 'Auth')
