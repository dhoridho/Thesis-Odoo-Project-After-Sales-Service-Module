...
class BudgetBudget(models.Model):
    ...
    parent_id = fields.Many2one('budget.budget', 'Parent Category', index=True, ondelete='cascade')
    user_id = fields.Many2one('res.users', 'Responsible', default=lambda self: self.env.user)
    date_from = fields.Date('From', required=True, states={'done': [('readonly', True)]})
    date_to = fields.Date('To', required=True, states={'done': [('readonly', True)]})
    ...
    budget_budget_line = fields.One2many('budget.budget.budget.line', 'budget_budget_id', 'Budget Lines', states={'done': [('readonly', True)]}, copy=True)
...
class BudgetBudgetBudgetLines(models.Model):
    _name = "budget.budget.budget.line"
    _description = "Budget Line"

    name = fields.Char(compute='_compute_line_name')
    budget_budget_id = fields.Many2one('budget.budget', 'Budget', ondelete='cascade', index=True, required=True)
    ...
    planned_amount = fields.Monetary('Planned Amount', required=True, help="Amount you plan to earn/spend. Record a positive amount if it is a revenue and a negative amount if it is a cost.")
    practical_amount = fields.Monetary( compute='_compute_practical_amount', string='Practical Amount', help="Amount really earned/spent.")
    theoritical_amount = fields.Monetary( compute='_compute_theoritical_amount', string='Theoretical Amount', help="Amount you are supposed to have earned/spent at this date.")
    percentage = fields.Float( compute='_compute_percentage', string='Achievement', help="Comparison between practical and theoretical amount. This measure tells you if you are below or over budget.")
    company_id = fields.Many2one(related='budget_budget_id.company_id', comodel_name='res.company', string='Company', store=True, readonly=True)
    is_above_budget = fields.Boolean(compute='_is_above_budget')
    budget_budget_state = fields.Selection(related='budget_budget_id.state', string='Budget State', store=True, readonly=True)

    @api.model
    def read_group(self, domain, fields, groupby, offset=0, limit=None, orderby=False, lazy=True):
        # overrides the default read_group in order to compute the computed fields manually for the group

        result = super(BudgetBudgetBudgetLines, self).read_group(domain, fields, groupby, offset=offset, limit=limit, orderby=orderby, lazy=lazy)
    ...
    def _compute_line_name(self):
        ...
        computed_name = self.budget_budget_id.name
    ...
    def _line_dates_between_budget_dates(self):
        for rec in self:
            budget_date_from = rec.budget_budget_id.date_from
            budget_date_to = rec.budget_budget_id.date_to
            if rec.date_from:
