...
class ResConfigSettings(models.TransientModel):
    ...
    bco_expiry_date = fields.Integer(string="Budget Carry Over Expiry Date")
    ...
    def get_values(self):
        ...
        res.update({
            'bco_expiry_date': IrConfigParam.get_param('bco_expiry_date', '1'),
        })
        return res

    def set_values(self):
        super(ResConfigSettings, self).set_values() 
        self.env['ir.config_parameter'].sudo().set_param('bco_expiry_date', self.bco_expiry_date) 
        if self.budget_carry_over_approval_matrix == True:
            self.env.ref('equip3_construction_operation.approval_matrix_budget_carry_over_configuration_menu').active = True
            self.env.ref('equip3_construction_operation.approval_matrix_budget_carry_over_configuration_menu_internal').active = True
        else:
            self.env.ref('equip3_construction_operation.approval_matrix_budget_carry_over_configuration_menu').active = False
            self.env.ref('equip3_construction_operation.approval_matrix_budget_carry_over_configuration_menu_internal').active = False


class ApprovalMatrixBudgetCarryOver(models.Model):
    _name = 'approval.matrix.budget.carry.over'
    _description = 'Approval Matrix Budget Carry Over'
    ...
    approval_matrix_ids = fields.One2many('approval.matrix.budget.carry.over.line', 'approval_matrix_id', string='Approving Matrix Budgut Carry Over', tracking=True)
    ...
    def fields_view_get(self, view_id=None, view_type=None,
        ...
        res = super(ApprovalMatrixBudgetCarryOver, self).fields_view_get(
    ...
    def search_read(self, domain=None, fields=None, offset=0, limit=None, order=None):
        ...
        return super(ApprovalMatrixBudgetCarryOver, self).search_read(domain, fields, offset, limit, order)
    ...
    def read_group(self, domain, fields, groupby, offset=0, limit=None, orderby=False, lazy=True):
        ...
        if  self.env.user.has_group('equip3_construction_accessright_setting.group_construction_engineer') and not self.env.user.has_group('equip3_construction_accessright_setting.group_construction_director'):
            domain.append(('project_id.id','in',self.env.user.project_ids.ids))
        return super(ApprovalMatrixBudgetCarryOver, self).read_group(domain, fields, groupby, offset=offset, limit=limit, orderby=orderby, lazy=lazy)
    ...
    def _check_is_approver_matrix_line_ids_exist(self):
        for record in self:
            if not record.approval_matrix_ids:
                raise ValidationError("Can't save budget carry over approval matrix because there's no approver in approver line!")
                ...
                for matrix in approval_matrix_id:
                    for proj in matrix.project_id:
                        if proj in record.project_id:
                            raise ValidationError("The budget carry over approval matrix for this project is already exist in branch %s. Please change the project or the branch.\nExisted approval : '%s'." %((approval_matrix_id.branch_id.name),(approval_matrix_id.name)))
            else:
    ...
    def copy(self, default=None):
        res = super(ApprovalMatrixBudgetCarryOver, self.with_context(keep_line_sequence=True)).copy(default)
        return res


class ApprovalMatrixBudgetCarryOverLine(models.Model):
    _name = "approval.matrix.budget.carry.over.line"
    _description = "Approval Matrix Budget Carry Over"
    
    @api.model
    def default_get(self, fields):
        res = super(ApprovalMatrixBudgetCarryOverLine, self).default_get(fields)
        if self._context:
    ...
    approval_matrix_id = fields.Many2one('approval.matrix.budget.carry.over', string='Approval Matrix')
    ...
    def unlink(self):
        ...
        res = super(ApprovalMatrixBudgetCarryOverLine, self).unlink()
        approval._reset_sequence()
        return res

    @api.model
    def create(self, vals):
        res = super(ApprovalMatrixBudgetCarryOverLine, self).create(vals)
