...
class WizardEquipmentLeaseEstimations(models.TransientModel):
    _name = 'wizard.equipment.lease.estimation'
    _description = "Budget Carry Over Wizard Equipment Lease Estimation"
    _rec_name = "date"
    ...
    def _sequence_ref(self):
        for line in self:
            no = 0
            line.sr_no = no
            for l in line.carry_id.wizard_equipment_lease_estimation_ids:
                if l:
                    no += 1
                    l.sr_no = no
