...
class WizardLabourEstimations(models.TransientModel):
    _name = 'wizard.labour.estimation'
    _description = "Budget Carry Over Wizard Labour Estimation"
    _rec_name = "date"
    ...
    def _sequence_ref(self):
        for line in self:
            no = 0
            line.sr_no = no
            for l in line.carry_id.wizard_labour_estimation_ids:
                if l:
                    no += 1
                    l.sr_no = no

    @api.onchange('time', 'unit_price', 'contractors')
    def _onchange_total_amount(self):
        for rec in self:
            rec.total_amount = rec.time * rec.contractors * rec.unit_price