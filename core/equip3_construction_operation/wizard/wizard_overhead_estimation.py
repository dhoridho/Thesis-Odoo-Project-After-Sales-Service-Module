...
class WizardOverheadEstimations(models.TransientModel):
    _name = 'wizard.overhead.estimation'
    _description = "Budget Carry Over Wizard Overhead Estimation"
    _rec_name = "date"
    ...
    def _sequence_ref(self):
        for line in self:
            no = 0
            line.sr_no = no
            for l in line.carry_id.wizard_budget_carry_overhead_ids:
                if l:
                    no += 1
                    l.sr_no = no
