...
from odoo.addons.equip3_manuf_operations.models.acrux_chat_connector import ca_request
from odoo.addons.equip3_manuf_operations.models.qiscus_connector import qiscus_request
...
class MrpApprovalMatrix(models.Model):
    ...
    line_ids = fields.One2many('mrp.approval.matrix.line', 'matrix_id',  string='Approval Lines')
    is_branch_required = fields.Boolean(related='company_id.show_branch')

    _sql_constraints = [
        ('unique_mrp_approval_matrix', 'unique(company_id, branch_id, matrix_type)', 'The branch already used for another Approval Matrix. Please choose another branch.')
    ]
    ...
    def get_approval_entries(self, Model, states=None):
        self.ensure_one()
        entry = self.env['mrp.approval.matrix.entry']
    ...
    def get_model_action_xmlid(self):
        # inherit this method to get action xml_id for your model
        self.ensure_one()
        if self.matrix_type == 'mp':
            return 'equip3_manuf_operations.action_mrp_plan'
        elif self.matrix_type == 'mo':
            return 'mrp.mrp_production_action'
        return

    def get_model_menu_xmlid(self):
        # inherit this method to get menu xmlid for your model
        self.ensure_one()
        if self.matrix_type == 'mp':
            return 'equip3_manuf_operations.menu_manufacturing_plan'
        elif self.matrix_type == 'mo':
           return 'mrp.menu_mrp_production_action'
        return
    ...
    def post_log(self, Model, partner_ids, values):
        ...
        xml_id = 'equip3_manuf_operations.mrp_approval_html_body'
    ...
    def send_email_notification(self, Model, partner_id, values):
        self.ensure_one()
        xml_id = 'equip3_manuf_operations.mrp_approval_mail_reuse'
        company = self.get_company_model(Model)
    ...
    def send_whatsapp_notification(self, Model, partner_id, values):
        self.ensure_one()
        xml_id = 'equip3_manuf_operations.mrp_approval_text_body'
    ...
    def action_reject(self, Model, reason=False):
        ...
        if not self.env.context.get('skip_reject_wizard'):
            return {
                ...
                'res_model': 'mrp.approval.matrix.reject',
                'target': 'new',
                'view_mode': 'form',
                'context': {
...
class MrpApprovalMatrixLine(models.Model):
    ...
    matrix_id = fields.Many2one('mrp.approval.matrix', 'Approval Matrix')
