...
class InternalApprovalMatrix(models.Model):
    _name = "itr.approval.matrix"
    _description = "Internal Transfer Request Approval Matrix"
    ...
    itr_approval_matrix_line_ids = fields.One2many('itr.approval.matrix.line', 'itr_approval_matrix', string='Internal Approving Matrix')
    location_child_ids = fields.Many2many('stock.location', 'internal_location_matrix_rel', 'location_id', string="Details")
    ...
    @api.depends('itr_approval_matrix_line_ids')
    def approval_line_calc(self):
        for record in self:
            record.approval_line_count = len(record.itr_approval_matrix_line_ids)
    ...
    def check_approver_sequence(self):
        for record in self:
            for location in record.location_child_ids:
                location_obj = self.env['itr.approval.matrix'].search([('location_child_ids', 'in', location.ids), ('id', '!=', record.id)], limit=1)
                if location_obj:
                    raise ValidationError("Location exists %s in Internal Transfer Request %s" %(location.display_name, location_obj.name))

class InternalApprovalMatrixLine(models.Model):
    _name = "itr.approval.matrix.line"
    _description = "Internal Approval Matrix Line"

    @api.model
    def default_get(self, fields):
        res = super(InternalApprovalMatrixLine, self).default_get(fields)
        if self._context:
            context_keys = self._context.keys()
            next_sequence = 1
            if 'itr_approval_matrix_line_ids' in context_keys:
                if len(self._context.get('itr_approval_matrix_line_ids')) > 0:
                    next_sequence = len(self._context.get('itr_approval_matrix_line_ids')) + 1
    ...
    itr_approval_matrix = fields.Many2one('itr.approval.matrix', string="Internal Approval Matrix")
    approved_users = fields.Many2many('res.users', 'approved_users_internal_patner_rel', 'itr_id', 'user_id', string='Users')
