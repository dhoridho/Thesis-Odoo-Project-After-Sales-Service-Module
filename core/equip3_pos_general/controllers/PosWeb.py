...
class pos_controller(PosController):
    ...
    def get_model_session_info(self, model_name, _max=1):
        min_id = 0

        request.env.cr.execute("select max(id) from %s" % (str(model_name).replace('.', '_')))
        result = request.env.cr.fetchall()
        max_id = result[0][0] if len(result) == 1 else _max

        request.env.cr.execute("select count(id) from %s" % (str(model_name).replace('.', '_')))
        result = request.env.cr.fetchall()
        count = result[0][0] if len(result) == 1 else None

        return { model_name: { 'min_id': min_id, 'max_id': max_id, 'count': count } }
    ...
    def pos_web(self, config_id=False, **k):
        ...
        session_info['pos_retail'] = False
        session_info['equip3_pos_masterdata'] = True

        session_info['model_ids'] = {}
        session_info['model_ids'].update(self.get_model_session_info('product.product', 1))
        session_info['model_ids'].update(self.get_model_session_info('product.template', 1))
        session_info['model_ids'].update(self.get_model_session_info('product.template.barcode', 10))
        session_info['model_ids'].update(self.get_model_session_info('stock.production.lot', 10))
        session_info['model_ids'].update(self.get_model_session_info('stock.quant', 10))
        session_info['model_ids'].update(self.get_model_session_info('product.brand', 10))
        session_info['model_ids'].update(self.get_model_session_info('product.pricelist.item', 10))
        session_info['model_ids'].update(self.get_model_session_info('res.partner', 10))

        session_info['model_ids'].update(self.get_model_session_info('pos.order', 10))
        session_info['model_ids'].update(self.get_model_session_info('pos.order.line', 10))
        session_info['model_ids'].update(self.get_model_session_info('pos.voucher', 10))

        # Start Promotions
        session_info['model_ids'].update(self.get_model_session_info('pos.promotion', 10))
        session_info['model_ids'].update(self.get_model_session_info('pos.promotion.discount.order', 10))
        session_info['model_ids'].update(self.get_model_session_info('pos.promotion.discount.category', 10))
        session_info['model_ids'].update(self.get_model_session_info('pos.promotion.discount.quantity', 10))
        session_info['model_ids'].update(self.get_model_session_info('pos.promotion.gift.condition', 10))
        session_info['model_ids'].update(self.get_model_session_info('pos.promotion.gift.free', 10))
        session_info['model_ids'].update(self.get_model_session_info('pos.promotion.discount.condition', 10))
        session_info['model_ids'].update(self.get_model_session_info('pos.promotion.discount.apply', 10))
        session_info['model_ids'].update(self.get_model_session_info('pos.promotion.special.category', 10))
        session_info['model_ids'].update(self.get_model_session_info('pos.promotion.selected.brand', 10))
        session_info['model_ids'].update(self.get_model_session_info('pos.promotion.tebus.murah.selected.brand', 10))
        session_info['model_ids'].update(self.get_model_session_info('pos.promotion.specific.product', 10))
        session_info['model_ids'].update(self.get_model_session_info('pos.promotion.multi.buy', 10))
        session_info['model_ids'].update(self.get_model_session_info('pos.promotion.price', 10))
        session_info['model_ids'].update(self.get_model_session_info('pos.promotion.tebus.murah', 10))
        session_info['model_ids'].update(self.get_model_session_info('pos.promotion.multilevel.condition', 10))
        session_info['model_ids'].update(self.get_model_session_info('pos.promotion.multilevel.gift', 10))
        # End Promotions
        
        # Start Invoices
        session_info['model_ids'].update(self.get_model_session_info('account.move', 10))
        session_info['model_ids'].update(self.get_model_session_info('account.move.line', 10)) 
        # End Invoices
    ...
    def test_suite(self, mod=None, **kwargs):
        ...
        request.env.cr.execute("select max(id) from product_template")
        product_template_max_ids = request.env.cr.fetchall()
        request.env.cr.execute("select count(id) from product_template")
        count_product_templates = request.env.cr.fetchall()
        session_info['model_ids']['product.template']['max_id'] = product_template_max_ids[0][0] if len(
            product_template_max_ids) == 1 else 1
        session_info['model_ids']['product.template']['count'] = count_product_templates[0][0] if len(
            count_product_templates) == 1 else None
        request.env.cr.execute("select max(id) from stock_production_lot")
        lot_max_ids = request.env.cr.fetchall()
        session_info['model_ids']['stock.production.lot']['max_id'] = (lot_max_ids[0][0] if len(lot_max_ids) == 1 else 10) or 10
        request.env.cr.execute("select count(id) from stock_production_lot")
        count_lot = request.env.cr.fetchall()
        session_info['model_ids']['stock.production.lot']['count'] = count_lot[0][0] if len(count_lot) == 1 else None
        request.env.cr.execute("select max(id) from product_pricelist_item")
        pricelist_max_ids = request.env.cr.fetchall()
        session_info['model_ids']['product.pricelist.item']['max_id'] = pricelist_max_ids[0][0] if len(pricelist_max_ids) == 1 else 10
        request.env.cr.execute("select count(id) from product_pricelist_item")
        count_pricelist = request.env.cr.fetchall()
        session_info['model_ids']['product.pricelist.item']['count'] = count_pricelist[0][0] if len(count_pricelist) == 1 else None
    ...
    def publicScanPlacedOrder(self, uid=None, **kk):
        ...
        return request.render('equip3_pos_general.scan_successfully', qcontext={
    ...
    def scanQrOrder(self, order_id='', **k):
        ...
        if len(orders) == 1:
            ...
            response = request.render('equip3_pos_general.qrcode_order', context)
    ...
    def web_login(self, *args, **kw):
        ...
        if request.session.uid:
            ...
            if (pos_config or user.pos_config_ids) and user.pos_portal_user:
                return http.local_redirect('/web?#action=equip3_pos_general.point_of_sale_portal')
        return response