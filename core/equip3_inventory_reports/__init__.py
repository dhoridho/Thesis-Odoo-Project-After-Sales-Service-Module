    ...
    query = """
            ...
            AS $BODY$
                    ...
                    and 1 = case when array_length(warehouse_ids,1) >= 1 then
                        case when layer.warehouse_id = ANY(warehouse_ids) then 1 else 0 end
                        else 1 end
                    group by layer.company_id,layer.warehouse_id, cmp.name, move.date, layer.product_id, prod.default_code, tmpl.name, tmpl.categ_id, cat.complete_name;
                    ...
                    with all_data as (
                        ...
                        group by st.company_id, st.company_name, st.product_id, st.product_name, st.product_category_id, st.category_name, st.warehouse_id
                    ),
...
#     query = """
#
#             -- DROP FUNCTION public.inventory_stock_age_report_new(integer[], integer[], integer[], integer[]);
#             CREATE OR REPLACE FUNCTION public.inventory_stock_age_report_new(
#                 IN company_ids integer[],
#                 IN product_ids integer[],
#                 IN category_ids integer[],
#                 IN warehouse_ids integer[])
...
#                             sum(st.current_stock) as total_stock, sum(st.current_stock_value) as total_stock_value
#                         from stock_age_table st
#                         group by st.company_id, st.company_name, st.product_id, st.product_name, st.product_category_id, st.category_name, st.warehouse_id, st.in_date
#                     ),
#                     company_wise_stock_age_table as(
#                         Select sat.company_id, sum(sat.current_stock) as total_stock_qty, sum(sat.current_stock_value) as total_stock_value
#                         from stock_age_table sat
