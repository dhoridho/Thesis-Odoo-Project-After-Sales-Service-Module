...
#                     'period_id': period_id,
                ...
                if not installment.loan_id.journal_id1.default_debit_account_id:
                    raise UserError( _('Please configure Debit/Credit accounts on the Journal %s ') % (installment.loan_id.journal_id1.name))
                if not installment.loan_id.employee_loan_account:
...
#                 move_id.post()
    ...
    def unlink(self):
        raise UserError(_('You can not delete Loan Installment.'))
        return super(LoanInstallmentDetail, self).unlink()
...
#                            break
    ...
    def _employee_get(self):
        ids = self.env['hr.employee'].search([('user_id', '=', self._uid),('company_id','=',self.env.company.id)], limit=1)
        if ids:
            return ids
...
#         return super(EmployeeLoanDetails, self).copy(default)
    ...
    def onchange_employee_id(self, employee):
        ...
        policies_on_categ = []
        policies_on_empl = []
        for categ in employee_obj.sudo().category_ids:
...
#                     last_loan = self.browse(loans_list[0]) #probuse
                    ...
                    last_loan_date_obj = datetime.strptime(last_loan_date.strftime('%Y-%m-%d'), DEFAULT_SERVER_DATE_FORMAT)
            ...
            if policy.policy_type == 'eligible_duration':
                ...
                if not contract_date:
                    raise UserError(_('Please Set the contract date on employee.'))
                contract_date_obj = datetime.strptime(str(contract_date), '%Y-%m-%d')
    ...
    def unlink(self):
        for data in self:
            if data.state not in ['draft']:
                raise UserError(_('You can not delete Loan Request.'))
