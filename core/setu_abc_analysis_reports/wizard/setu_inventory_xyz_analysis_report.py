...
class SetuInventoryXYZAnalysisReport(models.TransientModel):
    ...
    def prepare_data_to_write(self, stock_data={}):
        ...
        for key, value in warehouse_wise_data.items():
            warehouse_wise_data[key] = dict(sorted(value.items(),
                                                   key=lambda item: item[1].get('stock_value', 0),
                                                   reverse=True))
    ...
    def download_report_in_listview(self):
        ...
        tree_view_id = self.env.ref('setu_abc_analysis_reports.setu_inventory_xyz_analysis_bi_report_tree').id
