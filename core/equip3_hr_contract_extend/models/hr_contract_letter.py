...
class HashmicroContractLetter(models.Model):
    _name = 'hr.contract.letter'
    ...
    career_letter_ids = fields.One2many('hr.contract.variables','contract_letter_id')
    ...
    #                 'career_tansition_id','create_date','create_uid','display_name','active','career_transition_id')
        ...
        for rec in self:
            if not rec.career_letter_ids:
                ...
                if not model:
                    raise ValidationError("model not found")
                dont_get = (
                    'message_needaction', 'message_main_attachment_id', 'message_is_follower', 'message_ids',
                    '__last_update', 'activity_date_deadline', 'activity_exception_decoration',
                    'activity_exception_icon', 'activity_ids', 'activity_state', 'activity_summary',
                    'activity_type_icon', 'activity_type_id', 'activity_user_id', 'approval_matrix_ids', 'id',
                    'is_employee_sequence_number', 'is_hide_approve', 'is_hide_confirm', 'is_hide_reject',
                    'message_attachment_count', 'message_channel_ids', 'message_follower_ids', 'message_has_error',
                    'message_has_error_counter', 'message_has_sms_error', 'message_ids', 'message_is_follower',
                    'message_main_attachment_id', 'message_needaction', 'write_uid', 'write_date',
                    'website_message_ids', 'same_as_previous', 'status', 'message_unread_counter', 'message_unread',
                    'message_partner_ids', 'message_needaction_counter', 'my_activity_date_deadline', 'kanban_state',
                    'calendar_mismatch', 'career_tansition_id', 'create_date', 'create_uid', 'display_name', 'active',
...
class HashmicroContractVariables(models.Model):
    _name = 'hr.contract.variables'
    variable_name = fields.Char()
    description = fields.Char()
    contract_letter_id = fields.Many2one('hr.contract.letter')