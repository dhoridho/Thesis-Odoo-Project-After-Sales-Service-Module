...
class SLPurchaseOrder(models.Model):
    _inherit = 'purchase.order'

    @api.depends('service_line_ids','service_line_ids.total','service_line_ids.subtotal',\
        'service_line_ids.quantity','discount_amount_sl',\
        'discount_method_sl','discount_type' ,'service_line_ids.discount_amount',\
        'service_line_ids.discount_method')
    def _sl_amount_all(self):
        ...
        for order in self:
            applied_discount = line_discount = sums = order_discount =  sl_amount_untaxed = sl_amount_tax  = 0.0
            ...
            if res_config:
                if res_config.tax_discount_policy == 'tax':
                    if order.discount_type == 'line':
                        order.sl_discount_amt = 0.00
                        order.update({
                            'sl_amount_untaxed': sl_amount_untaxed,
                            'sl_amount_tax': sl_amount_tax,
                            'sl_amount_total': sl_amount_untaxed + sl_amount_tax - line_discount,
                            'sl_discount_amt_line' : line_discount,
                        })
                    elif order.discount_type == 'global':
                        order.sl_discount_amt_line = 0.00
                        if order.discount_method_sl == 'percentage':
                            order_discount = sl_amount_untaxed * (order.discount_amount_sl / 100) 
                            
                            order.update({
                                'sl_amount_untaxed': sl_amount_untaxed,
                                'sl_amount_tax': sl_amount_tax,
                                'sl_amount_total': sl_amount_untaxed + sl_amount_tax - order_discount,
                                'sl_discount_amt' : order_discount,
                            })
                        elif order.discount_method_sl == 'fixed':
                            order_discount = order.discount_amount_sl
                            order.update({
                                'sl_amount_untaxed': sl_amount_untaxed,
                                'sl_amount_tax': sl_amount_tax,
                                'sl_amount_total': sl_amount_untaxed + sl_amount_tax - order_discount,
                    ...
                    if order.discount_type == 'line':
                        ...
                        order.update({
                            'sl_amount_untaxed': sl_amount_untaxed,
                            'sl_amount_tax': sl_amount_tax,
                            'sl_amount_total': sl_amount_untaxed + sl_amount_tax - applied_discount,
                            'sl_discount_amt_line' : applied_discount,
                        })
                    elif order.discount_type == 'global':
                        order.sl_discount_amt_line = 0.00
                        if order.discount_method_sl == 'percentage':
                            order_discount = sl_amount_untaxed * (order.discount_amount_sl / 100)
                            if order.service_line_ids:
                                for line in order.service_line_ids:
                                    if line.taxes:
                                        final_discount = 0.0
                                        try:
                                            final_discount = ((order.discount_amount_sl*line.subtotal)/100.0)
                            ...
                            order.update({
                                'sl_amount_untaxed': sl_amount_untaxed,
                                'sl_amount_tax': sums,
                                'sl_amount_total': sl_amount_untaxed + sums - order_discount,
                                'sl_discount_amt' : order_discount,  
                            })
                        elif order.discount_method_sl == 'fixed':
                            order_discount = order.discount_amount_sl
                            if order.service_line_ids:
                                for line in order.service_line_ids:
                                    if line.taxes:
                                        final_discount = 0.0
                                        try:
                                            final_discount = ((order.discount_amount_sl*line.subtotal)/sl_amount_untaxed)
                                        except ZeroDivisionError:
                                            pass
                                        discount = line.subtotal - final_discount
    ...
    #         'discount_method_sl': self.discount_method_sl,
    #         'discount_type' : self.discount_type,
    #         'discount_amount_sl':self.discount_amount_sl,
    #         'sl_discount_amt': self.sl_discount_amt,
    #         'sl_discount_amt_line': self.sl_discount_amt_line,
    #     }
    #     return invoice_vals            
    ...
    #     moves.filtered(lambda m: m.currency_id.round(m.sl_amount_total) < 0).action_switch_invoice_into_refund_credit_note()
    #     for line in moves.line_ids:
    #         name = line.name
    #     if moves.discount_type == 'line':
    #         price = moves.sl_discount_amt_line
    #     elif moves.discount_type == 'global':
    #         price = moves.sl_discount_amt
    #     else:
    #         price = 0
    #     if moves.line_ids:
    #         if name != 'Discount':
    ...
    #         'discount_type' : self.discount_type,
    #         'sl_discount_amt_line' : self.sl_discount_amt_line,
    #         'sl_amount_untaxed' : self.sl_amount_untaxed,
    #         'sl_amount_total': self.sl_amount_total,
    ...
    #     new_po = super(SLPurchaseOrder, self).copy(default=default)
    #     for line in new_po.service_line_ids:
    #         if line.product:
    #             seller = line.product._select_seller(
    #                 partner_id=line.partner_id, quantity=line.quantity,
    #                 date=line.service_id.date_order and line.service_id.date_order.date(), uom_id=line.uom)
    #             line.date_planned = line._get_date_planned(seller)
    #     return new_po


class SLPurchaseOrderLine(models.Model):
    _inherit = 'rfq.service.line'
                    ...
                    if line.discount_type == 'line':
                        if line.discount_method == 'fixed':
                            ...
                            line.update({
                                ...
                                'sl_discount_amt' : line.discount_amount,
                            })
    ...
    #     res =super(SLPurchaseOrderLine,self)._prepare_account_move_line(move)
