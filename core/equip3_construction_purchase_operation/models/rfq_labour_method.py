...
class LLPurchaseOrder(models.Model):
    ...
    @api.depends('labour_line_ids','labour_line_ids.total','labour_line_ids.subtotal',\
        'labour_line_ids.quantity','discount_amount_ll',\
        'discount_method_ll','discount_type' ,'labour_line_ids.discount_amount',\
        'labour_line_ids.discount_method')
    def _ll_amount_all(self):
        ...
        for order in self:
            applied_discount = line_discount = sums = order_discount =  ll_amount_untaxed = ll_amount_tax  = 0.0
            ...
            if res_config:
                if res_config.tax_discount_policy == 'tax':
                    if order.discount_type == 'line':
                        order.ll_discount_amt = 0.00
                        order.update({
                            'll_amount_untaxed': ll_amount_untaxed,
                            'll_amount_tax': ll_amount_tax,
                            'll_amount_total': ll_amount_untaxed + ll_amount_tax - line_discount,
                            'll_discount_amt_line' : line_discount,
                        })
                    elif order.discount_type == 'global':
                        order.ll_discount_amt_line = 0.00
                        if order.discount_method_ll == 'percentage':
                            order_discount = ll_amount_untaxed * (order.discount_amount_ll / 100) 
                            
                            order.update({
                                'll_amount_untaxed': ll_amount_untaxed,
                                'll_amount_tax': ll_amount_tax,
                                'll_amount_total': ll_amount_untaxed + ll_amount_tax - order_discount,
                                'll_discount_amt' : order_discount,
                            })
                        elif order.discount_method_ll == 'fixed':
                            order_discount = order.discount_amount_ll
                            order.update({
                                'll_amount_untaxed': ll_amount_untaxed,
                                'll_amount_tax': ll_amount_tax,
                                'll_amount_total': ll_amount_untaxed + ll_amount_tax - order_discount,
                    ...
                    if order.discount_type == 'line':
                        ...
                        order.update({
                            'll_amount_untaxed': ll_amount_untaxed,
                            'll_amount_tax': ll_amount_tax,
                            'll_amount_total': ll_amount_untaxed + ll_amount_tax - applied_discount,
                            'll_discount_amt_line' : applied_discount,
                        })
                    elif order.discount_type == 'global':
                        order.ll_discount_amt_line = 0.00
                        if order.discount_method_ll == 'percentage':
                            order_discount = ll_amount_untaxed * (order.discount_amount_ll / 100)
                            if order.labour_line_ids:
                                for line in order.labour_line_ids:
                                    if line.taxes:
                                        final_discount = 0.0
                                        try:
                                            final_discount = ((order.discount_amount_ll*line.subtotal)/100.0)
                            ...
                            order.update({
                                'll_amount_untaxed': ll_amount_untaxed,
                                'll_amount_tax': sums,
                                'll_amount_total': ll_amount_untaxed + sums - order_discount,
                                'll_discount_amt' : order_discount,  
                            })
                        elif order.discount_method_ll == 'fixed':
                            order_discount = order.discount_amount_ll
                            if order.labour_line_ids:
                                for line in order.labour_line_ids:
                                    if line.taxes:
                                        final_discount = 0.0
                                        try:
                                            final_discount = ((order.discount_amount_ll*line.subtotal)/ll_amount_untaxed)
                                        except ZeroDivisionError:
                                            pass
                                        discount = line.subtotal - final_discount
    ...
    #         'discount_method_ll': self.discount_method_ll,
    #         'discount_type' : self.discount_type,
    #         'discount_amount_ll':self.discount_amount_ll,
    #         'll_discount_amt': self.ll_discount_amt,
    #         'll_discount_amt_line': self.ll_discount_amt_line,
    #     }
    #     return invoice_vals            
    ...
    #     moves.filtered(lambda m: m.currency_id.round(m.ll_amount_total) < 0).action_switch_invoice_into_refund_credit_note()
    #     for line in moves.line_ids:
    #         name = line.name
    #     if moves.discount_type == 'line':
    #         price = moves.ll_discount_amt_line
    #     elif moves.discount_type == 'global':
    #         price = moves.ll_discount_amt
    #     else:
    #         price = 0
    #     if moves.line_ids:
    #         if name != 'Discount':
    ...
    #         'discount_type' : self.discount_type,
    #         'll_discount_amt_line' : self.ll_discount_amt_line,
    #         'll_amount_untaxed' : self.ll_amount_untaxed,
    #         'll_amount_total': self.ll_amount_total,
    ...
    #     new_po = super(LLPurchaseOrder, self).copy(default=default)
    #     for line in new_po.labour_line_ids:
    #         if line.product:
    #             seller = line.product._select_seller(
    #                 partner_id=line.partner_id, quantity=line.quantity,
    #                 date=line.labour_id.date_order and line.labour_id.date_order.date(), uom_id=line.uom)
    #             line.date_planned = line._get_date_planned(seller)
    #     return new_po


class LLPurchaseOrderLine(models.Model):
    _inherit = 'rfq.labour.line'
                    ...
                    if line.discount_type == 'line':
                        if line.discount_method == 'fixed':
                            ...
                            line.update({
                                ...
                                'll_discount_amt' : line.discount_amount,
                            })
    ...
    #     res =super(LLPurchaseOrderLine,self)._prepare_account_move_line(move)
