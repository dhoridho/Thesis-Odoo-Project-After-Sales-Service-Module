...
class OLPurchaseOrder(models.Model):
    _inherit = 'purchase.order'

    @api.depends('overhead_line_ids','overhead_line_ids.total','overhead_line_ids.subtotal',\
        'overhead_line_ids.quantity','discount_amount_ol',\
        'discount_method_ol','discount_type' ,'overhead_line_ids.discount_amount',\
        'overhead_line_ids.discount_method')
    def _ol_amount_all(self):
        ...
        for order in self:
            applied_discount = line_discount = sums = order_discount =  ol_amount_untaxed = ol_amount_tax  = 0.0
            ...
            if res_config:
                if res_config.tax_discount_policy == 'tax':
                    if order.discount_type == 'line':
                        order.ol_discount_amt = 0.00
                        order.update({
                            'ol_amount_untaxed': ol_amount_untaxed,
                            'ol_amount_tax': ol_amount_tax,
                            'ol_amount_total': ol_amount_untaxed + ol_amount_tax - line_discount,
                            'ol_discount_amt_line' : line_discount,
                        })
                    elif order.discount_type == 'global':
                        order.ol_discount_amt_line = 0.00
                        if order.discount_method_ol == 'percentage':
                            order_discount = ol_amount_untaxed * (order.discount_amount_ol / 100) 
                            
                            order.update({
                                'ol_amount_untaxed': ol_amount_untaxed,
                                'ol_amount_tax': ol_amount_tax,
                                'ol_amount_total': ol_amount_untaxed + ol_amount_tax - order_discount,
                                'ol_discount_amt' : order_discount,
                            })
                        elif order.discount_method_ol == 'fixed':
                            order_discount = order.discount_amount_ol
                            order.update({
                                'ol_amount_untaxed': ol_amount_untaxed,
                                'ol_amount_tax': ol_amount_tax,
                                'ol_amount_total': ol_amount_untaxed + ol_amount_tax - order_discount,
                    ...
                    if order.discount_type == 'line':
                        ...
                        order.update({
                            'ol_amount_untaxed': ol_amount_untaxed,
                            'ol_amount_tax': ol_amount_tax,
                            'ol_amount_total': ol_amount_untaxed + ol_amount_tax - applied_discount,
                            'ol_discount_amt_line' : applied_discount,
                        })
                    elif order.discount_type == 'global':
                        order.ol_discount_amt_line = 0.00
                        if order.discount_method_ol == 'percentage':
                            order_discount = ol_amount_untaxed * (order.discount_amount_ol / 100)
                            if order.overhead_line_ids:
                                for line in order.overhead_line_ids:
                                    if line.taxes:
                                        final_discount = 0.0
                                        try:
                                            final_discount = ((order.discount_amount_ol*line.subtotal)/100.0)
                            ...
                            order.update({
                                'ol_amount_untaxed': ol_amount_untaxed,
                                'ol_amount_tax': sums,
                                'ol_amount_total': ol_amount_untaxed + sums - order_discount,
                                'ol_discount_amt' : order_discount,  
                            })
                        elif order.discount_method_ol == 'fixed':
                            order_discount = order.discount_amount_ol
                            if order.overhead_line_ids:
                                for line in order.overhead_line_ids:
                                    if line.taxes:
                                        final_discount = 0.0
                                        try:
                                            final_discount = ((order.discount_amount_ol*line.subtotal)/ol_amount_untaxed)
                                        except ZeroDivisionError:
                                            pass
                                        discount = line.subtotal - final_discount
    ...
    #         'discount_method_ol': self.discount_method_ol,
    #         'discount_type' : self.discount_type,
    #         'discount_amount_ol':self.discount_amount_ol,
    #         'ol_discount_amt': self.ol_discount_amt,
    #         'ol_discount_amt_line': self.ol_discount_amt_line,
    #     }
    #     return invoice_vals            
    ...
    #     moves.filtered(lambda m: m.currency_id.round(m.ol_amount_total) < 0).action_switch_invoice_into_refund_credit_note()
    #     for line in moves.line_ids:
    #         name = line.name
    #     if moves.discount_type == 'line':
    #         price = moves.ol_discount_amt_line
    #     elif moves.discount_type == 'global':
    #         price = moves.ol_discount_amt
    #     else:
    #         price = 0
    #     if moves.line_ids:
    #         if name != 'Discount':
    ...
    #         'discount_type' : self.discount_type,
    #         'ol_discount_amt_line' : self.ol_discount_amt_line,
    #         'ol_amount_untaxed' : self.ol_amount_untaxed,
    #         'ol_amount_total': self.ol_amount_total,
    ...
    #     new_po = super(OLPurchaseOrder, self).copy(default=default)
    #     for line in new_po.overhead_line_ids:
    #         if line.product:
    #             seller = line.product._select_seller(
    #                 partner_id=line.partner_id, quantity=line.quantity,
    #                 date=line.overhead_id.date_order and line.overhead_id.date_order.date(), uom_id=line.uom)
    #             line.date_planned = line._get_date_planned(seller)
    #     return new_po


class OLPurchaseOrderLine(models.Model):
    _inherit = 'rfq.overhead.line'
                    ...
                    if line.discount_type == 'line':
                        if line.discount_method == 'fixed':
                            ...
                            line.update({
                                ...
                                'ol_discount_amt' : line.discount_amount,
                            })
    ...
    #     res =super(OLPurchaseOrderLine,self)._prepare_account_move_line(move)
