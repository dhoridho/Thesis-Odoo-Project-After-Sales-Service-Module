...
class ApprovalMatrixPurchaseRequestOverbudget(models.Model):
    _name = "approval.matrix.purchase.request.overbudget"
    _description = "Approval Matrix Purchase Reqest Overbudget"
    ...
    approval_matrix_purchase_request_overbudget_line_ids = fields.One2many('approval.matrix.purchase.request.overbudget.line', 'approval_matrix_purchase_request_overbudget_id', string='Approving Matrix Purchase Request Overbudget Lines')


    @api.constrains('approval_matrix_purchase_request_overbudget_line_ids')
    def _check_validation_minimum_approver(self):
        for record in self:
            for approval_matrix_line in record.approval_matrix_purchase_request_overbudget_line_ids:
...
class ApprovalMatrixPurchaseRequestOverbudgetLine(models.Model):
    _name = "approval.matrix.purchase.request.overbudget.line"
    _description = "Approval Matrix Purchase Request Overbudget Line"

    @api.model
    def default_get(self, fields):
        res = super(ApprovalMatrixPurchaseRequestOverbudgetLine, self).default_get(fields)
        if self._context:
            context_keys = self._context.keys()
            next_sequence = 1
            if 'approval_matrix_purchase_request_overbudget_line_ids' in context_keys:
                if len(self._context.get('approval_matrix_purchase_request_overbudget_line_ids')) > 0:
                    next_sequence = len(self._context.get('approval_matrix_purchase_request_overbudget_line_ids')) + 1
    ...
    approval_matrix_purchase_request_overbudget_id = fields.Many2one('approval.matrix.purchase.request.overbudget', string="Approval Matrix")
    request_id = fields.Many2one('purchase.request', string="Purchase Request")
    approved_users = fields.Many2many('res.users', 'approved_users_purchase_request_overbudget_patner_rel', 'request_id', 'user_id', string='Users')
    ...
    def unlink(self):
        approval = self.approval_matrix_purchase_request_overbudget_id
        res = super(ApprovalMatrixPurchaseRequestOverbudgetLine, self).unlink()
        approval._reset_sequence()
        return res

    @api.model
    def create(self, vals):
        res = super(ApprovalMatrixPurchaseRequestOverbudgetLine, self).create(vals)
        if not self.env.context.get("keep-line_sequence", False):
            res.approval_matrix_purchase_request_overbudget_id._reset_sequence()
        return res