...
class AccountBudgetWizard(models.TransientModel):
    _name = "budget.change.req.wizard"
    _description = 'Account Budget Change Request Wizard'

    reason = fields.Text(string="Reason")

    def action_reject(self):
        account_budget_change_request_id = self.env['budget.change.req'].browse([self._context.get('active_id')])
        user = self.env.user
        ...
        if approving_matrix_line:
            ...
            template_id = self.env.ref('equip3_accounting_budget.email_template_rejection_of_budget_change_request')
            # wa_template_id_rejected = self.env.ref('equip3_accounting_budget.wa_template_rejection_of_budget_change_request')
            base_url = self.env['ir.config_parameter'].sudo().get_param('web.base.url')
            url = base_url + '/web#id='+ str(account_budget_change_request_id.id) + '&action='+ str(action_id.id) + '&view_type=form&model=budget.change.req'
            ctx = {
            ...
            if account_budget_change_request_id.is_allowed_to_wa_notification:
                account_budget_change_request_id._send_wa_reject_budget_change_req(account_budget_change_request_id.request_partner_id.name, phone_num, account_budget_change_request_id.create_date.date(), self.env.user.name, self.reason)
