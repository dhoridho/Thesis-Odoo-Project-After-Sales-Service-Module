odoo.define('equip3_pos_payment_edc.PaymentLineEdcCreditCard', function (require) {
    'use strict';
    ...
    class PaymentLineEdcCreditCard extends PosComponent {
        constructor() {
            super(...arguments);
        }
        
        getInstallmentTenor(paymentline){
            const order = this.env.pos.get_order();
            if (order.installment_tenor){
                return parseInt(order.installment_tenor);
            }
            return false;
        }

        async sendRequest(paymentline) {
            const self = this
            const order = this.env.pos.get_order()
            ...
            console.log('PaymentLineEdcCreditCard: Click sendRequest:', 
                    'is_edc_bca(', payment.is_edc_bca,') ', 
                    'installment_tenor(', order.installment_tenor, ')');
            if(!payment || (payment && !payment.is_edc_bca) || !order.installment_tenor){
                return;
            }

            //TODO: Sent Request data to Edc device
            if(order.installment_tenor){
                ...
                let data = {
                    'data': {
                        ...
                        'version': '03',
                        'trans_type': payment.trans_type_code,
                        ...
                        'redeem_flag': '',
                        'dcc_fllag': 'N',
                        'installment_plan': payment.installment_plan,
                        'installment_tenor': order.installment_tenor,
                        'generic_data': '',
                        'reff_number': '',
                        'original_date': '',
                ...
                if(result.status == 'waiting'){
                    ...
                    console.log('Payment EDC: - ' , payment);
                    ...
                    paymentline.installment_amount = payment.data.info_amount;
                    paymentline.installment_tenor = order.installment_tenor;
        ...
        async checkPaymentEDC(order_number){
            const self = this
            timerCounter = parseInt(countdownEDC / 5) + 1;
            return new Promise(function (resolve, reject) {
                clearInterval(timer);
                timer = setInterval(function () {
                    --timerCounter;
                    let vals = {
                        'domain': [['order_number','=',order_number], ['trans_type','=','01']],
                        'fields': ['id','payment_state','resp_code','invoice_number','order_number','approval_code', 'info_amount'],
                        'limit': 1,
                    ...
                    self.rpc({
                        ...
                        console.log('Check Payment EDC: (', order_number, ') - ' ,timerCounter, ' - ' , payments)
            ...
            $('body').append(`
                <div class="edc-payment-loader-wrap">
                    <div style=" display: block; text-align: center;">
                        ...
                        <div id="edc-payment-countdown" class="edc-payment-countdown" style="display:none;">05:00</div>
    ...
    PaymentLineEdcCreditCard.template = 'PaymentLineEdcCreditCard';
    Registries.Component.add(PaymentLineEdcCreditCard);
    return PaymentLineEdcCreditCard;
});