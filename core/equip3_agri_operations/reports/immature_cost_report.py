...
class ImmatureCostReport(models.Model):
    _inherit = 'agriculture.list.report'
    _name = 'agriculture.immature.cost.report'
    _description = 'Immature Cost Report'
    _auto = False
    ...
    def _get_year_selection(self):
        record_ids = self.env['agriculture.daily.activity.record'].search([('crop_age', '=', 'TBM')])
...
class ReportImmatureCostReport(models.AbstractModel):
    _name = 'report.equip3_agri_operations.agriculture_immature_cost_report'
    _description = 'Report Immature Cost Report'
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if 'report_data' not in data: 
            report_data = self.env['agriculture.immature.cost.report'].get_report_values()
        else:
