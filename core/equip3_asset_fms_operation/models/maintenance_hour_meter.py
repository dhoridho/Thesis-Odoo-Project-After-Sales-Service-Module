...
class MaintenanceHourMeter(models.Model):
    ...
    def get_total_value_hourmeter(self):
        for record in self:
            maintenance_id = self.search([('maintenance_asset', '=', record.maintenance_asset.id)])            
            floorodoo_value = 0
            ...
            maintenance_plan = self.env['maintenance.plan'].search(['&','&','&',('start_date', '<=', record.date), 
                                                                                ...
                                                                                ('state','=','active'),
                                                                                ('is_hourmeter_m_plan','=', True)])
            for plan in maintenance_plan:
                for plan_ids in plan.task_check_list_ids:
                    for line in record.maintenance_asset.frequency_hourmeter_ids.filtered(lambda line: line.is_hourmeter_m_plan.id == plan.id):
                        if record.maintenance_asset.id == plan_ids.equipment_id.id:
                            new_floor_value = total_odometer // plan.maintenance_frequency_ids
                            if plan.maintenance_frequency_ids > 0 and new_floor_value > line.floorodoo_value:
                    ...
                    for plan_ids in plan.task_check_list_ids:
                        if record.maintenance_asset.id == plan_ids.equipment_id.id:
                            if plan.maintenance_threshold_ids:
                                for line in record.maintenance_asset.threshold_hourmeter_ids.filtered(lambda line: line.is_hourmeter.id == plan.id):
