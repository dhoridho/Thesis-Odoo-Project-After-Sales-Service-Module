...
class CouponProgram(models.Model):
    ...
    def _compute_order_count(self):
        """For getting the no of times a coupon is used."""
        # product_data = self.env['sale.order.line'].read_group([('product_id', 'in', self.mapped(
        #     'discount_line_product_id').ids)], ['product_id'], ['product_id'])
        product_data = self.env['other.items'].read_group([('product_id', 'in', self.mapped(
            'discount_line_product_id').ids)], ['product_id'], ['product_id'])
        print("compute product data>>>>>>>>>>>>>", product_data)
        mapped_data = dict(
            [(m['product_id'][0], m['product_id_count']) for m in product_data])
        print("compute mapped data>>>>>>>>>>>>>", mapped_data)
        for program in self:
            program.order_count = mapped_data.get(
                program.discount_line_product_id.id, 0)
            print("compute program data>>>>>>>>>>>>>", program.order_count)

    @api.model
    def _filter_on_mimimum_amount(self, order):
        print("overridden>>>>>>>>>>>>>>>>>", order)
        # import pdb
        # pdb.set_trace()
        no_effect_lines = order._get_no_effect_on_threshold_lines()
        print("no_effect_line>>>>>>>>>", no_effect_lines)
        order_amount = {
            'amount_untaxed': order.untaxed_amt,
            'amount_tax': order.total_tax,
        }
        program_ids = list()
        for program in self:
            print("rewardtype>>>>>>>>>", program.reward_type)
            if program.reward_type != 'discount':
                # avoid the filtered
                lines = self.env['other.items']
                print("lines>>>>>>>>>>>>>", lines)
            else:
                lines = order.other_items_ids.filtered(lambda line:
                                                       line.product_id == program.discount_line_product_id or
                                                       line.product_id == program.reward_id.discount_line_product_id
                                                       )
            untaxed_amount = order_amount['amount_untaxed'] - \
                sum(line.price_subtotal for line in lines)
            print("untaxed_maount>>>>>>>>>>", untaxed_amount)
            tax_amount = order_amount['amount_tax'] - \
                sum(line.tax_id.amount for line in lines)
            print("tex>>>>>>>>>>>>", tax_amount)
            program_amount = program._compute_program_amount(
                'rule_minimum_amount', order.currency_id)
            print(")))))))))))))))))))))))))))))))))))))))))))))))))")
    ...
    def _filter_programs_on_products(self, order):
        ...
        """
        print("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< _filter_programs_on_products >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")

        order_lines = order.other_items_ids.filtered(
            lambda line: line.product_id) - order._get_reward_lines()

        products = order_lines.mapped('product_id')
        products_qties = dict.fromkeys(products, 0)
        print("products>>>>>>>>>>qtities>>>>>>>>>>", products, products_qties)
    ...
    def _check_promo_code(self, order, coupon_code):
        print("chekpromocode>>>>>>>>>>", order, coupon_code)
        message = {}
        print("promsssssssssssss>>>>>>>>>>", self.promo_code,
              order.promo_code, order.no_code_promo_program_ids)
        if self.maximum_use_number != 0 and self.order_count >= self.maximum_use_number:
            ...
            message = {'error': _(
                'The promo code is already applied on this order')}
        elif self in order.no_code_promo_program_ids:
            message = {'error': _(
                'The promotional offer is already applied on this order')}
            ...
            print("global discount>>>>>>>>>>>>", self._is_global_discount_program(
            ), order._is_global_discount_already_applied())
            ...
            print("_is_reward_in_order_lines booolean>>>>>>>>>>>>>>>",
                  order._is_reward_in_order_lines(self))
            message = {'error': _(
    ...
    def _filter_programs_from_common_rules(self, order, next_order=False):
        ...
        if not next_order:
            programs = programs and programs._filter_programs_on_products(
                order)
        ...
        if programs_curr_order:
            ...
            programs += programs_curr_order._filter_not_ordered_reward_programs(
                order)
        return programs
