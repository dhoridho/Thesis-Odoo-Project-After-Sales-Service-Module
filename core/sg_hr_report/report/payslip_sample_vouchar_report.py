...
class PayslipSampleReport(models.AbstractModel):
    _name = 'report.sg_hr_report.report_payslip_sample_voucher'
    _description = "Payslip Sample Voucher Report"
    ...
    def get_category(self, curr_id, code):
        ...
        pay_slip_line_ids = payslip_line.search([('slip_id', '=', curr_id.id)])
    ...
    def category_line(self, curr_ids, code, overtime_code, code_tittle):
        ...
        hr_sal_rule_categ_ids = rule_category_obj.search([('code', '=', code)])
        hr_sal_rule_ids = rule_obj.search([('category_id', 'in',
                                            hr_sal_rule_categ_ids.ids)])
        if hr_sal_rule_ids:
            ...
            if sal_rule_code_list:
                payslip_line_rec = payslip_line_obj.search([('slip_id', '=',
                                                             curr_ids.id),
                                                            ('code', 'in',
                                                             sal_rule_code_list
                                                             )])
