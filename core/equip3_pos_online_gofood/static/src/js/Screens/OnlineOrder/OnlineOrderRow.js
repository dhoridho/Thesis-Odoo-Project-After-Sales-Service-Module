odoo.define('equip3_pos_online_gofood.OnlineOrderRow', function (require) {
    'use strict';
    ...
    const OnlineOrderRowExt = (OnlineOrderRow) =>
        class extends OnlineOrderRow {
            ...
            async actionAcceptOrder(onlineOrder) {
                ...
                let self = this;
	            if(onlineOrder.order_from == 'gofood'){
            		framework.blockUI();
	                ...
	                await this.rpc({
	                    ...
	                    method: 'action_accept_gofood_order',
	                    args: [onlineOrder.id],
	                }).then(function (resp) { 
            ...
            async actionRejectOrder(onlineOrder){
	            ...
	            if(onlineOrder.order_from == 'gofood'){
	                ...
	                await this.rpc({
	                    model: 'pos.online.outlet.order',
	                    method: 'action_reject_gofood_order',
	                    args: [onlineOrder.id],
	                }).then(function (resp) { 
	                    if(resp.content.status == 'failed'){
	                        self.showPopup('ErrorPopup', {
            ...
            async actionMarkOrderReady(onlineOrder){
	            ...
	            if(onlineOrder.order_from == 'gofood'){
                    ...
                    if (confirmed) {
		                ...
		                await this.rpc({
		                    model: 'pos.online.outlet.order',
		                    method: 'action_mark_ready_gofood_order',
		                    args: [onlineOrder.id],
		                }).then(function (resp) { 
		                    if(resp.content.status == 'failed'){
		                        self.showPopup('ErrorPopup', {
