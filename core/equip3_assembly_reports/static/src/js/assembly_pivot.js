odoo.define('equip3_assem3bly_reports.AssemblyPivotView', function (require) {
"use strict";
...
var assemblyPivotView = PivotView.extend({
	...
	init: function(viewInfo, params) {
		...
		for(var field_name in this.fields){
			...
			for (var attr in this.fields[field_name]){
				newFields[field_name][attr] = this.fields[field_name][attr];
			}
		}
		this.arch.children.forEach(field => {
			if (field.attrs.string){
				newFields[field.attrs.name].string = field.attrs.string;
			}
		});
        ...
        this.arch.children.forEach(field => {
            ...
            if (field.attrs.add_string && name in measures) {
                measures[name].string = field.attrs.add_string;
            }
...
viewRegistry.add('assembly_pivot', assemblyPivotView);
