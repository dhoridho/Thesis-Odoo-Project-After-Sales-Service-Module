...
class SaleOrderPartnerCreditConst(models.TransientModel):
    _name = 'sale.order.const.partner.credit'
    _description = 'Sale Order Const Partner Credit'

    name = fields.Many2one("sale.order.const", "Sale Order", readonly=True)
    ...
    def default_get(self, fields):
        res = super(SaleOrderPartnerCreditConst, self).default_get(fields)
        ...
        if self._context.get('active_id', False) and self._context.get('active_model', False) == 'sale.order.const':
            # sale_obj = self.env['sale.order.const'].browse(self.env.context.get('active_id'))
            sale_obj = self.env['sale.order.const'].search(
            ...
            if sale_obj:
                ...
                so_pend_obj = self.env['sale.order.const'].search(
    ...
    def ok_credit_and_invoice_limit(self):
        ...
        if int(over_credit_limit_sequence) < int(invoice_overdue_sequence):
            ...
            self.with_context(context).ok_invoice_overdue()
            self.with_context(context).ok_sale_customer_credit()
        if not self.name.approving_matrix_limit_id and not self.name.is_customer_approval_matrix_const:
            is_send_mail = True
            self.name.order_confirm()
        elif self.name.is_customer_approval_matrix_const and not self.name.approving_matrix_limit_id:
            is_send_mail = True
    ...
    def ok_invoice_overdue(self):
        ...
        matrix_limit_id = self.env['limit.approval.matrix'].search([('minimum_amt', '<=', total_days), ('maximum_amt', '>=', total_days),
            ('config', '=', 'max_invoice_overdue_days'), ('company_id', '=', self.name.company_id.id)], limit=1)
        is_send_mail = False
        if matrix_limit_id:
            is_send_mail = True
            self.name.approving_matrix_limit_id = [(4, matrix_limit_id.id)]
            self.name.write({'state': 'waiting_for_over_limit_approval'})
        elif not context.get('double_overlimit_check') and not self.name.is_customer_approval_matrix_const:
            is_send_mail = True
            self.name.order_confirm()
        elif self.name.is_customer_approval_matrix_const and not context.get('double_overlimit_check'):
            is_send_mail = True
            self.name.write({'state': 'over_limit_approved'})
            self.name.action_request_for_approval_limit()
    ...
    def ok_sale_customer_credit(self):
        ...
        matrix_limit_id = self.env['limit.approval.matrix'].search([('minimum_amt', '<=', available_credit_limit), ('maximum_amt', '>=', available_credit_limit),
            ('config', '=', 'credit_limit'), ('company_id', '=', self.name.company_id.id)], limit=1)
        is_send_mail = False
        if matrix_limit_id:
            is_send_mail = True
            self.name.approving_matrix_limit_id = [(4, matrix_limit_id.id)]
    ...
    def ok_invoice_open(self):
        ...
        matrix_limit_id = self.env['limit.approval.matrix'].search([('minimum_amt', '<=', available_open_invoice_limit), ('maximum_amt', '>=', available_open_invoice_limit),
            ('config', '=', 'open_invoice_limit'), ('company_id', '=', self.name.company_id.id)], limit=1)
        is_send_mail = False
        if matrix_limit_id:
            is_send_mail = True
            self.name.approving_matrix_limit_id = [(4, matrix_limit_id.id)]
    ...
    def onhold_sale_order(self):
        if self and self.name and self.order_partner:
            ...
            sale_obj = self.env['sale.order.const'].search([('id','=',self.name.id)])
