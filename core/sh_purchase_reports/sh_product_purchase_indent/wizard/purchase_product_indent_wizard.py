...
class PurchaseProductIndentXLS(models.Model):
    _name = 'sh.purchase.product.indent.xls'
    _description = 'Purchase Product Indent Xls Report'
    ...
    def download_report(self):
        return{
            ...
            'url': 'web/content/?model=sh.purchase.product.indent.xls&field=excel_file&download=true&id=%s&filename=%s' % (self.id, self.file_name),
            'target': 'new',
        }


class PurchaseProductIndentWizard(models.TransientModel):
    _name = 'sh.purchase.product.indent.wizard'
    _description = 'Purchase Product Indent Wizard'
    ...
    sh_status = fields.Selection([('all', 'All'), ('draft', 'Draft'), ('sent', 'RFQ Sent'), (
        'purchase', 'Purchase Order'), ('done', 'Locked')], default='all', string='Status')
    ...
    def print_report(self):
        datas = self.read()[0]
        return self.env.ref('sh_purchase_reports.sh_purchase_product_indent_action').report_action([], data=datas)
