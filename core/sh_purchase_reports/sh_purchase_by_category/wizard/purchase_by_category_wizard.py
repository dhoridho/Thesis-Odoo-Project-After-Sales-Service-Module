...
class PurchaseByCategoryXLS(models.Model):
    _name = 'sh.purchase.category.xls'
    _description = 'Purchase by Category Xls Report'
    ...
    def download_report(self):
        return{
            ...
            'url': 'web/content/?model=sh.purchase.category.xls&field=excel_file&download=true&id=%s&filename=%s' % (self.id, self.file_name),
            'target': 'new',
        }


class PurchaseByCategoryWizard(models.TransientModel):
    _name = 'sh.purchase.category.wizard'
    _description = 'Purchase By Category Wizard'
    ...
    def print_report(self):
        datas = self.read()[0]
        return self.env.ref('sh_purchase_reports.sh_po_by_category_action').report_action([], data=datas)
