...
class Survey(http.Controller):
    ...
    def _redirect_with_error(self, access_data, error_key):
        ...
        if error_key == 'survey_void' and access_data['can_answer']:
            return request.render("equip3_hr_survey_extend.survey_void_content", {'survey': survey_sudo, 'answer': answer_sudo})
    ...
    def survey_start(self, survey_token, answer_token=None, email=False, **post):
        ...
        if not answer_sudo:
            try:
                ...
                return request.render("equip3_hr_survey_extend.survey_403_page", {'survey': survey_sudo})
    ...
    def _prepare_question_html(self, survey_sudo, answer_sudo, **post):
        ...
        if answer_sudo.state == 'done':
            ...
            survey_content = request.env.ref('equip3_hr_survey_extend.survey_fill_form_in_progress')._render(survey_data)
    ...
    def survey_display_page(self, survey_token, answer_token, **post):
        ...
        return request.render('equip3_hr_survey_extend.survey_page_fill',
    ...
    def survey_submit(self, survey_token, answer_token, **post):
        ...
        for question in questions:
            if question in inactive_questions.ids:  # if question is inactive, skip validation and save
                continue
            _logger.info('str(question.id)): ' + str(question.id))    
            _logger.info('str(post)): ' + str(post))            
            _logger.info('post.get(str(question.id)): ' + str(post.get(str(question.id))))
