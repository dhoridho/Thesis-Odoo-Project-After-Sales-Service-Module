...
odoo.define('equip3_hr_survey_extend.survey.timer', function (require) {
    "use strict";

    var surveyTimer = require('survey.timer');

    var DataSet = surveyTimer.include({
        /**
         * @override
         */
        start: function () {
        var self = this;
            self.countDownDate = moment.utc(self.timer).add(self.timeLimitMinutes, 'minutes');
            var timeLeft = self.timeLimitMinutes
            if (timeLeft != false) {
                timeLeft *= 60
            }
            ...
            if (self.timeLimitMinutes <= 0 || self.countDownDate.diff(moment.utc(), 'seconds') < 0) {
                ...
                var timerInterval = setInterval(async function() {
                    ...
                    if (timeLeft >= 0) {
                        var timeLeftMinutes = parseInt(timeLeft / 60);
                        var timeLeftSeconds = timeLeft - (timeLeftMinutes * 60);
                        self.$el.text(self._formatTime(timeLeftMinutes) + ':' + self._formatTime(timeLeftSeconds));
                    } else {
