...
class HrTrainingCancellation(models.Model):
    _name = 'hr.training.cancellation'
    _description = 'Training Cancellation for Employee'
    ...
    def create(self, vals):
        vals['name'] = self.env['ir.sequence'].next_by_code('hr.training.cancellation') or 'New'
        return super(HrTrainingCancellation, self).create(vals)
    ...
    training_request_id = fields.Many2one('training.request', string='Training Request', required=True,
    ...
    training_cancel_approver_user_ids = fields.One2many('training.cancel.approver.user', 'emp_training_cancel_id',
                                                        string='Approver')
    approvers_ids = fields.Many2many('res.users', 'emp_training_cancel_approvers_rel', string='Approvers List')
    ...
    def search_read(self, domain=None, fields=None, offset=0, limit=None, order=None):
        ...
        result = super(HrTrainingCancellation, self).search_read(domain=domain, fields=fields, offset=offset, limit=limit, order=order)
        return result
    ...
    def read_group(self, domain, fields, groupby, offset=0, limit=None, orderby=False, lazy=True):
        ...
        return super(HrTrainingCancellation, self).read_group(domain, fields, groupby, offset=offset, limit=limit, orderby=orderby, lazy=lazy)
    ...
    def fields_view_get(self, view_id=None, view_type=None,
        ...
        res = super(HrTrainingCancellation, self).fields_view_get(
            view_id=view_id, view_type=view_type, toolbar=toolbar, submenu=submenu)
        print("contextt")
        print(self.env.context.get('is_approve_manager'))
    ...
    def custom_menu_manager(self):
        ...
        search_view_id = self.env.ref("equip3_hr_training.view_hr_training_cancellation_filter")
        if self.env.user.has_group(
                'equip3_hr_employee_access_right_setting.group_hr_training_supervisor') and not self.env.user.has_group(
                'equip3_hr_employee_access_right_setting.group_hr_training_manager'):
            employee_ids = []
            ...
            return {
                ...
                'name': 'My Training Cancellation',
                'res_model': 'hr.training.cancellation',
                'target': 'current',
                'view_mode': 'tree,form',
                ...
                'help': """<p class="o_view_nocontent_smiling_face">
                    Create a new Training Calcellation
                </p>""",
    ...
    def custom_menu(self):
        ...
        if self.env.user.has_group(
                'equip3_hr_employee_access_right_setting.group_hr_training_self_service') and not self.env.user.has_group(
                'equip3_hr_employee_access_right_setting.group_hr_reward_warning_hr_manager'):
            return {
                'type': 'ir.actions.act_window',
                'name': 'My Training Cancellation',
                'res_model': 'hr.training.cancellation',
                ...
                'help': """<p class="o_view_nocontent_smiling_face">
                  Create a new Training Cancellation
                </p>"""
                # 'context':{'search_default_current':1, 'search_default_group_by_state': 1},
                # 'search_view_id':search_view_id.id,
    ...
    @api.onchange('training_request_id')
    def onchange_approver_user(self):
        for training_cancel in self:
            training_setting = self.env['ir.config_parameter'].sudo().get_param('equip3_hr_training.training_approval_matrix')
            if training_setting:
                if training_cancel.training_cancel_approver_user_ids:
                    remove = []
                    for line in training_cancel.training_cancel_approver_user_ids:
                        remove.append((2, line.id))
                    training_cancel.training_cancel_approver_user_ids = remove
                ...
                if setting == 'employee_hierarchy':
                    training_cancel.training_cancel_approver_user_ids = self.training_cancel_emp_by_hierarchy(
                        training_cancel)
                    self.app_list_training_cancel_emp_by_hierarchy()
                if setting == 'approval_matrix':
                    self.training_cancel_approval_by_matrix(training_cancel)

    def training_cancel_emp_by_hierarchy(self, training_cancel):
        approval_ids = []
        seq = 1
        data = 0
        line = self.get_manager(training_cancel, training_cancel.employee_id, data, approval_ids, seq)
        return line

    def get_manager(self, training_cancel, employee_manager, data, approval_ids, seq):
        ...
        while data < int(setting_level):
            ...
            if employee_manager['parent_id']['user_id']['id']:
                self.get_manager(training_cancel, employee_manager['parent_id'], data, approval_ids, seq)
                break
        return approval_ids

    def get_manager_hierarchy(self, training_cancel, employee_manager, data, manager_ids, seq, level):
        if not employee_manager['parent_id']['user_id']:
            return manager_ids
        while data < int(level):
            ...
            seq += 1
            if employee_manager['parent_id']['user_id']['id']:
                self.get_manager_hierarchy(training_cancel, employee_manager['parent_id'], data, manager_ids, seq, level)
                break

        return manager_ids

    def app_list_training_cancel_emp_by_hierarchy(self):
        for training_cancel in self:
            app_list = []
            for line in training_cancel.training_cancel_approver_user_ids:
                app_list.append(line.user_ids.id)
            training_cancel.approvers_ids = app_list

    def training_cancel_approval_by_matrix(self, training_cancel):
        app_list = []
        approval_matrix = self.env['hr.training.approval.matrix'].search(
            [('apply_to', '=', 'by_employee'), ('applicable_to', '=', 'training_request')])
        matrix = approval_matrix.filtered(lambda line: training_cancel.employee_id.id in line.employee_ids.ids)
        if matrix:
            data_approvers = []
            for line in matrix[0].approval_matrix_ids:
                if line.approver_types == "specific_approver":
                    ...
                    seq = 1
                    data = 0
                    approvers = self.get_manager_hierarchy(training_cancel, training_cancel.employee_id, data, manager_ids, seq,
                                                           line.minimum_approver)
                    for approver in approvers:
                        data_approvers.append((0, 0, {'user_ids': [(4, approver)]}))
                        app_list.append(approver)
            training_cancel.approvers_ids = app_list
            training_cancel.training_cancel_approver_user_ids = data_approvers

        if not matrix:
            data_approvers = []
            approval_matrix = self.env['hr.training.approval.matrix'].search(
                [('apply_to', '=', 'by_job_position'), ('applicable_to', '=', 'training_request')])
            matrix = approval_matrix.filtered(lambda line: training_cancel.job_id.id in line.job_ids.ids)
            if matrix:
                for line in matrix[0].approval_matrix_ids:
                    if line.approver_types == "specific_approver":
                        data_approvers.append((0, 0, {'minimum_approver': line.minimum_approver,
                        ...
                        seq = 1
                        data = 0
                        approvers = self.get_manager_hierarchy(training_cancel, training_cancel.employee_id, data,
                                                               manager_ids, seq,
                                                               line.minimum_approver)
                        for approver in approvers:
                            data_approvers.append((0, 0, {'user_ids': [(4, approver)]}))
            ...
            if not matrix:
                data_approvers = []
                approval_matrix = self.env['hr.training.approval.matrix'].search(
                    [('apply_to', '=', 'by_department'), ('applicable_to', '=', 'training_request')])
                matrix = approval_matrix.filtered(
                    lambda line: training_cancel.department_id.id in line.department_ids.ids)
                if matrix:
                    for line in matrix[0].approval_matrix_ids:
                        if line.approver_types == "specific_approver":
                            data_approvers.append((0, 0, {'minimum_approver': line.minimum_approver,
    ...
    def _compute_can_approve(self):
        for training_cancel in self:
            if training_cancel.approvers_ids:
                setting = self.env['ir.config_parameter'].sudo().get_param(
                    'equip3_hr_training.training_type_approval')
                setting_level = self.env['ir.config_parameter'].sudo().get_param(
                    'equip3_hr_training.training_level')
                app_level = int(setting_level)
                current_user = training_cancel.env.user
                if setting == 'employee_hierarchy':
                    matrix_line = sorted(
                        training_cancel.training_cancel_approver_user_ids.filtered(lambda r: r.is_approve == True))
                    app = len(matrix_line)
                    a = len(training_cancel.training_cancel_approver_user_ids)
                    if app < app_level and app < a:
                        if current_user in training_cancel.training_cancel_approver_user_ids[app].user_ids:
                            training_cancel.is_approver = True
                        else:
                            training_cancel.is_approver = False
                    else:
                        training_cancel.is_approver = False
                    ...
                    a = len(training_cancel.training_cancel_approver_user_ids)
                    if app < a:
                        for line in training_cancel.training_cancel_approver_user_ids[app]:
    ...
    def approver_wa_template(self):
        ...
        if send_by_wa:
            template = self.env.ref('equip3_hr_training.training_cancel_approver_wa_template')
            wa_sender = waParam()
            if template:
                url = self.get_url(self)
                if self.training_cancel_approver_user_ids:
                    matrix_line = sorted(
                        self.training_cancel_approver_user_ids.filtered(lambda r: r.is_approve == True))
                    approver = self.training_cancel_approver_user_ids[len(matrix_line)]
                    for user in approver.user_ids:
                        ...
                        if "${course_name}" in string_test:
                            string_test = string_test.replace("${course_name}", self.training_request_id.course_id.name)
    ...
    def approved_wa_template(self):
        send_by_wa = self.env['ir.config_parameter'].sudo().get_param('equip3_hr_training.send_by_wa_training')
        if send_by_wa:
            template = self.env.ref('equip3_hr_training.training_cancel_approved_wa_template')
            url = self.get_url(self)
    ...
    def rejected_wa_template(self):
        send_by_wa = self.env['ir.config_parameter'].sudo().get_param('equip3_hr_training.send_by_wa_training')
        if send_by_wa:
            template = self.env.ref('equip3_hr_training.training_cancel_rejected_wa_template')
            url = self.get_url(self)
            wa_sender = waParam()
            if template:
                if self.training_cancel_approver_user_ids:
    ...
    def action_confirm(self):
        ...
        if setting:
            ...
            for line in self.training_cancel_approver_user_ids:
                line.write({'approver_state': 'draft'})
        else:
            self.action_del_approve()

    def wizard_approve(self):
        return {
            'type': 'ir.actions.act_window',
            'res_model': 'training.cancel.request.wizard',
            'view_type': 'form',
            'view_mode': 'form',
    ...
    def action_approve(self):
        sequence_matrix = [data.name for data in self.training_cancel_approver_user_ids]
        sequence_approval = [data.name for data in self.training_cancel_approver_user_ids.filtered(
            lambda line: len(line.approved_employee_ids) != line.minimum_approver)]
        max_seq = max(sequence_matrix)
        min_seq = min(sequence_approval)
        approval = self.training_cancel_approver_user_ids.filtered(
            ...
            if setting == 'employee_hierarchy':
                if self.env.user not in record.approved_user_ids:
                    if record.is_approver:
                        for user in record.training_cancel_approver_user_ids:
                        ...
                        matrix_line = sorted(
                            record.training_cancel_approver_user_ids.filtered(lambda r: r.is_approve == False))
                        if len(matrix_line) == 0:
                            # record.write({'state': 'approved'})
                            record.action_del_approve()
                            self.approved_mail()
                ...
                if self.env.user not in record.approved_user_ids:
                    if record.is_approver:
                        for line in record.training_cancel_approver_user_ids:
    ...
    def action_del_approve(self):
        for rec in self:
            if rec.training_request_id:
                training_req = rec.training_request_id
                training_req.update({'state': 'cancelled'})
                training_histories = self.env['training.histories'].search(
                    [('training_req_id', '=', rec.training_request_id.id)], limit=1)
                if training_histories:
                    training_histories.unlink()
    ...
    def unlink(self):
        for rec in self:
            ...
            return super(HrTrainingCancellation, rec).unlink()
    ...
    def get_url(self, obj):
        ...
        menu_id = self.env['ir.model.data'].get_object_reference(
            'equip3_hr_training', 'sub_menu_training_cancelation_manager')[1]
        action_id = self.env['ir.model.data'].get_object_reference(
            'equip3_hr_training', 'action_training_cancellation_to_approve_custom')[1]
        url = base_url + "/web?db=" + str(self._cr.dbname) + "#id=" + str(
            obj.id) + "&view_type=form&model=hr.training.cancellation&menu_id=" + str(
    ...
    def approver_mail(self):
        ...
        for rec in self:
            if rec.training_cancel_approver_user_ids:
                matrix_line = sorted(rec.training_cancel_approver_user_ids.filtered(lambda r: r.is_approve == True))
                approver = rec.training_cancel_approver_user_ids[len(matrix_line)]
                for user in approver.user_ids:
                    try:
                        template_id = ir_model_data.get_object_reference(
                            'equip3_hr_training',
                            'email_template_training_cancellation_approval')[1]
                    except ValueError:
                        template_id = False
        ...
        for rec in self:
            if rec.training_cancel_approver_user_ids:
                try:
                    template_id = ir_model_data.get_object_reference(
                        'equip3_hr_training',
                        'email_template_training_cancellation_approved')[1]
                except ValueError:
                    template_id = False
                ctx = self._context.copy()
                url = self.get_url(self)
        ...
        for rec in self:
            if rec.training_cancel_approver_user_ids:
                try:
                    template_id = ir_model_data.get_object_reference(
                        'equip3_hr_training',
                        'email_template_training_cancellation_rejection')[1]
                except ValueError:
                    template_id = False
                ctx = self._context.copy()
                url = self.get_url(self)
    ...
    def get_auto_follow_up_approver(self):
        ...
        training_cancel_approve = self.search([('state', '=', 'to_approve')])
        for rec in training_cancel_approve:
            if rec.training_cancel_approver_user_ids:
                matrix_line = sorted(rec.training_cancel_approver_user_ids.filtered(lambda r: r.is_approve == True))
                approver = rec.training_cancel_approver_user_ids[len(matrix_line)]
                for user in approver.user_ids:
                    ...
                    if not approver.is_auto_follow_approver:
                        ...
                        query_statement = """UPDATE training_cancel_approver_user set is_auto_follow_approver = TRUE, repetition_follow_count = %s WHERE id = %s """
                        ...
                        if user not in approver.approved_employee_ids and approver.repetition_follow_count > 0:
                            ...
                            query_statement = """UPDATE training_cancel_approver_user set repetition_follow_count = %s WHERE id = %s """
                            self.sudo().env.cr.execute(query_statement, [count, approver.id])
                            self.env['mail.template'].browse(template_id).with_context(ctx).send_mail(rec.id,
                                                                                                      force_send=True)
...
class TrainingCancelApproverUser(models.Model):
    _name = 'training.cancel.approver.user'

    emp_training_cancel_id = fields.Many2one('hr.training.cancellation', string="Employee Training Cancel Id")
    name = fields.Integer('Sequence', compute="fetch_sl_no")
    user_ids = fields.Many2many('res.users', string="Approvers")
    approved_employee_ids = fields.Many2many('res.users', 'emp_training_cancel_user_ids', string="Approved user")
    ...
    matrix_user_ids = fields.Many2many('res.users', 'training_cancel_max_user_ids', string="Matrix user")
    is_delegation_mail_sent = fields.Boolean(string="Is Delegation Email Sent", default=False)
    #parent status
    state = fields.Selection(string='Parent Status', related='emp_training_cancel_id.state')

    @api.depends('emp_training_cancel_id')
    def fetch_sl_no(self):
        sl = 0
        for line in self.emp_training_cancel_id.training_cancel_approver_user_ids:
            sl = sl + 1
            line.name = sl
    ...
    def update_minimum_app(self):
        for rec in self:
            if len (rec.user_ids) < rec.minimum_approver and rec.emp_training_cancel_id.state == 'draft':
                rec.minimum_approver = len(rec.user_ids)
            if not rec.matrix_user_ids and rec.emp_training_cancel_id.state == 'draft':
