...
class SaleSummaryReportWizard(models.AbstractModel):
	_name = "report.equip3_pos_report.report_sales_summary"
	...
	def _get_report_values(self, docids, data=None):
		...
		categories_tot = []
		domain_config = []
		branch_data = sale_summary_rec.pos_branch_id or False
		company_data = sale_summary_rec.company_id or False
		name_branch = 'All'
		if branch_data:
		...
		ph_vat_breakdown = False
		for pos_order in pos_orders:
			final_total += pos_order.amount_total
			final_tax += pos_order.amount_tax

			if pos_order.state in ['paid','invoiced','done']:
				if pos_order.payment_ids:
					payments|=pos_order.payment_ids
				order_paid_ids.append(pos_order.id)

			for line in pos_order.lines:
				ph_vat_breakdown = self.count_ph_vat_breakdown(line,ph_vat_breakdown)
				disc = 0
				if line.discount > 0 :
					disc =(line.price_unit * line.qty * line.discount)/100
				final_discount += disc
				total = total + line.price_subtotal_incl
		...
		summery_data.append({
			'name':name_branch,
			'gross_total':float(total+discount),
			'sales_wo_tax':float(total-tax),
			'total_tax': float(tax),
		...
		st_line_ids = payments.ids or False
		if st_line_ids:
			self.env.cr.execute("""
		...
		if st_line_ids:
			self.env.cr.execute("""
				...
				GROUP BY ppm.name
			""", (tuple(st_line_ids),))
			payments_arr = self.env.cr.dictfetchall()
		else:
			payments_arr = []

		if order_paid_ids:
			self.env.cr.execute("""
				SELECT (pc.name) c_name, sum(qty) c_total
				...
				GROUP BY pc.name
				""", (tuple(order_paid_ids),))
			categ = self.env.cr.dictfetchall()
		else:
			categ = []
		main_data_dict.update({name_branch:{'journal':journal,'categ':categ}})
		...
		return {
			...
			'branch_identifier' : summery_data,
			...
			'ph_vat_breakdown':ph_vat_breakdown,
		}
