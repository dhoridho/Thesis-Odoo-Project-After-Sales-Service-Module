...
class ir_model_access(models.Model):
    ...
    def check(self, model, mode='read', raise_exception=True):
        # check_modul_instal = self._cr.execute("""select state from ir_module_module where name = 'simplify_access_management'""")
        # check_modul_instal = self._cr.fetchone()
        # check_modul_instal = check_modul_instal and check_modul_instal[0] or False
        # check_modul_instal = self.env['ir.module.module'].sudo().search([('name', '=', 'simplify_access_management')], limit=1)
            
        try:
            check_modul_instal = self._cr.execute("""select state from ir_module_module where name = 'simplify_access_management'""")
            check_modul_instal = self._cr.fetchone()
            check_modul_instal = check_modul_instal and check_modul_instal[0] or False
            check_modul_instal = check_modul_instal.state
        except:
            check_modul_instal = 'installed'
        if check_modul_instal != 'installed':
            return super(ir_model_access, self).check(model, mode, raise_exception)
        ...
        try:
            ...
            value = False
        # value = self.env['ir.config_parameter'].sudo().get_param('uninstall_simplify_access_management')
        if not value:
            if model:
                self._cr.execute("SELECT id FROM ir_model WHERE model='" + model + "'")
                model_numeric_id = self._cr.fetchone() or False
                if model_numeric_id:
...
{resolution_info}""".format(
        ...
        try:
            ...
            if self.env.user.id and read_value:
                a = "select access_management_id from access_management_comapnay_rel where company_id in " + str(tuple([0]+self.env.company.ids))
                self._cr.execute(a)
                a = self._cr.fetchall()
                if a:    
