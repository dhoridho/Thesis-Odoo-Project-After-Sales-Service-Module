...
class SaleOrder(models.Model):
    ...
    def _add_rec_mp_field_mapping(cls, mp_field_mappings=None):
        ...
        mp_field_mapping = {
            ...
            # MP Buyer Info
            'mp_buyer_id': ('buyer_info/buyer_id', lambda env, r: str(r) if r else None),
        ...
        def _handle_preorder(env, data):
            if data:
                preorder = data.get('preorder_type', False)
                if preorder != 0:
                    return True
                else:
                    return False
        ...
        def _handle_get_awb(env, data):
            if data:
                if data.get('awb', False):
                    return data.get('awb', None)
                else:
                    return data.get('autoresi_awb', None)
            return None

        mp_field_mapping.update({
            # MP Order Transaction & Payment
            'mp_awb_number': ('order_info/shipping_info', _handle_get_awb),
            ...
            'mp_delivered_deadline': ('shipment_fulfillment/confirm_shipping_deadline ', _handle_isoformat_to_dt_str),
            'mp_pickup_done_time': ('shipping_date', _handle_isoformat_to_dt_str),
            ...
            'mp_cancel_reason': ('cancel_request_info', _handle_cancel_req_reason),

            # is preorder
            'is_preorder': ('preorder', _handle_preorder),
    ...
    def _prepare_invoice(self):
        ...
        for order in self:
            if order.mp_account_id:
                if order.mp_account_id.marketplace == 'tokopedia':
                    res['payment_reference'] = 'Transaksi Penjualan Berhasil - %s' % (order.mp_invoice_number)
        return res
    ...
    def tokopedia_print_label(self):
        ...
        def img_margin(pil_img, top, right, bottom, left, color):
        ...
        for order in self:
            ...
            time.sleep(0.5)
            response = tp_order.action_get_shipping_label(order.mp_external_id)
            if response.status_code == 200:
                if '<!doctype html>' in response.text:
                    soup = BeautifulSoup(response.text, 'html.parser')
                    input_elm = soup.find('input', {'id': 'booking-code-input-1'})
                    if input_elm:
                        order.mp_awb_number = input_elm.get('value')
                    new_html = response.text.replace('class="top_bar"', 'class="top_bar" style="display:none;"')

                    barcode_elm = soup.find('img', {'id': 'barcode-printout-1'})
                    if barcode_elm:
                        shipper_id = int(soup.find('input', {'id': 'shipper-id-1'}).get('value'))
                        bc_data = {
                            'data': barcode_elm.get('alt'),
                            'show_text': False
                        }
                        bc, bc_buff, bc_img = None, io.BytesIO(), ''
                        if shipper_id in [1, 12, 22]:
                            bc = Image.open(io.BytesIO(pyzint.Barcode.CODE39(**bc_data).render_bmp()))
                            bc = img_margin(bc, 10, 10, 10, 10, (1,))
                            bc.save(bc_buff, format="JPEG")
                        elif shipper_id in [14, 11, 23, 24, 25, 26, 28]:
                            bc = Image.open(io.BytesIO(pyzint.Barcode.CODE128B(**bc_data).render_bmp()))
                            bc = img_margin(bc, 10, 10, 10, 10, (1,))
                            bc.save(bc_buff, format="JPEG")
                        bc_img = b64encode(bc_buff.getvalue()).decode()
                        barcode_elm_to_replace = '<img id="barcode-printout-1"'
                        barcode_elm_replacement = '<img id="barcode-printout-1" ' \
                            'src="data:image/jpeg;base64,%s" '
                        if bc_img:
                            new_html = new_html.replace(barcode_elm_to_replace,
                                                        barcode_elm_replacement % bc_img)
                    # if tp_order.without_product:
                    #     new_html = new_html.replace('<table class="products"',
                    #                                 '<table class="products" style="display: none;"')
                    order.mp_awb_html = new_html
                    order_list.append(str(order.id))

        if self.mp_account_id.default_awb_action == 'download':
            download_path = '/web/binary/tokopedia/download_pdf?order_ids=%s&mp_awb_number=%s' % ('_'.join(order_list), order.mp_awb_number)
            return {
                'name': 'Tokopedia AWB Label',
                'type': 'ir.actions.act_url',
                'url': str(base_url) + str(download_path),
                'target': 'new',
            }
        elif self.mp_account_id.default_awb_action == 'open':
            return {
                'name': 'Label',
                'res_model': 'ir.actions.act_url',
                'type': 'ir.actions.act_url',
                'target': 'new',
                'url': base_url + '/web/binary/tokopedia/open_pdf?order_ids=%s&mp_awb_number=%s' % ('_'.join(order_list), order.mp_awb_number),
            }
    ...
    @mp.tokopedia.capture_error
    def tokopedia_download_shipping_label(self):
        self.mp_account_id.write({'default_awb_action': 'download'})
        # self.mp_account_id.default_awb_action = 'open'
        return self.tokopedia_print_label()