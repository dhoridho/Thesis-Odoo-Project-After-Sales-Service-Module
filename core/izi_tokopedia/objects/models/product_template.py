...
# from odoo.addons.izi_tokopedia.objects.utils.tokopedia.images import TokopediaImages
...
class ProductTemplate(models.Model):
    ...
    tp_account_ids = fields.Many2many('mp.account', 'tp_shop_id', string="Tokopedia Accounts", domain="[('marketplace', '=', 'tokopedia')]")
    tp_categ_id = fields.Many2one('mp.tokopedia.category', string="Category", required=False)
    tp_brand_id = fields.Many2one('mp.tokopedia.brand', string="Brand", required=False)
    tp_attributes_ids = fields.One2many('mp.tokopedia.attribute.line', 'product_tmpl_id', string="Attribute", copy=True)
    tp_condition = fields.Selection([('NEW', 'New'), ('USED', 'Used')], string="Condition", default='NEW')
    tp_logistic_ids = fields.One2many(comodel_name="mp.tokopedia.shop.logistic", inverse_name="shop_id",
                                        string="Logistics",
                                        required=False)
    tp_sales_price = fields.Float('Sale Price', digits='Product Price', default=100.0)
    is_free_return = fields.Boolean('Is Free Return', default=False)
    is_must_insurance = fields.Boolean('Is Must Insurance', default=False)
    tp_stock_status = fields.Selection([
        ('LIMITED', 'LIMITED'),
        ('EMPTY', 'EMPTY'),
        ('UNLIMITED', 'UNLIMITED'),
    ], string='Stock Status', default='LIMITED')
    min_order = fields.Integer('Minimal Order', default=1)

    def _compute_tokopedia_accounts(self):
        for tokped in self:
            tokped.tp_account_ids = self.env['mp.account'].search([('marketplace', '=', 'tokopedia'), ('active', '=', True)])

    @api.onchange('tp_categ_id')
    def _onchange_set_attribute_tokopedia(self):
        product_id = self.id
        if self.tp_categ_id:
            attribs_line_obj = self.env['mp.tokopedia.attribute.line'].search([('category_id', '=', self.tp_categ_id.id), ('product_tmpl_id', '=', product_id)])
            if not attribs_line_obj:
                attributes_line = [(5, 0)]
                attribs_obj = self.env['mp.tokopedia.attribute'].search([('category_id', '=', self.tp_categ_id.id)])
                if attribs_obj:
                    attributes_line += [(0, 0, {
                        'attribute_id': recs.id,
                        'attribute_value_id': False,
                        'category_id': self.tp_categ_id.id,
                        'product_tmpl_id': product_id
                    }) for recs in attribs_obj]
                    self.tp_attributes_ids = attributes_line
                else:
                    self.tp_attributes_ids = False
            else:
                self.tp_attributes_ids = [(6, 0, attribs_line_obj.ids)]
        else:
            self.tp_attributes_ids = False

    @mp.tokopedia.capture_error
    def tokopedia_add_product(self, **kw):
        _notify = self.env['mp.base']._notify
        self.ensure_one()
        tp_shop_obj = self.env['mp.tokopedia.shop']
        tp_logistic_obj = self.env['mp.tokopedia.shop.logistic']
        ...
        tpd_item_ids = []
        if not self.tp_account_ids:
            raise UserError('Tokopedia account not found. Please set your account first.')
        for account in self.tp_account_ids:
            params = {}
            tp_account = account.tokopedia_get_account()
            tp_product = TokopediaProduct(tp_account, api_version="v3")
            # _notify('info', 'Upload product from {} is started... Please wait!'.format('TOKOPEDIA'),
            #         notif_sticky=False)

            tp_shop_id = tp_shop_obj.search([('mp_account_id', '=', account.id)])
            mp_product_ids = []
            mp_product_data = []
            quantity_on_hand = 0
            ...
            product_data = {
                'name': self.name,
                'condition': self.tp_condition,
                'Description': "" if not self.description else self.description,
                'sku': '' if not self.default_code else self.default_code.strip(),
                'price_currency': "IDR",
                'price': int(self.tp_sales_price) if self.tp_sales_price > 100 else 100,
                'status': self.tp_stock_status,
                'min_order': self.min_order,
                'category_id': self.tp_categ_id.category_id,
                'dimension': {
            ...
            if self.tp_logistic_ids:
                prologist = []
                for logist in self.tp_logistic_ids:
            ...
            if self.mp_attribute_value_ids:
                product_data.update({
                    'variant': self.tokopedia_add_item_variation()
                })

            if self.tp_attributes_ids:
                attribute_ids = []
                for attrib in self.tp_attributes_ids:
            ...
            if self.mp_product_image_ids:
                ...
                # url_image = "https://ecs7.tokopedia.net"
                ...
                # work_dir = '/tmp/product-%s/%s/%s/' % (self.id, date_today, tp_shop_id.shop_id)
                # work_dir = '/img/cache/700/product-1/%s/%s/' % (date_today, tp_shop_id.shop_id)
                base_path = '/var/www/html'
                image_path = 'marketplace/img/tokopedia/product-%s/' % (self.id)
                    ...
                    if x < 5:
                        ...
                        try:
                            ...
                            if mime_type == 'image/png':
                                ...
                                # temp_filename = r'%s' % (filename)
                                # tokped_filename = generate_filename
                                # new_filename = r'%s' % (tokped_filename)
                                # os.rename(temp_filename, new_filename)
                            os.chmod(tokped_filename, 0o777)
                            # tokped_image = url_image + tokped_filename
                            # tokped_image = "http://8.215.31.36/" + image_path + filename_without_path
                            tokped_image = base_image_url + image_path + filename_without_path
                            image_id_list.append({
                                'file_path': tokped_image
                            })
            ...
            if self.tp_item_id and self.tp_item_id != 0:
                product_data.update({
                    'id': int(self.tp_item_id),
                    'stock': int(quantity_on_hand) if int(quantity_on_hand) > 0 else 0,
                })
                api = TokopediaAPI(tp_account)
                ###self.tp_shop_id.mp_external_id
                params = ({
                    'shop_id': tp_shop_id.shop_id
                })
                ...
                time.sleep(5)
                account.tokopedia_get_products(**{'product_ids': int(self.tp_item_id)})
                # self.env['mp.base']._logger('tokopedia', 'Product %s updated' %
                ...
                var_response = tp_product.create_new_product(shop_id=tp_shop_id.shop_id,
                                                           products=mp_product_data)
                if var_response.status_code == 200:
                    tp_product_raw = json.loads(var_response.text, strict=False)['data']
                    # _logger.info('Response from Tokopedia: %s' % (tp_product_raw))
                    if tp_product_raw:
                        if 'error' in tp_product_raw:
                            raise ValidationError(tp_product_raw.get("error"))
                        if 'success_rows_data' in tp_product_raw:
                            product_ids = tp_product_raw.get("success_rows_data")
                            for product in product_ids:
                                self.tp_item_id = product.get('product_id')
                                tpd_item_ids.append({
                                    'name': product.get('product_id'),
                                    'product_tmpl_id': self.id,
                                    'tp_account_id': account.id
                                })
                            # mp_product_ids.append(int(product_ids.product_id))
                            account.tokopedia_get_products(**{'product_ids': int(self.tp_item_id)})
                            shutil.rmtree(work_dir, ignore_errors=True)
                            # self.env['mp.base']._logger('tokopedia', 'Product %s has been posted' %
                            #                             (kw['data'].name), notify=True, notif_sticky=False)
            # except Exception as e:
            #     self.env['mp.base']._logger('tokopedia', e, notify=True, notif_sticky=False)
        # self.tp_item_ids = [(6, 0, tpd_item_ids)]
    ...
    def tokopedia_add_item_variation(self):
                ...
                if product.combination_indices:
                    ...
                    if lensplit > 1:
                        ...
                        if i == 0:
                            model_list.append({
                                'is_primary': True,
                                'status': product.product_tmpl_id.tp_stock_status,
