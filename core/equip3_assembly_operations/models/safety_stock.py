...
class AssemblySafetyStock(models.Model):
	_name = 'assembly.safety.stock'
	_description = 'Assembly Safety Stock Management'
	...
	stock_line_ids = fields.One2many('assembly.safety.stock.line', 'stock_id', string='Products')
	...
	def create(self, values):
		...
		return super(AssemblySafetyStock, self).create(values)

	def write(self, values):
		if values.get('warehouse_id'):
			self._check_warehouse(values.get('warehouse_id'))
		return super(AssemblySafetyStock, self).write(values)


class AssemblySafetyStockLine(models.Model):
	_name = 'assembly.safety.stock.line'
	_description = 'Assembly Safety Stock Management Line'

	stock_id = fields.Many2one('assembly.safety.stock', string='Safety Stock', copy=False, required=True, ondelete='cascade')
	...
	def create(self, values):
		...
		return super(AssemblySafetyStockLine, self).create(values)

	def write(self, values):
		if values.get('product_id'):
			self._check_product(self.stock_id.id, values['product_id'])
		return super(AssemblySafetyStockLine, self).write(values)
