...
class AssemblyProductionRecord(models.Model):
	_name = 'assembly.production.record'
	_inherit = ['mail.thread', 'mail.activity.mixin']
	_description = 'Assembly Production Record'
	...
	def _compute_show_submit_button(self):
		for production in self:
			...
			if production.state != 'draft' or production.record_type == 'disassembly':
				continue
			production.show_submit_button = True

	@api.depends(
		'disassembly_record_ids', 'disassembly_record_ids.state', 'disassembly_record_ids.product_qty', 
		'disassembly_record_ids.rejected_qty', 'record_type', 'product_qty', 'rejected_qty')
	def _compute_disassembly_remaining_qty(self):
		for record in self:
			record.disassembly_remaining_qty = 0.0
			if record.record_type == 'assembly':
				product_qty = record.product_qty + record.rejected_qty
				disassembly_record_ids = record.disassembly_record_ids.filtered(lambda d: d.state == 'confirm')
				disassembly_product_qty = sum(disassembly_record_ids.mapped('product_qty'))
				disassembly_rejected_qty = sum(disassembly_record_ids.mapped('rejected_qty'))
				record.disassembly_remaining_qty = product_qty - (disassembly_product_qty + disassembly_rejected_qty)
	...
	def _compute_is_autogenerate(self):
		for record in self:
			...
			record.is_autogenerate = product_id and product_id._assembly_is_auto_generate() or False

	@api.depends('product_tracking', 'product_qty', 'rejected_qty', 'finished_lot_ids', 
	'finished_lot_ids.assembly_qty', 'rejected_lot_ids', 'rejected_lot_ids.assembly_qty')
	def _compute_default_lot_qty(self):
		for record in self:
			...
			if record.product_tracking == 'serial':
				...
				product_qty = record.product_qty - sum(record.finished_lot_ids.mapped('assembly_qty'))
				rejected_qty = record.rejected_qty - sum(record.rejected_lot_ids.mapped('assembly_qty'))
			record.default_finished_lot_qty = product_qty
			record.default_rejected_lot_qty = rejected_qty

	@api.depends('move_byproduct_ids', 'move_byproduct_ids.product_id', 'move_byproduct_ids.product_uom_qty', 'byproduct_lot_ids', 'byproduct_lot_ids.assembly_qty')
	def _compute_byproduct_products(self):
		for record in self:
			...
			manual_product_ids = product_ids.filtered(lambda p: p._assembly_is_manual_generate())
			auto_product_ids = product_ids.filtered(lambda p: p._assembly_is_auto_generate())
			...
			manual_no_repeat_product_ids = manual_product_ids.filtered(lambda p:
				sum(record.move_byproduct_ids.filtered(lambda b: b.product_id == p).mapped('product_uom_qty')) > \
					sum(record.byproduct_lot_ids.filtered(lambda b: b.product_id == p).mapped('assembly_qty')))
	...
	assembly_bom_id = fields.Many2one(
		'mrp.bom', 'Assembly Bill of Materials',
		readonly=True, states={'draft': [('readonly', False)]},
		domain="""[
			('equip_bom_type', '=', 'assembly'),
			('type', '=', 'normal'),
	...
	move_raw_ids = fields.One2many(
		'stock.move', 'assembly_component_id', 'Components',
		copy=True, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]},
		domain=[('scrapped', '=', False)])
	move_finished_ids = fields.One2many(
		'stock.move', 'assembly_finished_id', 'Finished Products',
		copy=True, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]},
		domain=[('scrapped', '=', False)])
	...
	location_src_id = fields.Many2one(
		...
		readonly=True, required=True,
		domain="[('usage','=','internal'), ('id', 'child_of', warehouse_view_location_id), '|', ('company_id', '=', False), ('company_id', '=', company_id)]",
	...
	picking_type_id = fields.Many2one(
		...
		domain="[('code', '=', 'mrp_operation'), ('company_id', '=', company_id), ('warehouse_id', '=', warehouse_id)]",
		required=True, check_company=True, readonly=True, states={'draft': [('readonly', False)]})
	...
	move_dest_ids = fields.One2many('stock.move', 'created_assembly_id',
	...
	account_move_ids = fields.One2many('account.move', 'assembly_id', string='Journal Entries', copy=False, readonly=True)

	purchase_request_ids = fields.One2many('purchase.request', 'assembly_id', string='Purchase Requests', readonly=True)
	internal_transfer_count = fields.Integer(compute=_compute_internal_transfer_count)

	record_type = fields.Selection(
		selection=[('assembly', 'Assembly'), ('disassembly', 'Disassembly')],
		string='Record Type',
		required=True,
		default='assembly'
	)

	assembly_record_id = fields.Many2one('assembly.production.record', string='Assembly Production Record', domain="[('record_type', '=', 'assembly'), ('state', '=', 'confirm'), ('disassembly_remaining_qty', '>', 0.0)]", copy=False)
	disassembly_record_ids = fields.One2many('assembly.production.record', 'assembly_record_id', string='Disassamble Production Records', domain="[('record_type', '=', 'disassembly'), ('product_id', '=', 'product_id'), ('warehouse_id', '=', 'warehouse_id')]")
	disassembly_remaining_qty = fields.Float(string='Disassembly Remaining Quantity', compute=_compute_disassembly_remaining_qty, store=True)
	...
	finished_lot_ids = fields.One2many(comodel_name='stock.production.lot', inverse_name='assembly_production_finished_id', string='Finished Lot/Serial Number')
	byproduct_lot_ids = fields.One2many(comodel_name='stock.production.lot', inverse_name='assembly_production_byproduct_id', string='ByProduct Lot/Serial Number')
	rejected_lot_ids = fields.One2many(comodel_name='stock.production.lot', inverse_name='assembly_production_rejected_id', string='Rejected Lot/Serial Number')
	...
	def _get_moves_finished_values(self):
		moves = []
		for production in self:
			if production.record_type == 'disassembly':
			...
			move_for = 'finished' if production.record_type != 'disassembly' else 'raw'
	...
	def _get_moves_raw_values(self):
		...
		for production in self:
			...
			move_for = 'raw' if production.record_type != 'disassembly' else 'finished'
	...
	def _get_move_values(self, move_for, product_id, product_uom_qty, product_uom, bom_line_id=False, byproduct_id=False):
		company = self.company_id

		location_production_id = self.product_id.with_company(company).property_stock_production.id
		...
		if move_for == 'finished':
			...
			date_planned_finished = date_planned_finished + relativedelta(days=company.manufacturing_lead)
			...
			if self.record_type == 'disassembly':
		...
		values = {
			...
			'assembly_product_uom_qty': product_uom_qty,
			'assembly_quantity_done': product_uom_qty,
			'warehouse_id': self.warehouse_id.id,
		...
		if move_for == 'raw':
			lot_ids = []
			move_line_vals_list = [(5,)]

			if product_id.tracking == 'none':
				move_line_vals_list += [(0, 0, {
					'product_id': product_id.id,
				...
				if self.record_type == 'assembly' or not self.assembly_record_id:
					quants = self.env['stock.quant'].search([
						('location_id', '=', location_id),
						('product_id', '=', product_id.id),
						('company_id', '=', company.id),
					]).filtered(lambda o: o.available_quantity > 0.0)

					qty_to_take = product_uom_qty
					for lot in quants.mapped('lot_id'):
						if float_compare(qty_to_take, 0.0, precision_rounding=product_id.uom_id.rounding) <= 0:
							break
						lot_quants = quants.filtered(lambda o: o.lot_id == lot)
						lot_available_quantity = sum(lot_quants.mapped('available_quantity'))
						qty_taken = min(qty_to_take, lot_available_quantity)
						move_line_vals_list += [(0, 0, {
							'product_id': product_id.id,
							'product_uom_id': product_uom.id,
							'location_id': location_id,
							'location_dest_id': location_dest_id,
							'company_id': company.id,
							'qty_done': lot_available_quantity,
							'lot_id': lot.id,
						})]
						qty_to_take -= qty_taken
						lot_ids += [lot.id]
				else:
					assembly_move_lines = self.assembly_record_id.move_raw_ids.move_line_ids

					qty_to_take = product_uom_qty
					for lot in assembly_move_lines.mapped('lot_id'):
						if float_compare(qty_to_take, 0.0, precision_rounding=product_id.uom_id.rounding) <= 0:
							break
						lot_move_lines = assembly_move_lines.filtered(lambda o: o.lot_id == lot)
						lot_qty = sum(lot_move_lines.mapped('qty_done'))
						qty_taken = min(qty_to_take, lot_qty)
						move_line_vals_list += [(0, 0, {
							'product_id': product_id.id,
							'product_uom_id': product_uom.id,
							'location_id': location_id,
	...
	@api.onchange('record_type', 'bom_id', 'assembly_record_id')
	def _onchange_bom_id(self):
		record_type = self.record_type
		assembly_record = self.assembly_record_id

		if record_type == 'assembly' or (record_type == 'disassembly' and not assembly_record):
			bom = self.bom_id
			self.product_tmpl_id = bom.product_tmpl_id.id
			self.product_id = bom.product_id.id or self.product_tmpl_id.product_variant_id.id
			self.product_uom_id = self.env.context.get('default_product_uom_id', bom.product_uom_id.id)
			self.warehouse_id = self.env.context.get('default_warehouse_id', bom.picking_type_id.warehouse_id.id) or self.warehouse_id.id
		else:
			self.bom_id = assembly_record.bom_id.id
			self.product_tmpl_id = assembly_record.product_tmpl_id.id
			self.product_id = assembly_record.product_id.id
			self.product_uom_id = assembly_record.product_uom_id.id
			self.warehouse_id = assembly_record.warehouse_id.id
			self.assembly_bom_id = False

	@api.onchange('record_type', 'bom_id', 'assembly_record_id', 'product_id', 'picking_type_id')
	def _onchange_set_product_qty(self):
		if self.record_type == 'assembly':
			self.product_qty = self.env.context.get('default_finished_qty', self.bom_id.product_qty)
		else:
			if self.assembly_record_id:
				to_disassembly_qty = self.assembly_record_id.disassembly_remaining_qty
			else:
				to_disassembly_qty = 0.0
				if self.product_id and self.location_dest_id:
					to_disassembly_qty = self.env['stock.quant']._get_available_quantity(self.product_id, self.location_dest_id)
			self.product_qty = to_disassembly_qty

	@api.onchange('record_type', 'assembly_bom_id')
	def _onchange_set_bom(self):
		if self.record_type == 'disassembly' and self.assembly_bom_id:
			self.bom_id = self.assembly_bom_id.id
			self.assembly_record_id = False
	...
	@api.onchange('record_type', 'bom_id', 'product_id', 'product_qty', 'rejected_qty', 'product_uom_id', 'assembly_record_id', 'warehouse_id')
	def _onchange_move_raw(self):
		if self.record_type == 'disassembly' and self.assembly_record_id:

			product_qty = self.product_qty
			assembly_id = self.assembly_record_id
			assembly_product_qty = assembly_id.product_qty + assembly_id.rejected_qty

			move_raw_ids = [(5,)]
			for move in assembly_id.move_raw_ids:
				if move.bom_line_id:
					factor = move.bom_line_id.product_qty / assembly_id.bom_id.product_qty
				else:
					factor = move.product_uom_qty / assembly_product_qty
	...
	def _onchange_move_finished(self):
		if self.record_type == 'disassembly' and self.assembly_record_id:
			...
			move_finished_ids = [(5,)]
			for move in assembly_id.move_finished_ids.filtered(lambda m: m.product_id == self.product_id):
				values = self._get_move_values(
					move_for='finished',
					product_id=move.product_id,
					product_uom_qty=product_qty, 
	...
	def _onchange_warehouse_id(self):
		...
		self.location_src_id = picking_type.default_location_src_id.id
		self.location_dest_id = picking_type.default_location_dest_id.id
	...
	def create(self, values):
		...
		if not values.get('name', False) or values['name'] == _('New'):
			code = "%s.production.record" % values['record_type']
			values['name'] = self.env['ir.sequence'].next_by_code(code) or _('New')
		production = super(AssemblyProductionRecord, self).create(values)
	...
	def write(self, values):
		...
		if product_qty <= 0:
			raise UserError(_("The quantity must be positive!"))
		return super(AssemblyProductionRecord, self).write(values)

	def _check_disassembly_qty(self):
		self.ensure_one()
		if self.record_type == 'disassembly' and self.assembly_record_id:
			if self.assembly_record_id.disassembly_remaining_qty == 0:
				raise UserError(_('The APR you selected has been fully disassemblyd!'))
			if self.product_qty > self.assembly_record_id.disassembly_remaining_qty:
				raise UserError(_("Not enough quantity left on selected assembly record!"))
	...
	def _check_to_consume_qty(self):
		...
		for move in moves_to_check:
			...
			if quantity_done > move.assembly_product_free_qty:
	...
	def _check_tracking(self):
		...
		if self.product_tracking in ('lot', 'serial'):
			need_to_fill = []
			if self.product_qty != sum(self.finished_lot_ids.mapped('assembly_qty')):
				need_to_fill += ['finished']
			if self.rejected_qty != sum(self.rejected_lot_ids.mapped('assembly_qty')):
		...
		for product_id in tracking_byproducts.mapped('product_id'):
			...
			generated_qty = sum(self.byproduct_lot_ids.filtered(lambda l: l.product_id == product_id).mapped('assembly_qty'))
	...
	def action_confirm(self):
		...
		self._check_disassembly_qty()
		self._check_to_consume_qty()
		self._check_tracking()

		if self.record_type == 'assembly':
			# we dont want moves to be splitted
			for move in self.move_raw_ids | self.move_byproduct_ids:
				move.write({'product_uom_qty': move.assembly_quantity_done})

			# material moves
			material_moves = self.move_raw_ids
			material_moves._action_done()
			material_cost = abs(sum(material_moves.stock_valuation_layer_ids.mapped('value')))

			# byproduct moves
			byproduct_cost = 0.0
			byproduct_moves = self.move_byproduct_ids
			for byproduct in byproduct_moves.mapped('byproduct_id'):
				this_byproduct_cost = (byproduct.assembly_allocated_cost * material_cost) / 100
				this_byproduct_moves = byproduct_moves.filtered(lambda o: o.byproduct_id == byproduct)
				this_byproduct_moves.write({'price_unit': this_byproduct_cost / sum(this_byproduct_moves.mapped('product_qty'))})
				byproduct_cost += this_byproduct_cost
			...
			assembly_record = self.assembly_record_id
			# material moves
			material_moves = self.move_finished_ids.filtered(lambda m: m.product_id == self.product_id)
			for move in material_moves:
				lot_ids = []
				move_line_vals_list = [(5,)]
				
				if move.has_tracking == 'none':
					move_line_vals_list += [(0, 0, dict(move._prepare_move_line_vals(), qty_done=move.product_uom_qty))]
				else:
					qty_to_take = move.product_uom_qty

					if assembly_record:
						assembly_move_lines = assembly_record.move_finished_ids.filtered(
							lambda o: o.product_id == move.product_id).move_line_ids.filtered(
								lambda o: o.qty_done - o.disassembly_qty > 0.0)

						for lot in assembly_move_lines.mapped('lot_id'):
							if float_compare(qty_to_take, 0.0, precision_rounding=rounding) <= 0:
								break
							lot_move_lines = assembly_move_lines.filtered(lambda o: o.lot_id == lot)
							lot_qty = sum(lot_move_lines.mapped('qty_done')) - sum(lot_move_lines.mapped('disassembly_qty'))
							qty_taken = min(qty_to_take, lot_qty)
							move_line_vals_list += [(0, 0, dict(move._prepare_move_line_vals(), qty_done=qty_taken, lot_id=lot.id))]
							for move_line in lot_move_lines:
								move_line.disassembly_qty = qty_taken
							qty_to_take -= qty_taken
							lot_ids += [lot.id]
			...
			# finished moves
			finished_moves = self.move_raw_ids
			finished_moves.write({'price_unit': material_cost / sum(finished_moves.mapped('product_qty'))})
			finished_moves._action_done()
	...
	def action_submit_transfer_request(self):
		...
		for sequence, move in enumerate(self.move_raw_ids):
			qty = move.product_uom_qty - move.assembly_product_free_qty
		...
		values = {
			...
			'default_destination_warehouse_id': self.picking_type_id.default_location_src_id.get_warehouse().id,
			'default_destination_location_id': self.picking_type_id.default_location_src_id.id,
			'default_branch_id': self.branch_id.id,
			'default_scheduled_date': now,
			'default_source_document': self.name,
			'default_product_line_ids': line_ids,
			'default_assembly_id': self.id,
			'assembly_pop_back': self.id
		}
	...
	def action_submit_purchase_request(self):
		...
		line_ids = []
		for sequence, move in enumerate(self.move_raw_ids.filtered(lambda o: o.product_uom_qty - o.assembly_product_free_qty > 0.0)):
			qty = move.product_uom_qty - move.assembly_product_free_qty
	...
	def _handle_tracking(self, moves, qty_to_produce, lot_producing_ids):
		for lot_producing_id in lot_producing_ids:
			lot_producing_id.expiration_date = lot_producing_id.assembly_expiration_date
		...
		for move in moves:
			...
			if tracking == 'serial':
				...
				if move.product_id.is_in_autogenerate:
					...
					arange = len(lot_producing_ids)
					qty_done = [lot.assembly_qty for lot in lot_producing_ids]
			else:
				arange = 1
				qty_done = [qty_to_produce]
	...
	def action_generate_serial_byproducts(self):
		self.ensure_one()
		byproduct_product_ids = self.move_byproduct_ids.mapped('product_id').filtered(lambda p: p._assembly_is_auto_generate())
		...
		for product in byproduct_product_ids:
			...
			generated_qty = sum(lot_ids.mapped('assembly_qty'))
			...
			if product.tracking == 'serial':
				lot_values += [(4, product.assembly_create_next_lot(1.0, expiration_date=exp_date).id) for i in range(int(to_generate_qty))]
			else:
				if lot_ids:
					lot_values += [(1, lot_ids[-1].id, {'assembly_qty': lot_ids[-1].assembly_qty + to_generate_qty})]
				else:
					lot_values += [(4, product.assembly_create_next_lot(to_generate_qty, expiration_date=exp_date).id)]
		if lot_values:
	...
	def action_generate_serial_finished_goods(self):
		...
		if not product._assembly_is_auto_generate() or self.product_qty <= 0.0:
			return
