...
class MrpBom(models.Model):
    ...
    def _product_tmpl_domain(self):
        if self.env.context.get('default_equip_bom_type', 'mrp') == 'assembly':
            return """[
                ('type', 'in', ('product', 'consu')),
                '|', ('company_id', '=', False), ('company_id', '=', company_id),
                ('produceable_in_assembly', '=', True)
            ]"""
        return super(MrpBom, self)._product_tmpl_domain()

    product_tmpl_id = fields.Many2one(domain=_product_tmpl_domain)

    assembly_forecast_cost = fields.Float(compute='_assembly_get_bom_cost', store=False)

    @api.depends('product_id', 'product_tmpl_id', 'bom_line_ids', 'bom_line_ids.product_id', 'bom_line_ids.product_qty', 'equip_bom_type')
    def _assembly_get_bom_cost(self):
        assembly_boms = self.filtered(lambda o: o.equip_bom_type == 'assembly')

        for bom in assembly_boms:
            company = bom.company_id or self.env.company
            bom_quantity = bom.product_qty
            product = bom.product_id or bom.product_tmpl_id.product_variant_id
            total = bom._assembly_get_bom_lines(bom, bom_quantity, product, False, 1)
            byproduct_total = bom._assembly_get_byproduct_lines(bom, bom_quantity, product, False, 1, total)
            total -= byproduct_total
            bom.assembly_forecast_cost = total

        (self - assembly_boms).assembly_forecast_cost = 0.0

    def _assembly_get_bom_lines(self, bom, bom_quantity, product, line_id, level):
        ...
        for line in bom.bom_line_ids:
            ...
            if line.child_bom_id:
                ...
                sub_total = self._assembly_get_price(line.child_bom_id, factor, product)
            else:
                sub_total = price
    ...
    def _assembly_get_byproduct_lines(self, bom, bom_quantity, product, line_id, level, total_material):
        currency = self.env.company.currency_id
        byproduct_total = 0
        for line in bom.byproduct_ids:
            sub_total = currency.round((line.assembly_allocated_cost * total_material) / 100)
            byproduct_total += sub_total
        return byproduct_total

    def _assembly_get_price(self, bom, factor, product):
        is_cost_per_warehouse = eval(self.env['ir.config_parameter'].get_param('equip3_inventory_base.is_cost_per_warehouse', 'False'))
        ...
        for line in bom.bom_line_ids:
            ...
            if line.child_bom_id:
                qty = safe_div(
                    ...
                    line.child_bom_id.product_qty)
                sub_price = self._assembly_get_price(line.child_bom_id, qty, product)
                price += sub_price
            else:
                if is_cost_per_warehouse:
                    warehouse_prices = product.warehouse_price_ids
...
class MrpBomLine(models.Model):
    ...
    assembly_cost = fields.Float(string='Assembly Cost', compute='_compute_assembly_cost', store=False)
    ...
    def _compute_assembly_cost(self):
        ...
        assembly_bom_lines = self.filtered(lambda o: o.bom_id.equip_bom_type == 'assembly')
        for bom_line in assembly_bom_lines:
            ...
            if bom_line.child_bom_id:
                ...
                sub_total = bom_line.bom_id._assembly_get_price(bom_line.child_bom_id, factor, product)
            else:
                if is_cost_per_warehouse:
                    warehouse_prices = product.warehouse_price_ids
                    product_price = 0.0
            ...
            sub_total = currency.round(sub_total)
            bom_line.assembly_cost = sub_total

        (self - assembly_bom_lines).assembly_cost = 0.0


class MRPBomByProduct(models.Model):
    _inherit = 'mrp.bom.byproduct'

    assembly_allocated_cost = fields.Float(string='Assembly Allocated Cost (%)')
    assembly_cost = fields.Float(string='Assembly Cost', compute='_compute_assembly_cost', store=False)

    @api.depends('bom_id', 'bom_id.equip_bom_type', 'bom_id.bom_line_ids', 'bom_id.bom_line_ids.assembly_cost', 'assembly_allocated_cost')
    def _compute_assembly_cost(self):
        assembly_byproducts = self.filtered(lambda o: o.bom_id.equip_bom_type == 'assembly')
        for byproduct in assembly_byproducts:
            bom = byproduct.bom_id
            material_cost = sum(bom.bom_line_ids.mapped('assembly_cost'))
            byproduct.assembly_cost = (material_cost * byproduct.assembly_allocated_cost) / 100

        (self - assembly_byproducts).assembly_cost = 0.0
