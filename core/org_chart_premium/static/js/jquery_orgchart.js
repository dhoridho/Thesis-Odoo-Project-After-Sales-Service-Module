  ...
  OrgChart.prototype = {
    ...
    init: function (opts) {
      ...
      var $chart = this.$chart = $('<div>', {
        ...
        'class': 'orgchart orgchartwindowsize' + (this.options.chartClass !== '' ? ' ' + this.options.chartClass : '') + (this.options.direction !== 't2b' ? ' ' + this.options.direction : ''),
    ...
    createNode: function (data) {
      ...
      if (opts.verticalLevel && level >= opts.verticalLevel) {
        ...
        if (this.options.exportFilename == 'OrgChartDepartmentPro') {
          var job_id = '';
          if (data.className == 'company-level') {
            ...
            contentSymbol = 'fa-users btn job-id'
            job_id = 'data-id="' + data.id + '"'
          }

          // If in job position has no employee, display label "Vacant" and change the background color of the content
          if (data.className == 'job-level' && data.title == '0') {
            $nodeDiv.append('<i class="edge verticalEdge bottomEdge fa"></i>')
            .children('.content')
            .css({'background-color': '#808080', 'font-size': '14px', 'font-weight': 'bold'})
            .prepend('<span>Vacant<span>')
          }
          else {
            $nodeDiv.append('<i class="edge verticalEdge bottomEdge fa"></i>')
            .children('.content').prepend('<i class="fa '+ contentSymbol + ' symbol mr-1" style="margin:0; padding:0; color:white;"' + job_id + '></i>');
          }

          $nodeDiv.append('<i class="edge verticalEdge bottomEdge fa"></i>')
            .children('.title').prepend('<i class="fa '+ titleSymbol + ' symbol mr-1" ' + job_id + '></i>');
          // $nodeDiv.append('<i class="edge verticalEdge bottomEdge fa"></i>')
          //   .children('.content').prepend('<i class="fa '+ contentSymbol + ' symbol mr-1" style="margin:0; padding:0; color:white;"></i>');
        } else {
            $nodeDiv.append('<i class="edge verticalEdge bottomEdge fa"></i>')
            .children('.content').prepend('<i class="fa '+ contentSymbol + ' symbol mr-1" style="margin:0; padding:0; color:white;"></i>');
        }
    ...
    buildHierarchy: function ($appendTo, data) {
      ...
      if (hasChildren) {
        ...
        if (isVerticalLayer) {
          ...
          if (childrenData[0].className == 'dept-level') {
            rightLine = 'rightLineDept'
            leftLine = 'leftLineDept'
            topLine = 'topLineDept'
            downLine = 'downLineDept'
          } else if (childrenData[0].className == 'job-level') {
            rightLine = 'rightLineJob'
            leftLine = 'leftLineJob'
            topLine = 'topLineJob'
            downLine = 'downLineJob'
          }
    ...
    export: function (exportFilename, exportFileextension) {
      ...
      console.log(sourceChart.clientHeight);
      console.log(sourceChart.clientWidth);
