...
class Equip3HumanResourceMyOvertimeRequestLine(RestApi):
    @route(['/api/employee/my_overtime_request_line/','/api/employee/my_overtime_request_line/<int:id>'],auth='user', type='http', methods=['get'])
    def get_employee_overtime_request_line(self,id=None,**kw):
        offset =  self.get_limit_offset(page=int(kw.get('page')) if 'page' in kw else 1)
        obj = 'hr.overtime.request.line'
        ...
        if kw.get("is_last_7"):
            query = """
            select data.id from hr_overtime_request_line data WHERE data.date::date  >= current_date - interval '7' day and data.date  <= current_date  and data.employee_id = %s
            """
        ...
        if kw.get("is_last_30"):
            query = """
                select data.id from hr_overtime_request_line data WHERE data.date::date  >= current_date - interval '30' day and data.date  <= current_date and data.employee_id = %s            
                """
            request._cr.execute(query, [request.env.user.employee_id.id])
            data_result_ids = request.env.cr.dictfetchall()
            ids = []
        ...
        if kw.get("search"):
            domain.append("|")
            domain.append("|")
            domain.append(("request_id.name",'ilike',kw.get("search")))
        ...
        if not id:
            request_param = {"ids":','.join(str(data.id) for data in data_ids),"fields":['request_id',
                                                                                         'state',
                                                                                         'date'
                                                                                         ],
                             "order":"id desc",
        ...
        if 'start_time' in response_data[obj]:
            response_data[obj]['start_time'] = str(timedelta(hours=response_data[obj]['start_time']))
            
        if 'end_time' in response_data[obj]:
            response_data[obj]['end_time'] = str(timedelta(hours=response_data[obj]['end_time']))
            
        if 'number_of_hours' in response_data[obj]:
            response_data[obj]['number_of_hours'] = str(timedelta(hours=response_data[obj]['number_of_hours']))
