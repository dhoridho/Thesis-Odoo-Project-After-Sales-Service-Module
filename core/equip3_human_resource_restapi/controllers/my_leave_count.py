...
class Equip3HumanResourceMyleaveBalance(RestApi):
    @route(['/api/employee/my_leave_count'],auth='user', type='http', methods=['get'])
    def get_employee_leave_count(self,id=None,**kw):
        ...
        if kw.get("is_last_7"):
            query = """
            select data.id from hr_leave_count data WHERE data.start_date  >= current_date - interval '7' day and data.start_date  <= current_date  and data.employee_id = %s
            """
        ...
        if kw.get("is_last_30"):
            query = """
                select data.id from hr_leave_count data WHERE data.start_date  >= current_date - interval '30' day and data.start_date  <= current_date and data.employee_id = %s            
                """
            request._cr.execute(query, [request.env.user.employee_id.id])
            data_result_ids = request.env.cr.dictfetchall()
            ids = []
