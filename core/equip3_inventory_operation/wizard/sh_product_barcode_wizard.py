...
class ShProductBarcodeMobileWizard(models.TransientModel):
    ...
    product_line_id = fields.One2many('sh.product.barcode.mobile.wizard.line', 'sh_product_barcode_id', string="Product Line")
    ...
    def _onchange_sh_product_barcode_mobile(self):
        ...
        if self and self.sh_product_barcode_mobile:
            ...
            if search_product:
                ...
                '''
                self.post_msg = msg
                quant_ids = self.env['stock.quant'].search([('product_id', '=', search_product.id), ('location_id.usage', '=', 'internal')])
                product_line_data = []
                temp_list = []
                line_list_vals = []
                for line in quant_ids:
                    if {'product_id': line.product_id.id, 'location_id': line.location_id.id} in temp_list:
                        ...
                        if filter_list:
                            ...
                            filter_list[0]['on_hand_qty'].append(line.quantity)
                            filter_list[0]['value'].append(line.value)
                    else:
                ...
                self.product_line_id = product_line_data
...
class ShProductBarcodeMobileWizardLine(models.TransientModel):
    _name = "sh.product.barcode.mobile.wizard.line"
    _description = "Sh Product Barcode Mobile Line "

    location_id = fields.Many2one('stock.location', string="Location")
    on_hand_qty = fields.Float(string="On Hand Quantity")
    ...
    sh_product_barcode_id = fields.Many2one('sh.product.barcode.mobile.wizard', string="Product Barcode")
