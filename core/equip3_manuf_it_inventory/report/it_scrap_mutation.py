...
class ItScrapMutation(models.Model):
    _inherit = 'it.inventory.list.report'
    _name = 'it.scrap.mutation'
    _description = 'Mutation of Scraps'
    _auto = False
    ...
    def _query(self):
        ...
        SELECT
            ...
            ROW_NUMBER() OVER (ORDER BY ss.product_id) AS sequence
        FROM
            stock_scrap ss
        LEFT JOIN
            stock_scrap_request ssr
            ON (ssr.id = ss.scrap_id)
        LEFT JOIN
            stock_warehouse sw
            ON (sw.id = ssr.warehouse_id)
        WHERE 
            sw.is_it_inventory_warehouse = True
        GROUP BY
            ss.product_id
        ORDER BY
            ss.product_id
