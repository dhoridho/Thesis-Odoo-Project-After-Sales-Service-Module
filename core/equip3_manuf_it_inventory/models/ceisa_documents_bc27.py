...
class ITICeisaDocumentsBC27(models.Model):
    _name = 'ceisa.documents.bc27'
    ...
    entitas_line_id = fields.One2many('ceisa.entitas.line', 'bc27_document_id', string='Entitas')
    ...
    recipient_codeapi = fields.Selection(string='Kode Jenis API Penerima',
                                     selection=[
                                         ('01', 'Angka Pengenal Importir Umum (APIU)'),
                                         ('02', 'Angka Pengenal Importir Perseroan (APIP)'),
                                         ('04', 'Angka Pengenal Importir Terbatas (APIT)'),
    ...
    recipient_permit_number = fields.Char("Nomor Izin Penerima")
    recipient_permit_date = fields.Date("Tanggal Izin Penerima", default=fields.Date.context_today)
    ...
    document_line_id = fields.One2many('ceisa.documents.line', 'bc27_document_id', string='Lampiran Dokumen BC-2.7')
    transportation_line_id = fields.One2many('ceisa.transportation.line', 'bc27_document_id', string='Sarana Angkut')
    ...
    reason_delivery = fields.Many2one('ceisa.reason.export', string='Tujuan Pengiriman')
    trade_purpose = fields.Selection(
        string='Tujuan Pengiriman Selection',
        selection=[
            ('1', 'Penyerahan BKP'),
            ('2', 'Penyerahan JKP'),
            ('3', 'Retur'),
    ...
    product_line_id = fields.One2many('ceisa.products.line', 'bc27_document_id', string='Rincian Barang O2M')
    product_line_ids = fields.Many2many('ceisa.products.line', 'bc27_document_id', string='Rincian Barang')
    ###pungutan
    ...
    tax_line_id = fields.One2many('ceisa.tax.line', 'bc27_document_id', string='Pungutan')
    ...
    package_line_id = fields.One2many('ceisa.package.line', 'bc27_document_id', string='Kemasan')
    container_line_id = fields.One2many('ceisa.container.line', 'bc27_document_id', string='Peti Kemas')
    ###Jaminan
    guarantee_line_id = fields.One2many('ceisa.guarantee.line', 'bc27_document_id', string='Jaminan')
    ###pernyataan
    ...
    def create(self, vals):
        ...
        if not 'document_type_id' in vals:
            doc_type = self.env['ceisa.document.type'].search([('code', '=', '27')], limit=1)
            vals['document_type_id'] = doc_type.id
        result = super(ITICeisaDocumentsBC27, self).create(vals)
        return result
    ...
    def write(self, vals):
        if not 'document_type_id' in vals:
            doc_type = self.env['ceisa.document.type'].search([('code', '=', '27')], limit=1)
            vals['document_type_id'] = doc_type.id
        result = super(ITICeisaDocumentsBC27, self).write(vals)
        return result
