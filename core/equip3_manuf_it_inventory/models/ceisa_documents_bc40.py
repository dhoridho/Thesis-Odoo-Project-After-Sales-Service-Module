...
class ITICeisaDocumentsBC40(models.Model):
    _name = 'ceisa.documents.bc40'
    _description = 'CEISA Documents TPB BC-40'
    ...
    entitas_line_id = fields.One2many('ceisa.entitas.line', 'bc40_document_id', string='Entitas')
    ...
    # exim_code_api = fields.Char('Nomor API')
    ###Owner Entitas
    ...
    owner_nib_entitas = fields.Char('NIB Entitas Pemilik')
    owner_status_code = fields.Many2one('ceisa.business.status', string='Kode Status Pemilik')
    owner_codeapi = fields.Selection(string='Kode Jenis API Pemilik',
                                    selection=[
    ...
    owner_code_api = fields.Char('Nomor API Pemilik')
    ...
    sender_code_api = fields.Char('Nomor API Pengirim')
    sender_identity_number = fields.Char('Nomor Identitas Pengirim')
    sender_status_code = fields.Many2one('ceisa.business.status', string='Kode Status Pengirim')
    sender_codeapi = fields.Selection(string='Kode Jenis API Pengirim',
                                     selection=[
                                         ('01', 'Angka Pengenal Importir Umum (APIU)'),
                                         ('02', 'Angka Pengenal Importir Perseroan (APIP)'),
                                         ('04', 'Angka Pengenal Importir Terbatas (APIT)'),
    ...
    sender_nib_entitas = fields.Char("Nomor NIB Pengirim")
    ...
    document_line_id = fields.One2many('ceisa.documents.line', 'bc40_document_id', string='Lampiran Dokumen BC-4.0')
    transportation_line_id = fields.One2many('ceisa.transportation.line', 'bc40_document_id', string='Sarana Angkut')
    ...
    product_line_id = fields.One2many('ceisa.products.line', 'bc40_document_id', string='Rincian Barang O2M')
    product_line_ids = fields.Many2many('ceisa.products.line', 'bc40_document_id', string='Rincian Barang')
    ###pungutan
    ...
    tax_line_id = fields.One2many('ceisa.tax.line', 'bc40_document_id', string='Pungutan')
    ...
    package_line_id = fields.One2many('ceisa.package.line', 'bc40_document_id', string='Kemasan')
    container_line_id = fields.One2many('ceisa.container.line', 'bc40_document_id', string='Peti Kemas')
    ###Jaminan
    guarantee_line_id = fields.One2many('ceisa.guarantee.line', 'bc40_document_id', string='Jaminan')
    ###pernyataan
    ...
    trade_purpose = fields.Many2one('ceisa.export.purpose', string='Tujuan Pengiriman')
    ...
    def create(self, vals):
        ...
        if not 'document_type_id' in vals:
            doc_type = self.env['ceisa.document.type'].search([('code', '=', '40')], limit=1)
            vals['document_type_id'] = doc_type.id
        result = super(ITICeisaDocumentsBC40, self).create(vals)
        return result
    ...
    def write(self, vals):
        if not 'document_type_id' in vals:
            doc_type = self.env['ceisa.document.type'].search([('code', '=', '40')], limit=1)
            vals['document_type_id'] = doc_type.id
        result = super(ITICeisaDocumentsBC40, self).write(vals)
        return result
