...
class ITICeisaDocumentsBC25(models.Model):
    _name = 'ceisa.documents.bc25'
    _description = 'CEISA Documents TPB BC-25'
    ...
    entitas_line_id = fields.One2many('ceisa.entitas.line', 'bc25_document_id', string='Entitas')
    ...
    exim_code_api = fields.Char('Nomor API Pengusaha')
    ###Owner Entitas
    ...
    recipient_codeapi = fields.Char('Nomor API Penerima')
    recipient_niper = fields.Char('NIPER Penerima')
    ...
    document_line_id = fields.One2many('ceisa.documents.line', 'bc25_document_id', string='Lampiran Dokumen BC-2.5')
    transportation_line_id = fields.One2many('ceisa.transportation.line', 'bc25_document_id', string='Sarana Angkut')
    ...
    product_line_id = fields.One2many('ceisa.products.line', 'bc25_document_id', string='Rincian Barang O2M')
    product_line_ids = fields.Many2many('ceisa.products.line', 'bc25_document_id', string='Rincian Barang')
    ###pungutan
    ...
    tax_line_id = fields.One2many('ceisa.tax.line', 'bc25_document_id', string='Pungutan')
    ...
    package_line_id = fields.One2many('ceisa.package.line', 'bc25_document_id', string='Kemasan')
    container_line_id = fields.One2many('ceisa.container.line', 'bc25_document_id', string='Peti Kemas')
    ###Jaminan
    guarantee_line_id = fields.One2many('ceisa.guarantee.line', 'bc25_document_id', string='Jaminan')
    ###pernyataan
    ...
    def create(self, vals):
        ...
        if not 'document_type_id' in vals:
            doc_type = self.env['ceisa.document.type'].search([('code', '=', '25')], limit=1)
            vals['document_type_id'] = doc_type.id
        result = super(ITICeisaDocumentsBC25, self).create(vals)
        return result
    ...
    def write(self, vals):
        if not 'document_type_id' in vals:
            doc_type = self.env['ceisa.document.type'].search([('code', '=', '25')], limit=1)
            vals['document_type_id'] = doc_type.id
        result = super(ITICeisaDocumentsBC25, self).write(vals)
        return result
