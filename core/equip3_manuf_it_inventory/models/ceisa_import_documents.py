...
class ITICeisaImportDocuments(models.Model):
    _name = 'ceisa.import.documents'
    ...
    entitas_line_id = fields.One2many('ceisa.entitas.line', 'import_document_id', string='Entitas')
    ...
    #### Pemusatan Entitas
    central_partner_id = fields.Many2one('res.partner', string='Nama Partner Pemusatan')
    central_identity_type = fields.Selection(
        string='Jenis Identitas Pemusatan',
        selection=[
            ('5', 'NPWP 15 Digit'),
            ('0', 'NPWP 12 Digit'),
            ('1', 'NPWP 10 Digit'),
    ...
    central_identity_number = fields.Char('Nomor Identitas Pemusatan')
    central_name = fields.Char('Nama Pemusatan')
    central_address = fields.Char('Alamat Pemusatan')
    central_country = fields.Many2one('res.country', string='Kode Negara Pemusatan')
    #### PPJK Entitas
    ppjk_partner_id = fields.Many2one('res.partner', string='Nama Partner PPJK')
    ppjk_identity_type = fields.Selection(
        string='Jenis Identitas PPJK',
        selection=[
            ('5', 'NPWP 15 Digit'),
            ('0', 'NPWP 12 Digit'),
            ('1', 'NPWP 10 Digit'),
    ...
    ppjk_identity_number = fields.Char('Nomor Identitas PPJK')
    ppjk_name = fields.Char('Nama PPJK')
    ppjk_address = fields.Char('Alamat PPJK')
    ppjk_country = fields.Many2one('res.country', string='Kode Negara PPJK')
    #### SENDER Entitas
    sender_partner_id = fields.Many2one('res.partner', string='Nama Partner Pengirim')
    sender_identity_type = fields.Selection(
        string='Jenis Identitas Pengirim',
        selection=[
            ('5', 'NPWP 15 Digit'),
            ('0', 'NPWP 12 Digit'),
            ('1', 'NPWP 10 Digit'),
    ...
    sender_identity_number = fields.Char('Nomor Identitas Pengirim')
    sender_name = fields.Char('Nama Pengirim')
    sender_address = fields.Char('Alamat Pengirim')
    sender_country = fields.Many2one('res.country', string='Kode Negara Pengirim')
    #### Seller Entitas
    seller_partner_id = fields.Many2one('res.partner', string='Nama Partner Penjual')
    seller_identity_type = fields.Selection(
        string='Jenis Identitas Penjual',
        selection=[
            ('5', 'NPWP 15 Digit'),
            ('0', 'NPWP 12 Digit'),
            ('1', 'NPWP 10 Digit'),
    ...
    seller_identity_number = fields.Char('Nomor Identitas Penjual')
    seller_name = fields.Char('Nama Penjual')
    seller_address = fields.Char('Alamat Penjual')
    seller_country = fields.Many2one('res.country', string='Kode Negara Penjual')
    ...
    document_line_id = fields.One2many('ceisa.documents.line', 'import_document_id', string='Lampiran Dokumen Impor')
    transportation_line_id = fields.One2many('ceisa.transportation.line', 'import_document_id', string='Sarana Angkut')
    ...
    product_line_id = fields.One2many('ceisa.products.line', 'import_document_id', string='Rincian Barang O2M')
    product_line_ids = fields.Many2many('ceisa.products.line', 'import_document_id', string='Rincian Barang')
    ###pungutan
    ...
    tax_line_id = fields.One2many('ceisa.tax.line', 'import_document_id', string='Pungutan')
    ...
    package_line_id = fields.One2many('ceisa.package.line', 'import_document_id', string='Kemasan')
    container_line_id = fields.One2many('ceisa.container.line', 'import_document_id', string='Peti Kemas')
    ###Jaminan
    guarantee_line_id = fields.One2many('ceisa.guarantee.line', 'import_document_id', string='Jaminan')
    ###pernyataan
    ...
    def create(self, vals):
        ...
        result = super(ITICeisaImportDocuments, self).create(vals)
        return result
    ...
    def write(self, vals):
        if not 'document_type_id' in vals:
            doc_type = self.env['ceisa.document.type'].search([('code', '=', '20')], limit=1)
            vals['document_type_id'] = doc_type.id
        result = super(ITICeisaImportDocuments, self).write(vals)
        return result

    @api.onchange('picking_id')
    def _onchange_receive_notes(self):
        ceisa_value = []
        product_line = []
        ...
        for pick in picking_obj:
            ...
            self.owner_partner_id = pick.partner_id.id
            self.owner_identity_number = pick.partner_id.vat
            self.owner_address = pick.partner_id.city  ###pick.partner_id.city_id.name
            self.owner_country = pick.partner_id.country_id.id
            self.sender_partner_id = pick.partner_id.id
            self.sender_address = pick.partner_id.city  ###pick.partner_id.city_id.name
            self.sender_country = pick.partner_id.country_id.id
            self.seller_partner_id = pick.partner_id.id
            self.seller_address = pick.partner_id.city  ###pick.partner_id.city_id.name
            self.seller_country = pick.partner_id.country_id.id
