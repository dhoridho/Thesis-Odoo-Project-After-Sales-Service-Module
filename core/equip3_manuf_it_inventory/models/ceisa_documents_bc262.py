...
class ITICeisaDocumentsBC262(models.Model):
    _name = 'ceisa.documents.bc262'
    ...
    entitas_line_id = fields.One2many('ceisa.entitas.line', 'bc262_document_id', string='Entitas')
    ...
    trade_purpose = fields.Many2one('ceisa.import.purpose', string='Tujuan Pemasukan')
    origin_location_id = fields.Many2one('res.country.city', string='Daerah Asal Barang')
    document_line_id = fields.One2many('ceisa.documents.line', 'bc262_document_id', string='Lampiran Dokumen BC-2.6.2')
    transportation_line_id = fields.One2many('ceisa.transportation.line', 'bc262_document_id', string='Sarana Angkut')
    ...
    product_line_id = fields.One2many('ceisa.products.line', 'bc262_document_id', string='Rincian Barang O2M')
    product_line_ids = fields.Many2many('ceisa.products.line', 'bc262_document_id', string='Rincian Barang')
    ###pungutan
    ...
    tax_line_id = fields.One2many('ceisa.tax.line', 'bc262_document_id', string='Pungutan')
    ...
    package_line_id = fields.One2many('ceisa.package.line', 'bc262_document_id', string='Kemasan')
    container_line_id = fields.One2many('ceisa.container.line', 'bc262_document_id', string='Peti Kemas')
    ###Jaminan
    guarantee_line_id = fields.One2many('ceisa.guarantee.line', 'bc262_document_id', string='Jaminan')
    ###pernyataan
    ...
    def create(self, vals):
        ...
        if not 'document_type_id' in vals:
            doc_type = self.env['ceisa.document.type'].search([('code', '=', '262')], limit=1)
            vals['document_type_id'] = doc_type.id
        result = super(ITICeisaDocumentsBC262, self).create(vals)
        return result
    ...
    def write(self, vals):
        if not 'document_type_id' in vals:
            doc_type = self.env['ceisa.document.type'].search([('code', '=', '262')], limit=1)
            vals['document_type_id'] = doc_type.id
        result = super(ITICeisaDocumentsBC262, self).write(vals)
        return result
