...
class ITICeisaDocumentsBC41(models.Model):
    _name = 'ceisa.documents.bc41'
    _description = 'CEISA Documents TPB BC-41'
    ...
    entitas_line_id = fields.One2many('ceisa.entitas.line', 'bc41_document_id', string='Entitas')
    ...
    owner_permit_date = fields.Date("Tanggal Izin Pemilik")
    ...
    document_line_id = fields.One2many('ceisa.documents.line', 'bc41_document_id', string='Lampiran Dokumen BC-4.1')
    transportation_line_id = fields.One2many('ceisa.transportation.line', 'bc41_document_id', string='Sarana Angkut')
    ...
    product_line_id = fields.One2many('ceisa.products.line', 'bc41_document_id', string='Rincian Barang O2M')
    product_line_ids = fields.Many2many('ceisa.products.line', 'bc41_document_id', string='Rincian Barang')
    ###pungutan
    ...
    tax_line_id = fields.One2many('ceisa.tax.line', 'bc41_document_id', string='Pungutan')
    ...
    package_line_id = fields.One2many('ceisa.package.line', 'bc41_document_id', string='Kemasan')
    container_line_id = fields.One2many('ceisa.container.line', 'bc41_document_id', string='Peti Kemas')
    ###Jaminan
    guarantee_line_id = fields.One2many('ceisa.guarantee.line', 'bc41_document_id', string='Jaminan')
    ###pernyataan
    ...
    def create(self, vals):
        ...
        if not 'document_type_id' in vals:
            doc_type = self.env['ceisa.document.type'].search([('code', '=', '41')], limit=1)
            vals['document_type_id'] = doc_type.id
        result = super(ITICeisaDocumentsBC41, self).create(vals)
        return result
    ...
    def write(self, vals):
        if not 'document_type_id' in vals:
            doc_type = self.env['ceisa.document.type'].search([('code', '=', '41')], limit=1)
            vals['document_type_id'] = doc_type.id
        result = super(ITICeisaDocumentsBC41, self).write(vals)
        return result
