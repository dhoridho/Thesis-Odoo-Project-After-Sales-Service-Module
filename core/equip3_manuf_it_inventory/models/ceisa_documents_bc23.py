...
class ITICeisaDocumentsBC23(models.Model):
    _name = 'ceisa.documents.bc23'
    ...
    entitas_line_id = fields.One2many('ceisa.entitas.line', 'bc23_document_id', string='Entitas')
    ...
    exim_permit_date = fields.Date("Tanggal Izin Pengusaha")
    ...
    owner_permit_number = fields.Char("Nomor Izin Pemilik")
    owner_permit_date = fields.Date("Tanggal Izin Pemilik", default=fields.Date.context_today)
    owner_status_code = fields.Many2one('ceisa.business.status', string='Kode Status')
    owner_codeapi = fields.Char('Kode API Pemilik')
    ...
    #### Supplier Entitas
    supplier_partner_id = fields.Many2one('res.partner', string='Nama Partner Pemasok')
    supplier_identity_type = fields.Selection(
        string='Jenis Identitas Pemasok',
        selection=[
            ('5', 'NPWP 15 Digit'),
            ('0', 'NPWP 12 Digit'),
            ('1', 'NPWP 10 Digit'),
    ...
    supplier_name = fields.Char('Nama Pemasok')
    supplier_address = fields.Char('Alamat Pemasok')
    supplier_country = fields.Many2one('res.country', string='Kode Negara Pemasok')
    ...
    document_line_id = fields.One2many('ceisa.documents.line', 'bc23_document_id', string='Lampiran Dokumen BC-2.3')
    transportation_line_id = fields.One2many('ceisa.transportation.line', 'bc23_document_id', string='Sarana Angkut')
    ...
    product_line_id = fields.One2many('ceisa.products.line', 'bc23_document_id', string='Rincian Barang O2M')
    product_line_ids = fields.Many2many('ceisa.products.line', 'bc23_document_id', string='Rincian Barang')
    ###pungutan
    ...
    tax_line_id = fields.One2many('ceisa.tax.line', 'bc23_document_id', string='Pungutan')
    ...
    package_line_id = fields.One2many('ceisa.package.line', 'bc23_document_id', string='Kemasan')
    container_line_id = fields.One2many('ceisa.container.line', 'bc23_document_id', string='Peti Kemas')
    ###Jaminan
    guarantee_line_id = fields.One2many('ceisa.guarantee.line', 'bc23_document_id', string='Jaminan')
    ###pernyataan
    ...
    def create(self, vals):
        ...
        if not 'document_type_id' in vals:
            doc_type = self.env['ceisa.document.type'].search([('code', '=', '23')], limit=1)
            vals['document_type_id'] = doc_type.id
        result = super(ITICeisaDocumentsBC23, self).create(vals)
        return result
    ...
    def write(self, vals):
        if not 'document_type_id' in vals:
            doc_type = self.env['ceisa.document.type'].search([('code', '=', '23')], limit=1)
            vals['document_type_id'] = doc_type.id
        result = super(ITICeisaDocumentsBC23, self).write(vals)
        return result
