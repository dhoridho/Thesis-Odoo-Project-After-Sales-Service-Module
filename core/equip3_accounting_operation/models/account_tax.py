...
class AccountTax(models.Model):
    ...
    def compute_all(self, price_unit, currency=None, quantity=1.0, product=None, partner=None, is_refund=False, handle_price_include=True):
        ...
        tmp_tax_exclude = taxes.filtered(lambda r: r.price_include == False)
        tmp_tax_vals = tmp_tax_exclude + tmp_tax_include
        taxes = tmp_tax_vals
        ...
        total_excluded = self.tax_round(recompute_base(base, incl_fixed_amount, incl_percent_amount, incl_division_amount))
        ...
        for tax in taxes:
            ...
            # This is the default way to round tax amount : // tax_amount = round(tax_amount, precision_rounding=prec) //
            # When this module has been installed. tax rounding change to : // tax_amount = self.tax_round(tax_amount, precision_rounding=prec) //
            # This rounding based on Tax Round Type which is set up in Accounting Configuration.
            tax_amount = self.tax_round(tax_amount, precision_rounding=prec)
    ...
    #     #total_excluded = currency.round(recompute_base(base, incl_fixed_amount, incl_percent_amount, incl_division_amount))
    #     tax_excluded_val = recompute_base(base, incl_fixed_amount, incl_percent_amount, incl_division_amount)
    #     tax_base_val = base
    #     # 4) Iterate the taxes in the sequence order to compute missing tax amounts.
    #     # Start the computation of accumulated amounts at the total_excluded value.
    #     # base = total_included = total_void = total_excluded
    ...
    #     base = total_included = total_void = total_excluded = tax_excluded_val
    ...
    #         #     tmp_taxes = total_excluded if price_include else total_included
        
    #         #     tax_amount = total_included_checkpoints[i] - ( tmp_taxes + cumulated_tax_included_amount)
    #         #     cumulated_tax_included_amount = 0
    #         # else:
        
    #         #     tmp_taxes = total_excluded if price_include else total_included
        
    #         #     tax_amount = tax.with_context(force_price_include=False)._compute_amount(tmp_taxes, sign * price_unit, quantity, product, partner)
    #         tmp_taxes = tax_base_val if price_include else tax_excluded_val
    #         tax_amount = tax.with_context(force_price_include=price_include)._compute_amount(tmp_taxes, sign * price_unit, quantity, product, partner)
    #         # if price_include:
    #         # Round the tax_amount multiplied by the computed repartition lines factor.
    #         # This is the default way to round tax amount : // tax_amount = round(tax_amount, precision_rounding=prec) //
    #         # When this module has been installed. tax rounding change to : // tax_amount = self.tax_round(tax_amount, precision_rounding=prec) //
    ...
    #                 'name': partner and tax.with_context(lang=partner.lang).name or tax.name,
    #                 'amount': sign * line_amount,
    #                 'base': round((sign * tax_base_val) if price_include else (sign * tax_excluded_val), precision_rounding=prec),
    #                 'sequence': tax.sequence,
    #                 'account_id': tax.cash_basis_transition_account_id.id if tax.tax_exigibility == 'on_payment' else repartition_line.account_id.id,
    #                 'analytic': tax.analytic,
    #                 'price_include': price_include,
