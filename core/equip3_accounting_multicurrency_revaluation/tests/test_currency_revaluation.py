...
class TestCurrencyRevaluation(common.SavepointCase):
    ...
    def setUpClass(cls):
        ...
        cls.company = cls.env.ref("equip3_accounting_multicurrency_revaluation.res_company_reval")
        cls.env.user.write({"company_ids": [(4, cls.company.id, False)]})
        cls.env.user.company_id = cls.company
        cls.reval_journal = cls.env.ref(
            "equip3_accounting_multicurrency_revaluation.reval_journal"
        )
        cls.company.write(
            {
                "revaluation_loss_account_id": cls.env.ref(
                    "equip3_accounting_multicurrency_revaluation.acc_reval_loss"
                ).id,
                "revaluation_gain_account_id": cls.env.ref(
                    "equip3_accounting_multicurrency_revaluation.acc_reval_gain"
                ).id,
        ...
        payable_acc = cls.env.ref("equip3_accounting_multicurrency_revaluation.demo_acc_payable")
    ...
    def test_revaluation_payment(self):
        ...
        acc_suspense = self.env.ref(
            "equip3_accounting_multicurrency_revaluation.demo_acc_suspense"
        )
        ...
        payment_method = self.env.ref(
            "equip3_accounting_multicurrency_revaluation.account_payment_method_manual_in"
        )
    ...
    def update_company(self, with_analytic=False, **options):
        if with_analytic:
            for field, xml_id in [
                ...
                xml_id = "equip3_accounting_multicurrency_revaluation." + xml_id
                options[field] = self.env.ref(xml_id)
        self.company.write(options)

    def create_invoice(self, date, currency, quantity, price):
        revenue_acc = self.env.ref("equip3_accounting_multicurrency_revaluation.demo_acc_revenue")
        payment_term = self.env.ref("account.account_payment_term_end_following_month")
        sales_journal = self.env.ref("equip3_accounting_multicurrency_revaluation.sales_journal")
        invoice_line_data = {
