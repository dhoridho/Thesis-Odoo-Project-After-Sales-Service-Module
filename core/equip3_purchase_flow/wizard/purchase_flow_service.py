...
class PurchaseFlowService(models.TransientModel):
    _name = 'purchase.flow.service'

    name = fields.Char(string='Name', default='Purchase Flow Services Order')
    ...
    def button_blanket_order(self):
        action = self.env.ref('equip3_purchase_other_operation.product_purchase_blanket_order_services').read()[0]
        return action
    ...
    def button_direct_purchase(self):
        action = self.env.ref('equip3_purchase_other_operation_cont.action_direct_purchase_service').read()[0]
        return action
    ...
    def button_service_work_order(self):
        action = self.env.ref('equip3_purchase_operation.service_work_order_action').read()[0]
        return action