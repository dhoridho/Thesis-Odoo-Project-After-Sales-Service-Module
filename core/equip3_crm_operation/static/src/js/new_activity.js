...
var BasicActivity = AbstractField.extend({
    ...
    _markActivityCancel: function (params) {
            var activityID = params.activityID;;

            return this._setActivityCancel(activityID)
                .then(this._reload.bind(this, { activity: true, thread: true }));
        },
    /**
    ...
    _setActivityCancel: function (activityID, feedback, attachmentIds) {
          return this._rpc({
                  model: 'mail.activity',
    ...
    */
    _onMarkActivityDoneActions: function ($btn, $form, activityID) {
        ...
        $form.on('click', '.o_activity_popover_cancel', function (ev) {
            ev.stopPropagation();
            self._markActivityCancel({
                activityID: activityID,
            });
        });
    },
