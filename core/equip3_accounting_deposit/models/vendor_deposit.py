...
class VendorDeposit(models.Model):
    _name = 'vendor.deposit'
    _description = 'Vendor Deposit'
    ...
    partner_id = fields.Many2one('res.partner', string="Vendor", tracking=True, domain="[('is_vendor', '=', True)]")
    ...
    remaining_amount = fields.Monetary(string="Remaining Amount", readonly=True, tracking=True, compute ='_compute_remaining_amount_deposit', store=True)
    communication = fields.Char(string="Reference", tracking=True)
    move_id = fields.Many2one('account.move', string="Journal Entry", readonly=True, tracking=True, copy=False)
    ...
    deposit_account_id = fields.Many2one('account.account', string="Deposit Account", tracking=True, domain="[('user_type_id.type', '=', 'receivable'), ('company_id', '=', company_id)]")
    company_id = fields.Many2one('res.company', string='Company', default=lambda self: self.env.company, tracking=True)
    currency_id = fields.Many2one('res.currency',  string="Currency", default=lambda self: self.env.company.currency_id.id, tracking=True, store=True)
    ...
    invoice_count = fields.Integer(compute="_compute_invoice", string='Bills', copy=False)
    reconcile_count = fields.Integer(compute="_compute_reconcile", string='Bills', copy=False)
    reconcile_deposit_ids = fields.Many2many('account.move', 'reconcile_vendor_deposit_rel', 'deposit_id', 'move_id', string="Reconciled", copy=False)
    invoice_deposit_ids = fields.Many2many('account.move', 'vendor_deposit_invoice_rel', 'deposit_id', 'invoice_id', string="Billed", copy=False)
    ...
    is_vendor_deposit_approval_matrix = fields.Boolean(string="Is Vendor Deposit Approval Matrix", compute='_get_approve_button_from_config')
    is_allowed_to_wa_notification_vendor_deposit = fields.Boolean(string="Is Allowed to WA Notification Vendor Deposit",compute='_get_approve_button_from_config')
    approved_matrix_ids = fields.One2many('approval.matrix.accounting.lines', 'acc_vendor_deposit_id', string="Approved Matrix")
    ...
    deposit_history = fields.Many2many('account.move', 'vendor_deposit_history_rel', 'deposit_id', 'move_id', string="Deposit History", readonly=True, copy=False)
    ...
    convert_to_expense_move_id = fields.Many2one('account.move', string="Convert to Expense", readonly=True, tracking=True, copy=False)
    deposit_return_ids = fields.Many2many('account.move', 'ven_deposit_return_ids_rel', 'deposit_id', 'move_id', string="Deposit return", readonly=True, copy=False)
    move_ids = fields.Many2many('account.move', 'ven_deposit_move_ids_rel', 'deposit_id', 'move_id', string="Move Ids", readonly=True, copy=False)
    deposit_line_history = fields.One2many('vendor.deposit.line.history', 'ven_deposit_id', string='Deposit History', readonly=True, copy=False)
    analytic_group_ids = fields.Many2many('account.analytic.tag', 'ven_deposit_analytic_rel', 'deposit_id', 'analytic_id', domain="[('company_id', '=', company_id)]", string="Analytic Group", copy=False)
    ...
    def _send_wa_reject_vendor_deposit(self, submitter, phone_num, created_date, approver = False, reason = False):
        wa_template = self.env.ref('equip3_accounting_deposit.wa_template_new_rejection_vendor_deposit_1')
        wa_sender = waParam()
        if wa_template:
    ...
    def _send_wa_approval_vendor_deposit(self, approver, phone_num, created_date, submitter):
        wa_template = self.env.ref('equip3_accounting_deposit.wa_template_new_approval_vendor_deposit_1')
        wa_sender = waParam()
        if wa_template:
            if wa_template.broadcast_template_id:
                    ...
                    if approval_matrix_line_id.minimum_approver == len(approval_matrix_line_id.approved_users.ids):
                        ...
                        if next_approval_matrix_line_id and len(next_approval_matrix_line_id[0].user_ids) > 1:
                            for approving_matrix_line_user in next_approval_matrix_line_id[0].user_ids:
                                ...
                                if self.is_allowed_to_wa_notification_vendor_deposit:
    ...
    def _get_approval_matrix(self):
        for record in self:
            ...
            matrix_id = self.env['approval.matrix.accounting'].search([
                ...
                ('approval_matrix_type', '=', 'vendor_deposit_approval_matrix')
            ], limit=1)
    ...
    #     return super(VendorDeposit, self).search_read(domain=domain, fields=fields, offset=offset, limit=limit, order=order)
    ...
    #     # domain.extend([('company_id', '=', self.env.company.id)])
    #     domain.extend([('company_id', 'in', self.env.companies.ids)])
    #     return super(VendorDeposit, self).read_group(domain=domain, fields=fields, groupby=groupby, offset=offset, limit=limit,
    ...
    def action_add_amount_deposit(self):
        ...
        context.update({'active_model': 'vendor.deposit','active_ids': self.ids})
        action = {
                    'type': 'ir.actions.act_window',
                    'name': 'Add Amount Vendor Deposit',
                    'res_model': 'vendor.deposit.amount.wizard',
                    'view_type': 'form',
                    'view_mode': 'form',
                    'target': 'new',
                    'context': context,
    ...
    def _compute_approving_matrix_lines(self):
        data = [(5, 0, 0)]
        for record in self:
            if record.state == 'draft' and record.is_vendor_deposit_approval_matrix:
    ...
    def _send_wa_request_for_approval_vendor_deposit(self, approver, phone_num, currency, url, submitter):
            wa_template = self.env.ref('equip3_accounting_deposit.wa_template_new_request_approval_vendor_deposit_1')
            wa_sender = waParam()
            if wa_template:
                if wa_template.broadcast_template_id:
                    special_var = [{'variable' : '{approver_name}', 'value' : approver.name},
    ...
    def _get_approve_button_from_config(self):
        for record in self:
            # is_vendor_deposit_approval_matrix = self.env['ir.config_parameter'].sudo().get_param(
            #     'is_vendor_deposit_approval_matrix', False)
            # record.is_vendor_deposit_approval_matrix = is_vendor_deposit_approval_matrix
            record.is_vendor_deposit_approval_matrix = self.env['accounting.config.settings'].search([], limit=1).is_allow_vendor_deposit_approval_matrix
            record.is_allowed_to_wa_notification_vendor_deposit = self.env['accounting.config.settings'].search([], limit=1).is_allow_vendor_deposit_wa_notification
    ...
    def _compute_show_cancel(self):
        if self.amount == self.remaining_amount:
            self.is_show_cancel = True
        else:
            self.is_show_cancel = False

    @api.depends('amount', 'move_id', 'deposit_history', 'invoice_deposit_ids','reconcile_deposit_ids', 'deposit_return_ids', 'return_deposit', 'convert_to_expense_move_id')
    def _compute_remaining_amount_deposit(self):
        for rec in self:
            ...
            total_move_id = 0
            list_move_ids = []
            list_move_ids = list_move_ids + rec.move_id.ids + rec.deposit_history.ids+ rec.deposit_return_ids.ids + rec.return_deposit.ids  + rec.reconcile_deposit_ids.ids + rec.convert_to_expense_move_id.ids
            ...
            if rec.move_ids:
                for move_id in rec.move_ids:
                    ...
                    check_move_deposit = rec.move_id.filtered(lambda line: line.id == move_id.id)
            ...
            if rec.move_id:
                total_move_id = rec.move_id.amount_total_signed
                rec.amount = total_move_id + total_deposit_history
            else:
                total_move_id = rec.amount
            rec.remaining_amount = sum(rec.deposit_line_history.mapped('amount_total_signed')) if rec.deposit_line_history else total_move_id
    ...
    def create(self, vals):
        vals['name'] = self.env['ir.sequence'].next_by_code('vendor.deposit')
        return super(VendorDeposit, self).create(vals)

    def action_receive_deposit(self):
        for record in self:
            record.write({'state': 'post'})
            ref = 'Vendor Deposit ' + (record.name or '')
            name = 'Vendor Deposit ' + (record.name or '')
    ...
    def action_convert_revenue(self):
        context = dict(self.env.context) or {}
        context.update({'default_deposit_type': 'vendor_deposit'})
        return {
            'name': 'Vendor Deposit',
            'type': 'ir.actions.act_window',
    ...
    def action_view_invoice(self):
        return {
            ...
            'view_mode': 'tree,form',
            'views': [(self.env.ref('account.view_in_invoice_tree').id, 'tree'), (False, 'form')],
            'context': {'default_move_type': 'in_invoice'},
            'target': 'current',
            'domain': [('move_type', '=', 'in_invoice'), ('id', 'in', self.invoice_deposit_ids.ids)]
        }
    ...
    def action_view_list_approval(self):
        return {
            ...
            'type': 'ir.actions.act_window',
            'res_model': 'vendor.deposit.approval.line',
            'view_mode': 'tree,form',
            'target': 'current',
            'domain': [('vendor_deposit_id', 'in', self.ids),('approve', '=', False)]
        }

class CustomerDepositLineHistory(models.Model):
    _name = 'vendor.deposit.line.history'
    _description = 'Vendor Deposit History'

    ven_deposit_id = fields.Many2one('vendor.deposit', string='Vendor Deposit')
...
class VendorDepositApprovalLine(models.Model):
    _name = 'vendor.deposit.approval.line'
    _description = 'List Approval'

    vendor_deposit_id = fields.Many2one('vendor.deposit', string='Vendor Deposit')
    name = fields.Char(string='Name', related='vendor_deposit_id.name')
    ...
    def action_approve(self):
        for list_approval in self:
            for record in list_approval.vendor_deposit_id:
                action_id = self.env.ref('equip3_accounting_deposit.action_vendor_deposit')
                template_id = self.env.ref('equip3_accounting_deposit.email_template_vendor_deposit_approval_matrix')
                template_id_submitter = self.env.ref('equip3_accounting_deposit.email_template_vendor_deposit_submitter_approval_matrix')
                wa_template_id = self.env.ref('equip3_accounting_deposit.wa_template_vendor_deposit_approval_matrix')
            ...
            action = {
                ...
                'view_mode': 'form',
                'res_id': list_approval.vendor_deposit_id.id,
            }
            return action

    def action_confirm(self):
        for list_approval in self:
            for vendor_deposit_id in list_approval.vendor_deposit_id:
                debit_vals = {
    ...
    def action_reject(self):
        context = dict(self.env.context) or {}
        context.update({'active_model': 'vendor.deposit','active_ids': self.vendor_deposit_id.ids, 'id_approval': self.id})
        return {
            'type': 'ir.actions.act_window',
            'name': 'Vendor Deposit ',
            'res_model': 'wizard.vendor.deposit',
...
class ApprovalMatrixAccountingLines(models.Model):
    ...
    acc_vendor_deposit_id = fields.Many2one('vendor.deposit', string='Account Customer Deposit')
