...
var ProductConfiguratorFormController = FormController.extend({
    ...
    willStart: function () {
        ...
        if (this.initialState.data.product_template_id) {
            return this._configureProduct(
                ...
                this.initialState.data.warehouse_id.data.id,
            ).then(function () {
                return def;
            });
        }
    ...
    */
    ...
    _onFieldChanged: function (event) {
        ...
        var self = this;
        var productId = this.initialState.data.product_template_id.res_id;
        var warehouseId = 1
        if ('product_template_id' in event.data.changes){
            productId = event.data.changes.product_template_id.id;
        }
        if ('warehouse_id' in event.data.changes){
            warehouseId = event.data.changes.warehouse_id.id;
        }
        ...
        this._configureProduct(productId,warehouseId)
            .then(function () {
     ...
     */
    _configureProduct: function (productTemplateId,warehouseId) {
        var self = this;
    ...
    */
    _handleAdd: function () {
        ...
        var warehouseId = $modal.find('.warehouse_id').val();
        this.renderer.selectOrCreateProduct(
            ...
            self.rootProduct = {
                ...
                line_warehouse_id_new: parseInt(warehouseId),
                line_warehouse_id: parseInt(warehouseId),
            };
            console.log("===== kepanggil klik add")
            ...
            self.optionalProductsModal = new OptionalProductsModal($('body'), {
                ...
                title: _t('Product For Upselling'),
