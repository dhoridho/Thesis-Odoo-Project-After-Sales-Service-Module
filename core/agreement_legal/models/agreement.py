...
class Agreement(models.Model):
    ...
    def _get_default_stage_id(self):
        try:
            stage_id = self.env["agreement.stage"].search([],order="sequence asc",limit=1)
    ...
    def create(self, vals):
        if vals.get("code", _("New")) == _("New"):
            if vals.get("is_template", True):
                vals["code"] = self.env["ir.sequence"].next_by_code("contract.template")
            else:
                vals["code"] = self.env["ir.sequence"].next_by_code("contract") or _("New")   
