...
class hotel_reservation(models.Model):
    ...
    def hotel_reservation_web_unlink(self):
        hotel_reservation_ids = self.sudo().search(
            [('state', '=', 'draft')])
        remove_web_draft = self.env['ir.config_parameter'].sudo(
        ).get_param('remove_draft_web_reservation')
        # print(remove_web_draft)
        for reservation in hotel_reservation_ids:

            current_datetime = fields.Datetime.now()
            diff = current_datetime - reservation.date_order
    ...
    folio_ids = fields.Many2many('hotel.folio', compute="_compute_folio", string='Invoices', copy=False,
                                 readonly=True)
    # Promo code fields///
    applied_coupon_ids = fields.One2many(
        'coupon.coupon', 'hotel_reservation_order_id', string="Applied Coupons")
    generated_coupon_ids = fields.One2many(
        'coupon.coupon', 'reservation_order_id', string="Offered Coupons", copy=False)
    ...
    promo_code = fields.Char(related='code_promo_program_id.promo_code',
                             help="Applied program code", readonly=False)
    fiscal_position_id = fields.Many2one(
        'account.fiscal.position', string='Fiscal Position',
        domain="[('company_id', '=', company_id)]", check_company=True,
        help="Fiscal positions are used to adapt taxes and accounts for particular customers or sales orders/invoices."
        "The default value comes from the customer.")
    # promo code fields end//////

    # promo code functions start /////

    def _is_reward_in_order_lines(self, program):
        self.ensure_one()
        order_quantity = sum(self.other_items_ids.filtered(lambda line:
                                                           line.product_id == program.reward_product_id).mapped('product_uom_qty'))
        print("<<<<<<<<<<<<<<<<<<<<<<<<< _is_reward_in_order_lines >>>>>>>>>>>>>>>>>>>>>>>>", order_quantity)
    ...
    def _is_global_discount_already_applied(self):
        ...
        print("appolied programs>>>>>>>>>>", self.no_code_promo_program_ids, self.code_promo_program_id,
              self.applied_coupon_ids.mapped('program_id'), applied_programs)
        return applied_programs.filtered(lambda program: program._is_global_discount_program())

    def _get_reward_lines(self):
        "returns only hotel reservation lines with is_reward_line boolean ticked true."
        self.ensure_one()
        return self.other_items_ids.filtered(lambda line: line.is_reward_line)
    ...
    def _get_applicable_programs(self):
        ...
        programs = self.env['coupon.program'].with_context(
            ...
            '|', ('rule_date_from', '=', False), ('rule_date_from',
                                                  '<=', self.date_order),
            '|', ('rule_date_to', '=', False), ('rule_date_to',
                                                '>=', self.date_order),
    ...
    def _get_paid_order_lines(self):
        """ Returns the reservation order lines or other items lines that are not reward lines.
            It will also return reward lines being free product lines. """

        free_reward_product = self.env['coupon.program'].search(
            [('reward_type', '=', 'product')]).mapped('discount_line_product_id')
        print("freee reward products>>>>>>>>>>>>>>>>", free_reward_product)
        # if self.other_items_ids.filtered(lambda x: not x.is_reward_line or x.product_id in free_reward_product):
        return self.other_items_ids.filtered(lambda x: not x.is_reward_line or x.product_id in free_reward_product)
        # elif self.reservation_line:
        #     return self.reservation_line[0]

    def _get_base_order_lines(self, program):
        """ Returns the reservation other items order lines not linked to the given program.
        """
        return self.other_items_ids.filtered(lambda x: not (x.is_reward_line and x.product_id == program.discount_line_product_id))
    ...
    def _get_cheapest_line(self):
        ...
        other_min_value = 0
        reservation_min_value = 0
        if self.other_items_ids:
            other_items_min = min(self.other_items_ids.filtered(
                lambda x: not x.is_reward_line and x.price_subtotal > 0), key=lambda x: x['price_subtotal'])
            other_min_value = other_items_min.price_subtotal
        if self.reservation_line:
            reservation_min = min(self.reservation_line.filtered(
                lambda x: x.sub_total1 > 0), key=lambda x: x['sub_total1'])
            reservation_min_value = reservation_min.sub_total1
        if other_min_value > 0 and reservation_min_value > 0:
            if other_min_value > reservation_min_value:
                return reservation_min
            else:
                return other_items_min
        elif other_min_value > 0:
            return other_items_min
        else:
            return reservation_min

    def _get_reward_values_discount(self, program):
        print("<<<<<<<<<<<<<<<<<<<<<<<<<< _get_reward_values_discount >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>", program)
        ...
        print("toplineeeeeeeeeeeeeeesssssssssssssssss", lines)
        amount_total = sum(self._get_base_order_lines(
            program).mapped('price_subtotal'))
        print("mapped>>>>>>>>>>>>>>>>", self._get_base_order_lines(
            program).mapped('price_subtotal'))
        if program.discount_apply_on == 'cheapest_product':
            line = self._get_cheapest_line()
            print("cheapestt product lineL>>>>>>>>>>>>>>>>>>", line)
            if line:
                if amount_total:
                    discount_line_amount = min(
                        line.price_subtotal * (program.discount_percentage / 100), amount_total) if line._name == "other.items" else min(
                        line.sub_total1 * (program.discount_percentage / 100), amount_total)
                else:
                    discount_line_amount = line.sub_total1 * \
                        (program.discount_percentage / 100)
                print("amount total,discount line amount>>>>>>>>>>>>",
                      amount_total, discount_line_amount)
                if discount_line_amount:
                    tax_id = line.tax_id if line._name == "other.items" else line.taxes_id
                    taxes = self.fiscal_position_id.map_tax(tax_id)
                    print("fiscal tax>>>>>>>>>>", taxes)
                    reward_dict[tax_id] = {
                        'name': _("Discount: %s", program.name),
                        'product_id': program.discount_line_product_id.id,
            ...
            lines = self.reservation_line if self.reservation_line else lines
            if program.discount_apply_on == 'specific_products':
                ...
                free_product_lines = self.env['coupon.program'].search([('reward_type', '=', 'product'), (
                    'reward_product_id', 'in', program.discount_specific_product_ids.ids)]).mapped('discount_line_product_id')
                if lines._name == "other.items":
                    lines = lines.filtered(lambda x: x.product_id in (
                        program.discount_specific_product_ids | free_product_lines))
                else:
                    lines = lines.filtered(lambda x: x.room_number in (
                        program.discount_specific_product_ids | free_product_lines))
                print("specificlinessssssssssssssssssssssss>>>>>",
                      free_product_lines, lines)

            # when processing lines we should not discount more than the order remaining total
            currently_discounted_amount = 0
            print("self>contx>>>>>>>>>>>", self._name)
            for line in lines:
                print("line>>context>>>>>>>>>", line._name)
                tax_id = line.tax_id if line._name == "other.items" else line.taxes_id
                print(" other items lineeeeeeeeeeeeeeeeeeeee>>>>>>>", line)
                # discount_line_amount = min(self._get_reward_values_discount_percentage_per_line(
                #     program, line), amount_total - currently_discounted_amount)
                ...
                if program.discount_apply_on == 'specific_products':
                    ...
                    if lines._name == "other.items":
                        ...
                        discount_line_amount = (
                            total_amount * program.discount_percentage)/100
                    else:
                        for line in lines:
                            total_amount += line.sub_total1
                        discount_line_amount = (
                            total_amount * program.discount_percentage)/100
                else:
                    discount_line_amount = (
                        self.untaxed_amt * program.discount_percentage)/100
                print("descountlineamount>>>>>>>>>>", discount_line_amount)

                if discount_line_amount:

                    if tax_id in reward_dict:
                        # reward_dict[tax_id]['price_unit'] -= discount_line_amount
                        pass
                    else:
                        taxes = self.fiscal_position_id.map_tax(tax_id)
        ...
        if max_amount > 0:
            ...
            for val in list(reward_dict):
                amount_to_discount = amount_already_given + \
    ...
    def _get_reward_values_product(self, program):
        price_unit = self.other_items_ids.filtered(
            lambda line: program.reward_product_id == line.product_id)[0].price_unit
        print("<<<<<<<<<<<<<<<<<< _get_reward_values_product >>>>>>>>>>>>>>>>>>>>>", price_unit)
        other_items_ids = (self.other_items_ids - self._get_reward_lines()
                           ).filtered(lambda x: program._get_valid_products(x.product_id))
        max_product_qty = sum(other_items_ids.mapped('product_uom_qty')) or 1
        total_qty = sum(self.other_items_ids.filtered(
            lambda x: x.product_id == program.reward_product_id).mapped('product_uom_qty'))
        print("_get_reward_values_product other items,maxproduct,total qty>>>>>>>>>>>",
              other_items_ids, max_product_qty, total_qty)
        ...
        if program._get_valid_products(program.reward_product_id):
            print("going in 11111111111111111111111")
            ...
            if program.rule_minimum_amount:
                order_total = sum(other_items_ids.mapped(
                    'price_total')) - (program.reward_product_quantity * program.reward_product_id.lst_price)
                reward_product_qty = min(
                    reward_product_qty, order_total // program.rule_minimum_amount)
                print("going in 22222222222222222222222222222",
                      order_total, reward_product_qty)
        else:
            reward_product_qty = min(
                program.reward_product_quantity, total_qty)
            print("going in 33333333333333333333333333", reward_product_qty)

        reward_qty = min(int(int(max_product_qty / program.rule_min_quantity)
    ...
    def _create_reward_line(self, program):
        print("<<<<<<<<<<<<<<<<<<<<<<<< _create_reward_line >>>>>>>>>>>>>>>>>>>", program)
        self.write({'other_items_ids': [(0, False, value)
    ...
    def _create_reward_coupon(self, program):
        ...
        print("<<<<<<<<<<<<<<<<<<<<<<< _create_reward_coupon >>>>>>>>>>>>>>", coupon)
        if coupon:
        ...
        return coupon
    # promo code functions end ////

    def _compute_folio(self):
        for order in self:
            folio = self.env['hotel.folio'].sudo().search(
    ...
    def action_view_folio(self):
        ...
        action = self.env.ref('hotel.open_hotel_folio1_form_tree').read()[0]
        if len(folio) > 1:
    ...
    def cancel_reservation(self):
        for reservation in self:
            if reservation.state in ['draft', 'done', 'confirm']:
                room_his_id_search = self.env['hotel.room.booking.history'].search(
                    [('booking_id', '=', reservation.id)])
                room_his_id_search.unlink()
    ...
    def send_mail(self):
        template = self.env.ref(
            'hotel_management.reservation_confirmed_mail_template')
        values = {
        ...
        sending = template.send_mail(self.env.user.id,
...
class hotel_reservation_line(models.Model):
    ...
    def onchange_date_count_total_days(self):
        ...
        print("shopid>>>>>>>>>", self.line_id, shop_id)
        if not shop_id:
        ...
        # _logger.info("TOME COUNT====>>>>>%s",time_count1)
    ...
    def onchange_room_id(self):
        ...
        if self.room_number:
            ...
            ###################################################################
            _logger.info("Room ==>>>{}".format(room_line_id))
            if room_line_id.room_folio_ids:
                for history in room_line_id.room_folio_ids:
                    if history.state == 'done':
                        history_start_date = history.check_in
    ...
    sub_total1 = fields.Float(
        string='Sub Total', store=True, readonly=True, compute='count_price')
    ...
    def default_get(self, default_fields):
        res = super(hotel_reservation_line, self).default_get(default_fields)
        context = self._context
        reservation_id = self.env['hotel.reservation'].sudo().browse(
            self._context.get('line_id'))

        if len(reservation_id.reservation_line) >= 1:
            reservation_line_ids = reservation_id.reservation_line[-1]
            res['checkin'] = reservation_line_ids.checkin
            res['checkout'] = reservation_line_ids.checkout
        else:
            res['checkin'] = time.strftime('%Y-%m-%d 11:00:00')
            res['checkout'] = (date.today() + timedelta(days=1)
    ...
    def _get_display_price(self, product):
        ...
        if self.line_id.pricelist_id.discount_policy == 'with_discount':
            # print("###############", product.with_context(
            #     pricelist=self.line_id.pricelist_id.id).price)
        ...
        print("BASE PRICE FINAL PRICE", base_price, final_price)
    ...
    def action_done(self):
        for line in self:
            for invoice in line.invoice_ids:
                if (invoice.partner_id.id == line.partner_id.id) and invoice.payment_state != 'paid':
...
class product_template(models.Model):
    ...
    is_booking_date = fields.Boolean(string="Is Booking Date Applicable", default=True)

    def _get_value(self):
        _logger.info("SELF===>>>>>>>>>>>>>>>>>{}".format(self))
        product_id = self.env['product.product'].sudo().search([('product_tmpl_id','=',self.id)])
        _logger.info("Product ID===>>>>>>>>>>>>>>{}".format(product_id))
        hotel_service_id = self.env['hotel.services'].sudo().search([('service_id','=', product_id.id)])
        _logger.info("Hotel SERVICE ID===>>>>>>>>>>>>>>{}".format(hotel_service_id))
        _logger.info("HOTEL SERVICE ID===>>>>>>>>>{}".format(hotel_service_id.is_booking_date))

        if hotel_service_id:
            return hotel_service_id.is_booking_date
