...
#     name = fields.Char('Name')
    ...
    laundry_service_ids = fields.One2many(
        'hotel.laundry.services', 'hotel_laundry_service_id', 'Laundry Services', states={'confirmed': [('readonly', True)]})
...
class laundry_management(models.Model):
            ...
            if (service.request_type == 'from_room' and service.service_type == 'third_party') or (service.request_type == 'internal' and service.service_type == 'third_party' and service.is_chargable):
                ...
                for service_lines in service.laundry_service_product_ids:
                    ...
                    for service_line_item in service_lines.laundry_service_product_line_ids:
                        ...
                        # print("herrrrrrrrrrrrrrrrrrrrr66666666666")
                        invoice_id.write({
    ...
    def onchange_room_no(self):
        # print("onchange_room_no333333333333333333", self.room_number)
        res = {}
    ...
    user_id = fields.Many2one('res.users', 'Responsible', states={'confirmed': [('readonly', True)], 'laundry_returned': [
                              ('readonly', True)], 'sent_to_laundry': [('readonly', True)], 'customer_returned': [('readonly', True)], 'done': [('readonly', True)]}, default=lambda self: self._uid)
    date_order = fields.Datetime('Request Date', required=True, states={'confirmed': [('readonly', True)], 'laundry_returned': [('readonly', True)], 'sent_to_laundry': [
        ('readonly', True)], 'customer_returned': [('readonly', True)], 'done': [('readonly', True)]}, index=True, default=lambda self: datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    deadline_date = fields.Datetime('Request Deadline', states={'confirmed': [('readonly', True)], 'laundry_returned': [(
        'readonly', True)], 'sent_to_laundry': [('readonly', True)], 'customer_returned': [('readonly', True)], 'done': [('readonly', True)]},  )
    shop_id = fields.Many2one('sale.shop', 'Shop Name', states={'confirmed': [('readonly', True)], 'laundry_returned': [('readonly', True)], 'sent_to_laundry': [('readonly', True)], 'customer_returned': [
                              ('readonly', True)], 'done': [('readonly', True)]}, index=True, help="Will show list of shop that belongs to allowed companies of logged-in user.", default=_get_default_shop)
#        'warehouse_id': fields.many2one('stock.warehouse', 'Warehouse', states={'confirmed':[('readonly',True)], 'laundry_returned':[('readonly',True)],'sent_to_laundry':[('readonly',True)],'customer_returned':[('readonly',True)],'done':[('readonly',True)]}, select=True),
    company_id = fields.Many2one('res.company', 'Company', states={'confirmed': [('readonly', True)], 'laundry_returned': [('readonly', True)], 'sent_to_laundry': [
                                ('readonly', True)], 'customer_returned': [('readonly', True)], 'done': [('readonly', True)]}, required=True, index=True, default=lambda self: self.env['res.company']._company_default_get('purchase.order'))
    request_type = fields.Selection([('internal', 'Internal'), ('from_room', 'From Room')], 'Request Type', states={'confirmed': [('readonly', True)], 'laundry_returned': [
        ('readonly', True)], 'sent_to_laundry': [('readonly', True)], 'customer_returned': [('readonly', True)], 'done': [('readonly', True)]},   required=True,)
    room_number = fields.Many2one('hotel.room', 'Room No', states={'confirmed': [('readonly', True)], 'laundry_returned': [('readonly', True)], 'sent_to_laundry': [(
        'readonly', True)], 'customer_returned': [('readonly', True)], 'done': [('readonly', True)]},   help="Will show list of currently occupied room no that belongs to selected shop.")
    partner_id = fields.Many2one('res.partner', 'Guest Name', states={'confirmed': [('readonly', True)], 'laundry_returned': [
        ('readonly', True)], 'sent_to_laundry': [('readonly', True)], 'customer_returned': [('readonly', True)], 'done': [('readonly', True)]})
    supplier_id = fields.Many2one('hotel.laundry', 'Supplier', states={'confirmed': [('readonly', True)], 'laundry_returned': [
        ('readonly', True)], 'sent_to_laundry': [('readonly', True)], 'customer_returned': [('readonly', True)], 'done': [('readonly', True)]})
#        'supplier_address_id':fields.many2one('res.partner.address','Address',states={'confirmed':[('readonly',True)], 'laundry_returned':[('readonly',True)],'sent_to_laundry':[('readonly',True)],'customer_returned':[('readonly',True)],'done':[('readonly',True)]}),
    state = fields.Selection([('draft', 'Draft'), ('confirmed', 'Confirmed'), ('canceled', 'Canceled'), ('sent_to_laundry', 'Sent to Laundry'), (
        'laundry_returned', 'Laundry Returned'), ('customer_returned', 'Customer Returned'), ('done', 'Done')], 'State', readonly=True, default='draft')
    laundry_service_product_ids = fields.One2many('laundry.service.product', 'laundry_service_id', 'Laundry Service Product', states={'confirmed': [(
        'readonly', True)], 'laundry_returned': [('readonly', True)], 'sent_to_laundry': [('readonly', True)], 'customer_returned': [('readonly', True)], 'done': [('readonly', True)]})
    ...
    def get_cost_subtotal_value(self):
        ...
        for line in self:
            for records1 in line.laundry_service_product_line_ids:
                ...
                cost_subtotal, None, 1, line.laundry_services_id.laundry_services_id, line.laundry_services_id.supplier_id)
...
#            'qty_uom':fields.many2one('product.uom','UOM', ),
    ...
    cost_rate = fields.Float(compute='get_cost_value', method=True, string='Cost Rate', type="float",
                             store=True, help="This column will compute cost price based on the pricelist linked to selected supplier")
    ...
    cost_subtotal = fields.Float(compute='get_cost_subtotal_value', method=True, string='Cost Sub Total', type="float", store=True)
    ...
    cost_tax_id = fields.Many2many('account.tax', 'laundry_cost_order_tax', 'order_line_id', 'tax_id', 'Supplier Taxes',)
...
class laundry_service_product_line(models.Model):
    ...
    cost_price = fields.Float(compute='get_cost_value', method=True, string='Cost Price', store=True)
#             'cost_price':fields.float('Cost Price',  help="This column will compute cost price based on the pricelist linked to selected supplier"),
    sales_price = fields.Float('Sales Price',   help="This column will compute cost price based on the pricelist selected at header part")
#             'cost_subtotal':fields.float('Cost Sub Total', ),
    cost_subtotal = fields.Float(compute='get_cost_subtotal_value', method=True, string='Cost Sub Total', store=True)
