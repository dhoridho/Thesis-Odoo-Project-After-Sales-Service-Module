...
#  ######################################################################################################################################33
    ...
    def write(self, vals):
        ...
        if ctx.get('skip_write_overload', False):
        ...
        for product in self:
            if 'is_engcode_editable' not in vals:
                vals['is_engcode_editable'] = False
            client_vals = product.checkMany2oneClient(vals)
            sanitized_vals = product.plm_sanitize(vals)
            vals.update(client_vals)
            vals.update(sanitized_vals)
            if not product.description or ('description' in vals and not vals['description']):
                vals['description'] = '.'
        res = super(PlmComponent, self).with_context(ctx).write(vals)
        for product in self:
            if product._origin.id:
                product.checkFromOdooPlm()
