...
class MiningProductionPlanFuel(models.Model):
    _name = 'mining.production.plan.fuel'
    _description = 'Production Fuel'
    ...
    def _compute_product_ids(self):
        fuels = self.env['product.product'].search([('mining_product_type', '=', 'fuel')])
        for record in self:
            product_ids = []
            asset_id = record.asset_id
            if asset_id:
                if asset_id.vehicle_checkbox:
                    product_ids = [asset_id.fuel_type.id]
                else:
                    product_ids = fuels.ids
    ...
    product_id = fields.Many2one(comodel_name='product.product', string='Fuel', domain="[('id', 'in', product_ids)]", required=True)
