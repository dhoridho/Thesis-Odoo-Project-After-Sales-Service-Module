...
class WizardMPOrderReturn(models.TransientModel):
    _name = 'wiz.mp.order.return'
    _description = 'Wizard Marketplace Order Return'

    PARAMS = [
        ('by_date_range', 'By Date Range'),
        ('by_mp_return_number', 'By Return Number'),
    ]
    ...
    mp_return_number = fields.Char(string="MP Return Number", required=False)
    ...
    def get_return(self):
        ...
        elif self.params == 'by_mp_return_number':
            kwargs.update({'mp_return_number': self.mp_return_number})
        if hasattr(self.mp_account_id, '%s_get_return' % self.mp_account_id.marketplace):
            getattr(self.mp_account_id, '%s_get_return' % self.mp_account_id.marketplace)(**kwargs)
