...
class MPWebhookOrder(models.Model):
    ...
    mp_invoice_number = fields.Char(string='Marketplace Invoice Number', index=True)
    ...
    is_process = fields.Boolean(string="Is Processed ?", default=False)
    ...
    def fetch_webhook_order(self):
        ...
        if marketplace.count(marketplace[0]) == len(marketplace):
            if mp_account_ids.count(mp_account_ids[0]) == len(mp_account_ids):
                if hasattr(self, '%s_fetch_webhook_order' % marketplace[0]):
                    return getattr(self, '%s_fetch_webhook_order' % marketplace[0])()
            else:
