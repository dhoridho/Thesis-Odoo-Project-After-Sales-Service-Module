        ...
        */
        ...
        _preprocessSlideData: function (slidesDataList) {
            slidesDataList.forEach(function (slideData, index) {
                ...
                if (slideData.type === 'video') {
                    ...
                    slideData.src = $(slideData.embedCode).attr('src');  // embedCode containts an iframe tag, where src attribute is the url (youtube or embed document from odoo)
                    slideData.embedUrl =  'http:' + slideData.src;
                } else if (slideData.type === 'presentation_g_drive') {
                    slideData.embedCode = slideData.embedCode;
                    slideData.src = $(slideData.embedCode).attr('src');  // embedCode containts an iframe tag, where src attribute is the url (youtube or embed document from odoo)
                    slideData.embedUrl =  'http:' + slideData.src;
                }
                ...
                slideData._autoSetDone = _.contains(['infographic', 'presentation', 'document', 'webpage', 'googledrivevideo', 'zoom_meeting', 'presentation_g_drive'], slideData.type) && !slideData.hasQuestion;
            });
        ...
        _renderSlide: function () {
            ...
            if (_.contains(['document', 'presentation', 'infographic'], slide.type)) {
                ...
                return this.videoPlayer.appendTo($content);
            } else if (slide.type === 'presentation_g_drive') {
                this.videoPlayer = $(slide.embedCode);
                return this.videoPlayer.appendTo($content);
            }
            return Promise.resolve();
