odoo.define('equip3_asset_fsm_tracking.asset_tracking', function(require){
    ...
    function get_vehicle_asset(){
        ajax.jsonRpc("/asset", "call", {}).then(function(data){
            ...
            for (let i = 0; i < all_nav.length; i++) {
              // all[0].setHTML("asda")
              // console.log(all_nav[i].innerHTML)
              if(all_nav[i].innerHTML == 'Online'){
                all_nav[i].setHTML(`<span>Online</span> (${online_count})`)
              }
              if(all_nav[i].innerHTML == 'Offline'){
                all_nav[i].setHTML(`<span>Offline</span> (${offline_count})`)
              }
            }
    ...
    $(document).ready(function(){
      if ($('#asset_tracking_page').length > 0){
        ...
        window.get_vehicle = setInterval(function() {
          console.log('setInterval page load', new Date().toLocaleString())
          get_vehicle_asset();
        }, 20000);
      }
    ...
    $(document.body).on('click', '.vehicle_row' ,function(){
      ...
      get_vehicle_asset()
      window.get_vehicle_sin = setInterval(function() {
