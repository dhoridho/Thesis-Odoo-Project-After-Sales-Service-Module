...
class hotel_folio(models.Model):
    ...
    def button_dummy11(self):
        ...
        total = val1 + val
        if val:
            self._cr.execute("""UPDATE sale_order SET amount_untaxed =%s, amount_tax =%s, amount_total =%s WHERE id =%s""",
                             (val1, val, total, order.order_id.id))
        else:
            self._cr.execute(
                """UPDATE sale_order SET amount_untaxed =%s, amount_total =%s WHERE id =%s""", (val1, val1, order.order_id.id))
        self._cr.commit()
        return True
    ...
    order_reserve_invoice_ids = fields.Many2many(
        'account.move', 'order_reserve_invoice_rel', 'folio_id', 'invoice_id', 'Order Reservation Invoices', readonly=True)
    ...
    def on_change_checkout(self):
        ...
        if self.checkin_date:
            diffDate = datetime.datetime(*time.strptime(self.checkout_date, '%Y-%m-%d %H:%M:%S')[
                                         :5]) - datetime.datetime(*time.strptime(self.checkin_date, '%Y-%m-%d %H:%M:%S')[:5])
            qty = diffDate.days
...
class hotel_services(models.Model):
    ...
    def open_website_url(self):
        self.ensure_one()
        res = self.service_id.product_tmpl_id.open_website_url()
        res['url'] = self.service_id.website_url
        return res
