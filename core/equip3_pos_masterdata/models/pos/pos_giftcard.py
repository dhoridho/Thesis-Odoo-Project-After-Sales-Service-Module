...
class PosGiftCard(models.Model):
    ...
    def _send_mail_balance_and_expired_coupon(self, expired=False, balance=False):
        ...
        template_id = self.env['ir.model.data'].get_object_reference('equip3_pos_masterdata',
                                                                     'email_template_for_coupon_expire_7')
        balance_template_id = self.env['ir.model.data'].get_object_reference('equip3_pos_masterdata',
    ...
    def write_gift_card_from_ui(self, new_card_no):
        ...
        try:
            template_id = self.env['ir.model.data'].get_object_reference('equip3_pos_masterdata', 'email_template_exchange_number')
            if template_id and template_id[1]:
