odoo.define('equip3_pos_masterdata.FeatureButtons', function (require) {
    'use strict';
                    ...
                    for (let n = 0; n < selectedOrder.orderlines.models.length; n++) {
                        ...
                        if (l.product.type == 'product' && l.product.get_qty_available() < currentStockInCart) {
                            ...
                            this.env.pos.alert_message({
                                ...
                                body: l.product.display_name + this.env._t(' not enough for sale. Current stock on hand only have: ') + l.product.get_qty_available(),
                                timer: 10000
                            })
                        }
                    }
...
//        }
        ...
        _totalWillPaid() {
            ...
            this.state.total = this.env.pos.format_currency(total,false,currency);
            this.state.due = this.env.pos.format_currency(due,false,currency);
        ...
        async sendInput(key) {
            ...
            if (key == 'syncBackEnd') {
                ...
                if (confirmed) {
                    ...
                    if (!pingServer) {
                        return this.showPopup('ErrorPopup', {
                            title: this.env._t('Your Server Offline'),
                            body: this.env._t('Your internet or Hashmicro server Offline, not possible refresh POS Database Cache')
                        })
                    }
