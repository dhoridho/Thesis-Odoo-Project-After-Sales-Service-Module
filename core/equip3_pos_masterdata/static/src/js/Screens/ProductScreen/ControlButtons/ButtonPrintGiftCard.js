odoo.define('equip3_pos_masterdata.ButtonPrintGiftCard', function (require) {
    'use strict';
    ...
    class ButtonPrintGiftCard extends PosComponent {
        ...
        async onClick() {
            ...
            let lists = this.env.pos.couponGiftCardTemplate.filter(t => t.reward.discount_fixed_amount > 0 && t.program_type == 'coupon_program' && t.reward.discount_type == 'fixed_amount').map((t) => ({
                id: t.id,
                item: t,
                name: t.name + this.env._t(' with Amount ') + this.env.pos.format_currency(t.reward.discount_fixed_amount,false,currency)
            }))
            if (lists.length == 0) {
                return this.env.pos.alert_message({
                    title: this.env._t('Warning'),
                    body: this.env._t('Have not Gift Card Template with amount: ') + this.env.pos.format_currency(selectedLine.get_price_with_tax() / selectedLine.quantity,false,currency) + this.env._t('. Please go to menu Sale / Product / Gift Card Template: Create 1 template with Fixed Amount the same with amount of Selected Line and add it to POS Config / Gift Card can Create from POS')
                })
            }
            ...
            if (confirmed && selectedItems['items'].length > 0) {
                ...
                if (confirmed) {
                    ...
                    if (this.env.pos.config.validate_coupon) {
                        validate = await this.env.pos._validate_action(this.env._t(' Request your Manager approve create ') + totalGiftNumber + this.env._t(' Gift Card, with Amount fixed is: ') + this.env.pos.format_currency(couponProgram.reward.discount_fixed_amount,false,currency));
                    }
                    if (!validate) {
                        return false;
                    } else {
