odoo.define('equip3_pos_masterdata.PaymentMethodButton', function (require) {
    'use strict';
    ...
    const RetailPaymentMethodButton = (PaymentMethodButton) =>
        class extends PaymentMethodButton {
            ...
            get isButtonHidden() {
                let is_hidden = false;
                if(this.props.paymentMethod.is_receivables){
                    is_hidden = true;
                    if(this._currentOrder.get_client()){ // if Select Customer
                        is_hidden = false;
                    }
                }
                return is_hidden
