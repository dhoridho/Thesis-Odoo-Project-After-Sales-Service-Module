odoo.define('equip3_pos_masterdata.TakeAwayOrder', function (require) {
    'use strict';
    ...
    class TakeAwayOrder extends PosComponent {
        constructor() {
            super(...arguments);
            this.state = {
        ...
        get is_new_order_take_away(){
            if (!['Done', 'Ready Transfer'].includes(this.props.order.state)) return true
        }
        get is_done_order_take_away(){
            if (['Done', 'Ready Transfer'].includes(this.props.order.state)) return true
        }
    ...
    TakeAwayOrder.template = 'TakeAwayOrder';

    Registries.Component.add(TakeAwayOrder);
