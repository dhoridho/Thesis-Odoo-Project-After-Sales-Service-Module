odoo.define('equip3_pos_masterdata.KitchenScreen', function (require) {
    'use strict';
    ...
    class KitchenScreen extends IndependentToOrderScreen {
        constructor() {
            ...
            if (this.env.pos.config.screen_type != 'kitchen') {
                this.orders = this.orders.filter(r => r.state == "Ready Transfer")
            } else {
                this.orders = this.orders.filter(r => r.state == "New")
            }
            ...
            useListener('singleLineDone', this.singleActionOrderReceiptDone);
            this.searchDetails = {};
            this.filter = null;
        ...
        async getCheckerData(){
            ...
            await this.rpc({
                ...
                method: "get_checker_data",
                args: [],
            }).then(function (results) {
                _.each(results, function (line) {
                    checkerData.hasOwnProperty(line.pos_order_name)? '': checkerData[line.pos_order_name] = [];
                    checkerData[line.pos_order_name].push(line);
                });
                self.kitchenCheckerData = checkerData;
                self.render();
            });
        }
        _tableLongpolling() {
            ...
            if(this.env.pos.config.checker_screen){
                this.getCheckerData();
            }
        }
        ...
        sortOrder(orders) {
            ...
            if (this.env.pos.config.screen_type == 'kitchen' && this.env.pos.config.screen_type == 'takeaway') {
        ...
        newOrderReceiptsComing(receiptOrder) {
            ...
            if (this.env.pos.config.screen_type == 'takeaway'){
                for (var i = this.state.orders.length - 1; i >= 0; i--) {
                    if(this.state.orders[i].uid == receiptOrder.uid){
                        if(receiptOrder.state == 'Ready Transfer'){
                            //Set update_time after ticked on the KOT
                            receiptOrder['update_time'] = new Date().getTime();
                        }
                    }
                }
            }
        ...
        orderReceiptRemoved(vals) {
            ...
            let screen_type = this.env.pos.config.screen_type;
            if (screen_type == 'kitchen') { // 1st: set state for kitchen know receipt has removed by waiters
                ...
                this.env.pos.db.saveOrderReceipts(this.state.orders);
            }else if(screen_type == 'takeaway'){
                //Keep the State to new, before the kitchen is done
                this.state.orders.forEach(r => {
                    if (r.uid == vals.uid) {
                        r.userAction = vals.user
                        // if (vals.action == 'unlink_order') {
        ...
        singleActionOrderItemReceiptDone(order, state) {
            const self = this;
            var selLine = event.detail
        ...
        deliveryAll(event) { // method for kitchen only
            ...
            order.new.forEach(l => {
                if (['New', 'Priority', 'Paid'].includes(l.state)) {
                    l.state = 'Ready Transfer'
                    l.state = 'Done'
            ...
            order.state = 'Done'
            this.state.orders = this.state.orders.filter(o => o.request_time != order.request_time) // remove ticket out of screen
            this.saveOrderReceipts()
            this.state.selectedOrder = null;
            if (this.state.orders.length > 0) { // assign new ticket
