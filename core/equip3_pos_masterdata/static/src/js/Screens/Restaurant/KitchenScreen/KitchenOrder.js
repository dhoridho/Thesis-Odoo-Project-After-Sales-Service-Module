odoo.define('equip3_pos_masterdata.KitchenOrder', function (require) {
    'use strict';
    ...
    class KitchenOrder extends PosComponent {
        constructor() {
            super(...arguments);
            this.pos_categ_names = [];
            var self = this; 
            this.pos_categ_names = JSON.parse(self.env.pos.config.product_categ_display_names);

            var hideCancelLineCount = 0;
            var hideNewLineCount = 0;
            _.each(this.props.order['cancelled'], function (line) {
                if (!self.pos_categ_names.includes(line.category)) {
                    hideCancelLineCount+=1;
                };
            });
            _.each(this.props.order['new'], function (line) {
                if (!self.pos_categ_names.includes(line.category)) {
                    hideNewLineCount+=1;
                };
            });
            var cancelLength = this.props.order['cancelled'].length;
            var newLength = this.props.order['new'].length;
            this.show_order = 1;
            if ((cancelLength > 0 && cancelLength == hideCancelLineCount) || (newLength > 0 && newLength == hideNewLineCount)) {
                this.show_order = 0;
            }
            this.props.period_minutes_warning = this.env.pos.config.period_minutes_warning
            this.state = {
