odoo.define('equip3_pos_masterdata.ClientLine', function (require) {
    'use strict';
    ...
    const RetailClientLine = (ClientLine) =>
        class extends ClientLine {
            ...
            async _autoSyncBackend() {
                ...
                if (!this.env.pos.config.sync_partners_realtime || this.state.refresh == 'connecting' || this.env.pos.offlineModel) {
                    return true
                }
                const self = this
                this.env.pos.set_synch('connecting', '')
                this.state.refresh = 'connecting'
                const object = this.env.pos.get_model('res.partner');
                const partners = await this.rpc({
                    model: object.model,
                    method: 'search_read',
                    domain: [['id', '=', this.props.partner.id]],
                    ...
                    if (error && error.message && error.message.code == -32098) {
                        self.env.pos.set_synch('disconnected', 'Offline')
                    } else {
                ...
                if (partners != null) {
                    console.log('sync partner id: ' + this.props.partner.id)
                    this._syncPartner(partners)
                }
            }
            ...
            // async printBarcode() {
            //     await this.env.pos.do_action('equip3_pos_masterdata.res_partner_card_badge', {
            //         additional_context: {
            ...
            async changePricelist() {
                ...
                let {confirmed, payload: pricelist} = await this.showPopup('SelectionPopup', {
                    ...
                    id:'member_client_list_view_popup',
                    list: list,
                })
                if (confirmed) {
                    await this.rpc({
            ...
            get addClass(){
                ...
                if(this.props.partner && this.props.partner.active == false){
                    values['oe_hidden'] = true;
                }
                return values;
            }
        }
