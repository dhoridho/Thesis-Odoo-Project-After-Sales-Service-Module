odoo.define('equip3_pos_masterdata.ClientScreenWidget', function (require) {
    ...
    models.PosModel = models.PosModel.extend({
        ...
        get_logo: function () {
            if (this.config.logo) {
                ...
                return 'data:image/png;base64, ' + this.company_logo_base64
            }
        },
        ...
        render_html_for_customer_facing_display: function () { // TODO: we add shop logo to customer screen
            ...
            return Promise.all(get_image_promises).then(function () {
                ...
                var order_total_with_tax = self.format_currency(0,false,currency);
                ...
                if (order) {
                    ...
                    order_total_with_tax = self.format_currency(order.get_total_with_tax(),false,currency);
                }
                ...
                if (order) {
                    let discount = self.format_currency(order.get_total_discount(),false,currency)
                    $rendered_html.find('.pos-total').find('.pos_total-discount').html(discount);
                }
                if (order) {
                    $rendered_html.find('.pos-total').find('.pos_total-taxes').html(self.format_currency(order.get_total_tax(),false,currency));
                }
                let logo_base64 = self.get_logo();
                if(logo_base64){
                    logo_base64 = logo_base64.replace('data:image/png;base64, data:image/png;base64,', 'data:image/png;base64,');
                }
