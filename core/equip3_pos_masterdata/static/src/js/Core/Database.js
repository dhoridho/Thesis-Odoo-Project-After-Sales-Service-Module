...
odoo.define('equip3_pos_masterdata.database', function (require) {
    ...
    _super_db.add_products = function (products) {
        ...
        /** 
         Get ancestors from a new field config, because the default one always returns empty 
         and its effect on the category breadcrumb UI cannot get the product from their child category.
         */ 
        let category_ancestors = {};
        if(window.posmodel.config.category_ancestors){
            category_ancestors = JSON.parse(window.posmodel.config.category_ancestors);
        }
        ...
        for (let i = 0; i < products.length; i++) {
            ...
            if(typeof product.product_tmpl_id != 'undefined'){
            ...
            if (product.available_in_pos) {
                ...
                if (category_ancestors){
                    ancestors = category_ancestors[categ_id] || [];
                }
    ...
    _super_db.filter_add_partners = function (partners) {
    ...
    _super_db.add_partners = function (partners) {
        ...
        partners = this.filter_add_partners(partners);
        // end
        let updated_count = 0;
        let new_write_date = '';
        let partner;
        for (let i = 0, len = partners.length; i < len; i++) {
            ...
            if (partner.active == false) { // when backend delete partner, we reset variant inside partner_string_by_id and partner_by_id
    ...
    _super_db._partner_search_string = function (partner) {
        ...
        if (partner.special_name) {
            str += '|' + partner.special_name;
        }
    ...
    db.include({
        init: function (options) {
            ...
            this.payment_method_mdr = []
            this.pos_all_orders = []
            this.order_by_partner_id = {}
            this.pos_orders = []
            this.pos_all_order_lines = []
            this.line_by_id = {}

            // TODO: saved partners and products display on POS
            this.pos_coupon_by_id = {};

            //TODO: store the calculation of product pricelist
            this.cache_product_price = { 'product.product': {}, 'product.template': {} };
        },
        set_cache_product_price: function(model, product_id, values){
            if(!['product.product', 'product.template'].includes(model)){
                return false;
            }
            this.cache_product_price[model][product_id] = values;
            return true;
        },
        get_cache_product_price: function(model, product_id){
            if(!['product.product', 'product.template'].includes(model)){
                return false;
            }
            let cache = this.cache_product_price[model];
            if(cache){
                let cache_data = cache[product_id];
                if(cache_data){
                    return cache_data;
                }
                return false
        ...
        clear_cache_product_price: function(){
            this.cache_product_price['product.product'] = {};
            this.cache_product_price['product.template'] = {};
            console.warn('[clear_cache_product_price] ', this.cache_product_price);
            return this.cache_product_price;
        },
        clear_cache_product_price_by_id: function(model, product_id){
            if(!['product.product', 'product.template'].includes(model)){
                return false;
            }
            delete this.cache_product_price[model][product_id];
            console.warn('[clear_cache_product_price_by_id] model: ', model, ' ID: ', product_id);
            return true;
        },
        ...
        _product_search_string: function (product) { // TODO: supported search with name_second
            ...
            //     })
            // }
            if(window.posmodel.lot_by_product_id){
        ...
        set_pos_payment_method_mdr:function(datas){
            this.payment_method_mdr = [];
            this.payment_method_mdr_by_payment_id = {};
            for (let data of datas){
                this.payment_method_mdr.push(data);

                let payment_method_id = data.payment_method_id[0];
                if(!this.payment_method_mdr_by_payment_id[payment_method_id]){
                    this.payment_method_mdr_by_payment_id[payment_method_id] = [data];
                } else {
                    this.payment_method_mdr_by_payment_id[payment_method_id].push(data);
                }
            }
        },
        ...
        save_pos_orders: function (orders) {
            ...
            for (let i = 0; i < orders.length; i++) {
                ...
                //Creation Date
                let create_date = moment(order.create_date, 'YYYY-MM-DD HH:mm:ss')
                let _year = parseInt(create_date.format('YYYY'));
                let _month = parseInt(create_date.format('MM'));
                let _day = parseInt(create_date.format('DD'));
                let creation_date = new Date(_year,_month-1,_day);
                ...
                var check = this.pos_all_orders.findIndex(el => el.id === order.id)
                if(check>=0){
                    this.pos_all_orders.splice(check, 1);
                }
                this.pos_all_orders.unshift(order)
                ...
                if (order.partner_id) {
                    if (!this.order_by_partner_id[order.partner_id[0]]) {
                        ...
                        if(this.order_by_partner_id[order.partner_id[0]]){
                            var check = this.order_by_partner_id[order.partner_id[0]].findIndex(el => el.id === order.id)
                            if(check>=0){
                                this.order_by_partner_id[order.partner_id[0]].splice(check, 1);
                            }   
                            this.order_by_partner_id[order.partner_id[0]].unshift(order)
                        }
                        else{
                            this.order_by_partner_id[order.partner_id[0]] = [order]
                        }
            ...
            this.pos_orders = this.pos_all_orders
            this.order_search_string = "";
        ...
        save_pos_order_line: function (lines) {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                var check = order['lines'].findIndex(el => el.id === line.id)
                if(check>=0){
                    order['lines'].splice(check, 1);
                }
                order['lines'].push(line)


                var check = this.pos_all_order_lines.findIndex(el => el.id === line.id)
                if(check>=0){
                    this.pos_all_order_lines.splice(check, 1);
                }
                this.pos_all_order_lines.push(line)
        ...
        save_pos_coupon: function (coupons) {
            for (let coupon of coupons){
                this.pos_coupon_by_id[coupon.id] = coupon;
            }
        },
        get_pos_coupon_by_number(number){
            let values = [];
            for (let coupon_id in this.pos_coupon_by_id){
                let coupon = this.pos_coupon_by_id[coupon_id];
                if (coupon.number == number || coupon.code == number){
                    values.push(coupon);
                }
            }
            return values;
        },
        get_pos_coupon_by_id(coupon_id){
            let coupon = this.pos_coupon_by_id[coupon_id];
            if(!coupon){
                return false;
            }
        ...
        getTakeAwayTicketOrderNumber() {
            let RequestTakeAwayTicketNumber = this.load('RequestTakeAwayTicketNumber', 0);
            this.save('RequestTakeAwayTicketNumber', RequestTakeAwayTicketNumber + 1);
            return RequestTakeAwayTicketNumber + 1
        },
        ...
        get_pos_stock_locations(){
            let pos_config = window.posmodel.config;
            let stock_locations = [pos_config.stock_location_id[0]];
            if(pos_config.multi_location && pos_config.stock_location_ids){
                stock_locations = [...new Set([...stock_locations, ...pos_config.stock_location_ids])];
            }
