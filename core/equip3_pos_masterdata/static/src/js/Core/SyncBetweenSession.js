odoo.define('equip3_pos_masterdata.synchronization', function (require) {
    ...
    const session = require('web.session');
    const RetailOrder = require('equip3_pos_masterdata.order');
    const exports = {};
    ...
    exports.pos_bus = Backbone.Model.extend({
        ...
        syncing() {
            ...
            async function waitSync() {
                ...
                for (let i = 0; i < self.pos.iot_connections.length; i++) {
                    ...
                    // BROWSER CACHE BASED SYNC
                    // Update browser cache for sync data
                    if (results) {
                        let result_vals = [];
                        try {
                            let result_vals = JSON.parse(results)['values'];
                        }catch(err) {
                            console.error('[waitSync] result_vals error:', err.message)
                            console.error(err)
                        }
                        let requests_normal = result_vals.filter((n) => n['action'] != 'request_printer' && n['action'] != 'transfer_succeed_receipt')
                        if (requests_normal.length > 0) {
                            self.pos.db.add_sync_data(results); // store to browse cache
                        }
                    }

                    // Sync data from browser cache
                    let cache_results = self.pos.db.get_sync_data();
                    if (cache_results.length > 0) {
                        self.set_online();
                        self.repush_to_another_sessions();
                        let notifications = [];
                        try {
                            notifications = JSON.parse(cache_results)['values'];
                        }catch(err) {
                            console.error('[waitSync] cache_results error:', err.message)
                            console.error(err)
                        }
                        if (notifications.length > 0) {
                            ...
                            // Remove data from sync cache
                            const sync_data = self.pos.db.get_sync_data();
                            if (sync_data && sync_data.length) {
                                for (let i = 0; i < sync_data.length; i++) {
                                    let value = sync_data[i];
                                    self.pos.db.remove_sync_data(value['sequence']);
                                }
                            }
                        }
                    }
                    ...
                    if (results) {
                        ...
                        let notifications = [];
                        try {
                            notifications = JSON.parse(results)['values'];
                        }catch(err) {
                            console.error('[waitSync] results error:', err.message)
                            console.error(err)
                        }
                        if (notifications.length > 0) {
                            const requests_printer = notifications.filter((n) => n['action'] == 'request_printer')
        ...
        _build_send_to: function () {
            if (!this.pos.config.multi_user) {
                if(this.pos.config.sync_to_pos_config_ids){
        ...
        send_notification: function (value, send_manual = false) {
            ...
            if(this.pos.config && this.pos.config.sync_to_pos_config_ids){
    ...
    db.include({
        ...
        add_sync_data: function (data) {
            var sync_data = this.load('sync_data', []);
            this.sync_sequence += 1;
            data['sync_sequence'] = this.sync_sequence;
            sync_data.push(data)
            this.save('sync_data', sync_data);
        },
        get_sync_data: function () {
            let sync_data = this.load('sync_data');
            if (sync_data && sync_data.length) {
                return sync_data.toString();
            } else {
                return []
            }
        },
        remove_sync_data: function (sequence) {
            let sync_data = this.load('sync_data', []);
            let sync_data_new = _.filter(sync_data, function (data) {
                return data['sync_sequence'] !== sequence;
            });
            this.save('sync_data', sync_data_new);
        },
        get_orders_done() {
    ...
    models.PosModel = models.PosModel.extend({
        ...
        get_order_by_name: function (name) {
            const order = this.get('orders').models.find(o => o.name == name);
            if (!order) {
                if (this.chrome) {
                    this.chrome.showNotification(name, this.env._t('Has Removed or Paid before'))
                }
                return null
            }
            return order
        ...
        get_sync_sequence_number: async function () {
            return await rpc.query({
                ...
                console.error('[get_sync_sequence_number] Error: ', error)
                return '';
            });
        },
        ...
        sync_unlink_order: function (uid, action, user) {
            ...
            if (order) {
                ...
                if(self.config.screen_type != 'takeaway'){
                    if (selected_order && selected_order['uid'] == order['uid'] && !has_setting_restaurant) {
                        setTimeout(function () {
    ...
    models.Order = models.Order.extend({
        initialize: function (attributes, options) {
            ...
            if (!this.created_time) {
                this.created_time = new Date().toLocaleTimeString('en-US',{hour: '2-digit', minute:'2-digit'});
            }
            if (this.pos.pos_bus) {
        ...
        saveLogActionOfOrder: function (vals) {
            if (this.pos.offlineModel) {
                console.warn('You network offline')
            }
            if (!this.pos.config.pos_order_tracking || this.pos.the_first_load || this.pos.offlineModel) {
                return true
            }
