odoo.define('equip3_pos_masterdata.giftCardRedeemPopup', function(require) {
    'use strict';
    ...
    class giftCardRedeemPopup extends AbstractAwaitablePopup {
                ...
                rpc.query(params, {async: false}).then(function(res){
                    if(res.length > 0){
                        if (res[0]){
                            ...
                            self.redeem = res[0];
                            $('#lbl_card_no').html("Your Balance is  "+ self.env.pos.format_currency(res[0].card_value,false,this.env.pos.get_order().currency));
        ...
        _autoFillData(){
            ...
            QtyGetPromise.then(function(result){
                ...
                $('#lbl_card_no').html("Your Balance is  "+ self.env.pos.format_currency(result.card_value,false,self.env.pos.get_order().currency));
                $('#text_redeem_amount').show();
