odoo.define('equip3_pos_masterdata.gift_card_models', function (require) {
    "use strict";
    ...
    models.Paymentline = models.Paymentline.extend({
        ...
        get_mdr_amount_rounding: function () {
            var amount = 0
            let mdr = this.pos.db.payment_method_mdr.find((p) => p.id === this.payment_mdr_id);
            if(mdr){
                if(mdr.percentage){
                    amount += this.amount * (mdr.percentage/100);
                }
                if(mdr.surcharge){
                    amount += mdr.surcharge;
                }
            }
            var rounding = this.pos.currency.rounding;
            return round_pr(amount, rounding);
        },
        get_mdr_customer_amount: function () {
            var amount = 0
            if(this.mdr_paid_by=='Customer'){
                amount += this.get_mdr_amount_rounding();
            }
            return amount;
        },
    });
