...
class HelpdeskTicket(models.Model):
    ...
    #
    attachment_ids = fields.Many2many('ir.attachment',  'attachment_id','res_id',
                                  string="Attachment",
    ...
    def _default_team_id_data(self):
        # team_id = self.env['helpdesk.team'].search([('member_ids', 'in', self.env.uid)], limit=1).id
        team_id = self.env['helpdesk.team'].search([('is_default_team','=',True)])
        for rec in team_id:
            get_team_data = rec.id
            self.team_id = get_team_data
        return team_id
    ...
    user_id = fields.Many2one(
        ...
        domain=lambda self: [('groups_id', 'in', self.env.ref('website_axis_helpdesk.group_helpdesk_user').id)])
    ...
    def _compute_domain_user_ids(self):
        for task in self:
            if task.team_id and task.team_id.visibility_member_ids:
                helpdesk_manager = self.env['res.users'].search([('groups_id', 'in', self.env.ref('website_axis_helpdesk.group_helpdesk_manager').id)])
                task.domain_user_ids = [(6, 0, (helpdesk_manager + task.team_id.visibility_member_ids).ids)]
            else:
                helpdesk_users = self.env['res.users'].search([('groups_id', 'in', self.env.ref('website_axis_helpdesk.group_helpdesk_user').id)]).ids
    ...
    def create(self, list_value):
        ...
        template = self.env.ref('website_axis_helpdesk.new_ticket_request_email_template')
    ...
    def rating_apply(self, rate, token=None, feedback=None, subtype_xmlid=None):
        return super(HelpdeskTicket, self).rating_apply(rate, token=token, feedback=feedback, subtype_xmlid="website_axis_helpdesk.mt_ticket_rated")
    ...
    def get_weekly_tickets(self):
        ...
        query = """
           SELECT date FROM helpdesk_ticket
    ...
    def get_tickets_list(self):
        ...
        last_month_ticket = self.env['helpdesk.ticket'].sudo().search_count([('date', 'in', last_month)])
        for i in range(7, 14):
            last_week_date = today - timedelta(days=i)
            date_list.append(last_week_date)
        last_week_ticket = self.env['helpdesk.ticket'].sudo().search_count([('date', 'in', date_list)])

        total_ticket = self.env['helpdesk.ticket'].sudo().search_count([])
        today_ticket = self.env['helpdesk.ticket'].sudo().search_count([('date', '=', fields.Date.today())])

        res = {
            # 'total_ticket': total_ticket,
            # 'today_ticket': today_ticket,
            'last_week_ticket': date_list,
            'last_month_ticket': last_month,

        }
        return res

    @api.model
    def get_count_ticket_list(self):
        today = date.today()
        date_list = []
        last_month = []
        first = today.replace(day=1)
