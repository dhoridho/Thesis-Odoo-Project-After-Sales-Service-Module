...
class TopSellingProductExcel(models.Model):
    _name = "sh.top.selling.products"
    _description = "Top Selling XLS"
    ...
    def download_report(self):
        return{
            ...
            'url': 'web/content/?model=sh.top.selling.products&field=excel_file&download=true&id=%s&filename=%s' % (self.id, self.file_name),
            'target': 'new',
        }
...
class TopSellingWizard(models.TransientModel):
    ...
    def print_top_selling_product_xls_report(self):
        ...
        filename = ('Top Selling Products Xls Report' + '.xls')
        fp = BytesIO()
        workbook.save(fp)

        export_id = self.env['sh.top.selling.products'].sudo().create({
        ...
        return{
            ...
            'res_id': export_id.id,
            'res_model': 'sh.top.selling.products',
            'view_type': 'form',
            'view_mode': 'form',
            'target': 'new',
        }
