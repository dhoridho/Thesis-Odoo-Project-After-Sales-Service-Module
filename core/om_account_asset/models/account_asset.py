...
class AccountAssetAsset(models.Model):
    ...
    name = fields.Char(string='Asset Name', required=True, readonly=True, states={'draft': [('readonly', False)]}, tracking=True)
    code = fields.Char(string='Reference', size=32, readonly=True, states={'draft': [('readonly', False)]}, tracking=True)
    ...
    value = fields.Monetary(string='Gross Value', required=True, readonly=True, currency_field='currency_id', states={'draft': [('readonly', False)]} , tracking=True)
    ...
    note = fields.Text()
    category_id = fields.Many2one('account.asset.category', string='Category', required=True, change_default=True, readonly=True, states={'draft': [('readonly', False)]}, tracking=True)
    date = fields.Date(string='Date', required=True, readonly=True, states={'draft': [('readonly', False)]}, default=fields.Date.context_today, tracking=True)
    state = fields.Selection([('draft', 'Draft'), ('open', 'Running'),('waiting_for_approve','Waiting Approval'), ('close', 'Close'),('rejected','Reject')], 'Status', required=True, copy=False, default='draft', tracking=True,
        help="When an asset is created, the status is 'Draft'.\n"
            "If the asset is confirmed, the status goes in 'Running' and the depreciation lines can be posted in the accounting.\n"
            "You can manually close an asset when the depreciation is over. If the last line of depreciation is posted, the asset automatically goes in that status.")
    active = fields.Boolean(default=True)
    partner_id = fields.Many2one('res.partner', string='Partner', readonly=True, states={'draft': [('readonly', False)]}, tracking=True)
    method = fields.Selection([('linear', 'Linear'), ('degressive', 'Degressive')], string='Computation Method', required=True, readonly=True, states={'draft': [('readonly', False)]}, default='linear', tracking=True,
        help="Choose the method to use to compute the amount of depreciation lines.\n  * Linear: Calculated on basis of: Gross Value / Number of Depreciations\n"
            "  * Degressive: Calculated on basis of: Residual Value * Degressive Factor")
    method_number = fields.Integer(string='Number of Depreciations', readonly=True, states={'draft': [('readonly', False)]}, default=5, tracking=True, help="The number of depreciations needed to depreciate your asset")
    method_period = fields.Integer(string='Number of Months in a Period', required=True, readonly=True, default=12, states={'draft': [('readonly', False)]}, tracking=True,
        help="The amount of time between two depreciations, in months")
    method_end = fields.Date(string='Ending Date', readonly=True, states={'draft': [('readonly', False)]})
    method_progress_factor = fields.Float(string='Degressive Factor', readonly=True, default=0.3, states={'draft': [('readonly', False)]}, tracking=True)
    ...
    method_time = fields.Selection([('number', 'Number of Entries'), ('end', 'Ending Date')], string='Time Method', required=True, readonly=True, default='number', states={'draft': [('readonly', False)]},
        help="Choose the method to use to compute the dates and number of entries.\n"
             "  * Number of Entries: Fix the number of entries and the time between 2 depreciations.\n"
             "  * Ending Date: Choose the time between 2 depreciations and the date the depreciations won't go beyond.")
    prorata = fields.Boolean(string='Prorata Temporis', readonly=True, states={'draft': [('readonly', False)]}, tracking=True,
    ...
    depreciation_line_ids = fields.One2many('account.asset.depreciation.line', 'asset_id', string='Depreciation Lines', readonly=True, states={'draft': [('readonly', False)], 'open': [('readonly', False)]}, tracking=True)
    salvage_value = fields.Monetary(string='Salvage Value', currency_field='currency_id', readonly=True, states={'draft': [('readonly', False)]}, tracking=True,
    ...
    account_analytic_id = fields.Many2one('account.analytic.account', string='Analytic Account')
    analytic_tag_ids = fields.Many2many('account.analytic.tag', string='Analytic Tag', tracking=True)
    date_first_depreciation = fields.Selection([
        ('last_day_period', 'Based on Last Day of Purchase Period'),
        ('manual', 'Manual')],
    ...
    def compute_generated_entries(self, date, asset_type=None):
        ...
        for ungroup_asset in ungrouped_assets:
            created_move_ids += ungroup_asset._compute_entries(date, group_entries=False)
    ...
    def compute_depreciation_board(self):
        ...
        if self.value_residual != 0.0:
            ...
            for x in range(len(posted_depreciation_line_ids), undone_dotation_number):
                sequence = x + 1
                amount = self._compute_board_amount(sequence, residual_amount, amount_to_depr, salvage_value, undone_dotation_number, posted_depreciation_line_ids, total_days, depreciation_date)
    ...
    def _amount_residual(self):
        for rec in self:
            ...
            rec.value_residual = rec.value - total_amount
    ...
    def _compute_entries(self, date, group_entries=False):
        ...
        return depreciation_ids.create_move(post_move=False)
    ...
    def open_entries(self):
        ...
        for asset in self:
            for depreciation_line in asset.depreciation_line_ids:
                if depreciation_line.move_id:
                    ...
                    # Convert move_ids to recordsets if they are integers
        ...
        for move_record in move_records:
            for asset in self:
                for depreciation_line in asset.depreciation_line_ids:
                    if depreciation_line.move_id and depreciation_line.move_id.id == move_record.id:
                        move_record.write({'amount_untaxed': depreciation_line.amount, 'amount_total': depreciation_line.amount})
                        move_record.refresh()  # Ensure the record is refreshed after write
                        # print(f"Updated move_record ID: {move_record.id} with amount_untaxed: {move_record.amount_untaxed}")            
        ...
        return {
            ...
            'context': {'default_asset_id': self.id, 'default_amount_untaxed': amount_untaxed},
        }
