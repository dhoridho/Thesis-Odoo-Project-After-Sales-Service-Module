...
class ManufCeisaDocumentsBC261(models.Model):
    _inherit = 'ceisa.documents.bc261'
    _description = 'CEISA Documents TPB BC-261'

    @api.model
    def default_get(self, fields):
        res = super(ManufCeisaDocumentsBC261, self).default_get(fields)
    ...
    def action_ceisa_send_document_bc261(self):
        self.check_required_fields()
        res_import = self._send_document_bc261('import_document')
        return res_import

    def _send_document_bc261(self, path):
        ...
        if username and password:
            ...
            self._api_ceisa_document_bc261(user_token)
        else:
    ...
    def _api_ceisa_document_bc261(self, user_token=False):
        ...
        payload = {
            ...
            ### tidak ada penjelasan di portal refrensi
            "tempatStuffing": '',
            "tglAkhirBerlaku": datetime.datetime.now().strftime('%Y-%m-%d'),
            "tglAwalBerlaku": datetime.datetime.now().strftime('%Y-%m-%d'),
            "totalDanaSawit": 0,
        ...
        if 'errors' in response_data_auth:
            ...
            if 'Token' in response_data_auth['Exception']:
                ...
                if new_token:
                    return self._api_ceisa_document_bc261(new_token)
                else:
    ...
    def check_required_fields(self):
        ...
        if not self.recipient_identity_type:
            raise ValidationError('Error! Jenis Identitas Penerima is not found.')
