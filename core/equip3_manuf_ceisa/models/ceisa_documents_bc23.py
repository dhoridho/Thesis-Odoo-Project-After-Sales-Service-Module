...
class ManufCeisaDocumentsBC23(models.Model):
    _inherit = 'ceisa.documents.bc23'
    _description = 'CEISA Documents TPB BC-23'

    @api.model
    def default_get(self, fields):
        res = super(ManufCeisaDocumentsBC23, self).default_get(fields)
    ...
    def action_ceisa_send_document_bc23(self):
        self.check_required_fields()
        res_import = self._send_document_bc23('document_bc23')
        return res_import

    def _send_document_bc23(self, path):
        ...
        if username and password:
            ...
            self._api_ceisa_document_bc23(user_token)
        else:
    ...
    def _api_ceisa_document_bc23(self, user_token=False):
        ...
        payload = {
            ...
            "kodeAsuransi": self.insurance_type if self.insurance_type else '',
            "kodeIncoterm": self.cara_penyerahan.code if self.cara_penyerahan else '',
            "hargaPenyerahan": self.incoterm_value,
            "kodePelMuat": self.origin_port_office.code if self.origin_port_office else '',
            ...
            'kodeKenaPajak': self.tax_code if self.tax_code else '',
            'kodeTujuanTpb': self.destination_tpb_type_id.code if self.destination_tpb_type_id else '',
            'kodeKantorBongkar': self.destination_unloading_office.code if self.destination_unloading_office else '',
        }
        ...
        if 'errors' in response_data_auth:
            ...
            if 'Token' in response_data_auth['Exception']:
                ...
                if new_token:
                    return self._api_ceisa_document_bc23(new_token)
                else:
    ...
    def _get_entitas(self):
        ...
        ### PEMASOK / SUPPLIER
        entitases.append({
            "alamatEntitas": self.supplier_address if self.supplier_address else '',
            "kodeEntitas": '5',
            "kodeNegara": self.supplier_country.code if self.supplier_country else '',
            "namaEntitas": self.supplier_partner_id.display_name if self.supplier_partner_id else '',
            "seriEntitas": 2
        })
        ### PEMILIK
        entitases.append({
            ...
            "nomorIjinEntitas": self.owner_permit_number if self.owner_permit_number else '',
            "tanggalIjinEntitas": self.owner_permit_date.strftime('%Y-%m-%d') if self.owner_permit_date else '',
            "seriEntitas": 3
        })
        inti = 4
        ...
        if username and password:
            if self.valuta_id:
                ...
                if 'Exception' in data_kurs:
                    exceptor = data_kurs['Exception'].strip().split()
    ...
    def check_required_fields(self):
        ...
        if not self.exim_permit_number:
            raise ValidationError('Error! Nomor Izin Entitas Pengusaha is not found.')
        if not self.exim_permit_date:
            raise ValidationError('Error! Tanggal Izin Entitas Pengusaha is not found.')
        # if not self.exim_country:
        #     raise ValidationError('Error! Negara Pengusaha is not found.')
        ...
        if not self.owner_permit_number:
            raise ValidationError('Error! Nomor Izin Entitas Pemilik is not found.')
        if not self.owner_permit_date:
            raise ValidationError('Error! Tanggal Izin Entitas Pemilik is not found.')
        if not self.owner_status_code:
            raise ValidationError('Error! Status Kode Pemilik is not found.')
        if not self.owner_codeapi:
            raise ValidationError('Error! Jenis API Pemilik is not found.')
        ...
        if not self.supplier_partner_id:
            raise ValidationError('Error! Nama Pemasok is not found.')
        if not self.supplier_address:
            raise ValidationError('Error! Alamat Pemasok is not found.')
        if not self.supplier_country:
            raise ValidationError('Error! Negara Pemasok is not found.')
