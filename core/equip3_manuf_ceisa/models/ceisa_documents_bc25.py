...
class ManufCeisaDocumentsBC25(models.Model):
    _inherit = 'ceisa.documents.bc25'
    _description = 'CEISA Documents TPB BC-25'

    @api.model
    def default_get(self, fields):
        res = super(ManufCeisaDocumentsBC25, self).default_get(fields)
    ...
    def action_ceisa_send_document_bc25(self):
        self.check_required_fields()
        res_import = self._send_document_bc25('import_document')
        return res_import

    def _send_document_bc25(self, path):
        ...
        if username and password:
            ...
            self._api_ceisa_document_bc25(user_token)
        else:
    ...
    def _api_ceisa_document_bc25(self, user_token=False):
        ...
        payload = {
            ...
            'kodeTujuanPengiriman': self.reason_delivery.code if self.reason_delivery else '',
            'volume': self.volume,
        ...
        if 'errors' in response_data_auth:
            ...
            if 'Token' in response_data_auth['Exception']:
                ...
                if new_token:
                    return self._api_ceisa_document_bc25(new_token)
                else:
    ...
    def _get_entitas(self):
        ...
        entitases.append({
            ...
            "kodeEntitas": '3',
            "kodeJenisApi": self.exim_code_api if self.exim_code_api else '',
        ...
        entitases.append({
            ...
            "niperEntitas": self.recipient_niper if self.recipient_niper else '',
    ...
    def check_required_fields(self):
        ...
        if not self.exim_status_code:
            raise ValidationError('Error! Status Kode Pengusaha is not found.')
        if not self.exim_code_api:
            raise ValidationError('Error! Jenis API Pengusaha is not found.')
        if not self.exim_nib_entitas:
        ...
        if not self.recipient_niper:
            raise ValidationError('Error! NIPER Penerima is not found.')
