...
class ManufCeisaDocumentsBC40(models.Model):
    _inherit = 'ceisa.documents.bc40'
    _description = 'CEISA Documents TPB BC-40'

    @api.model
    def default_get(self, fields):
        res = super(ManufCeisaDocumentsBC40, self).default_get(fields)
    ...
    def action_ceisa_send_document_bc40(self):
        self.check_required_fields()
        res_import = self._send_document_bc40('import_document')
        return res_import

    def _send_document_bc40(self, path):
        ...
        if username and password:
            ...
            self._api_ceisa_document_bc40(user_token)
        else:
    ...
    def _api_ceisa_document_bc40(self, user_token=False):
        ...
        payload = {
            ...
            'kodeTujuanPengiriman': self.trade_purpose.code if self.trade_purpose else '',
            'nik': self.nik,
            'userPortal': username if username else '',
            'volume': self.volume,
        ...
        if 'errors' in response_data_auth:
            ...
            if 'Token' in response_data_auth['Exception']:
                ...
                if new_token:
                    return self._api_ceisa_document_bc40(new_token)
                else:
    ...
    def _get_entitas(self):
        ...
        entitases.append({
            ...
            "kodeEntitas": '7',
            "kodeJenisApi": self.owner_code_api if self.owner_code_api else '',
            ...
            "nibEntitas": self.owner_nib_entitas if self.owner_nib_entitas else '',
        ...
        entitases.append({
            ...
            "kodeEntitas": '9',
            "kodeJenisApi": self.sender_code_api if self.sender_code_api else '',
            ...
            "nibEntitas": self.sender_nib_entitas if self.sender_nib_entitas else '',
    ...
    def check_required_fields(self):
        ...
        if not self.exim_nib_entitas:
            raise ValidationError('Error! Nomor NIB Pengusaha is not found.')
        ...
        if not self.owner_code_api:
            raise ValidationError('Error! Nomor API Pemilik is not found.')
        ...
        if not self.sender_code_api:
            raise ValidationError('Error! Nomor API Pengirim is not found.')
        if not self.sender_nib_entitas:
            raise ValidationError('Error! Nomor NIB Pengirim is not found.')
