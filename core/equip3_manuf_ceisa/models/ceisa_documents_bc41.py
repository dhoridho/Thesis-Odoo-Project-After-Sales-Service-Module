...
class ManufCeisaDocumentsBC41(models.Model):
    _inherit = 'ceisa.documents.bc41'
    _description = 'CEISA Documents TPB BC-41'

    @api.model
    def default_get(self, fields):
        res = super(ManufCeisaDocumentsBC41, self).default_get(fields)
    ...
    def action_ceisa_send_document_bc41(self):
        self.check_required_fields()
        res_import = self._send_document_bc41('import_document')
        return res_import

    def _send_document_bc41(self, path):
        ...
        if username and password:
            ...
            self._api_ceisa_document_bc41(user_token)
        else:
    ...
    def _api_ceisa_document_bc41(self, user_token=False):
        ...
        payload = {
            ...
            'biayaPengurang': self.deduction_cost if self.deduction_cost else 0.01,
            'biayaTambahan': self.additional_cost if self.additional_cost else 0.01,
            'vd': self.valuntary_value if self.valuntary_value else 0,
            'kodeLokasiBayar': self.payment_location if self.payment_location else '',
        ...
        if 'errors' in response_data_auth:
            ...
            if 'Token' in response_data_auth['Exception']:
                ...
                if new_token:
                    return self._api_ceisa_document_bc41(new_token)
                else:
    ...
    def check_required_fields(self):
        ...
        if not self.owner_permit_number:
            raise ValidationError('Error! Nomor Izin Pemilik is not found.')
        if not self.owner_permit_date:
            raise ValidationError('Error! Tanggal Izin Pemilik is not found.')
