...
class ManufCeisaDocumentsBC262(models.Model):
    _inherit = 'ceisa.documents.bc262'
    _description = 'CEISA Documents TPB BC-262'

    @api.model
    def default_get(self, fields):
        res = super(ManufCeisaDocumentsBC262, self).default_get(fields)
    ...
    def action_ceisa_send_document_bc262(self):
        self.check_required_fields()
        res_import = self._send_document_bc262('import_document')
        return res_import

    def _send_document_bc262(self, path):
        ...
        if username and password:
            ...
            self._api_ceisa_document_bc262(user_token)
        else:
    ...
    def _api_ceisa_document_bc262(self, user_token=False):
        ...
        payload = {
            ...
            'kodeTujuanPemasukan': self.trade_purpose.code if self.trade_purpose else '',
            ...
            ### hanya ada di bc261 dan bc262
            "jaminan": self._get_guarantee_line(),
        ...
        if 'errors' in response_data_auth:
            ...
            if 'Token' in response_data_auth['Exception']:
                ...
                if new_token:
                    return self._api_ceisa_document_bc262(new_token)
                else:
    ...
    def _get_entitas(self):
        ...
        entitases.append({
            ...
            "kodeEntitas": '9',
            "kodeJenisIdentitas": self.sender_identity_type if self.sender_identity_type else '',
            "kodeStatus": self.sender_status_code.code if self.sender_status_code else '',
            "namaEntitas": self.sender_partner_id.display_name if self.sender_partner_id else '',
            "nomorIdentitas": self.sender_identity_number if self.sender_identity_number else '',
            "seriEntitas": 3,
        })
    ...
    def _get_guarantee_line(self):
        guarantee = []
        iguar = 1
        if self.guarantee_line_id:
            for warr in self.guarantee_line_id:
                guarantee.append({
                     "idJaminan": iguar,
                     "nomorBpj": warr.no_bpj if warr.no_bpj else '',
                     "tanggalBpj": warr.date_bpj.strftime('%Y-%m-%d') if warr.date_bpj else '',
                     "kodeJenisJaminan": warr.guarantee_type.code if warr.guarantee_type else '',
                     "nomorJaminan": warr.guarantee_no if warr.guarantee_no else '',
                     "tanggalJaminan": warr.guarantee_date.strftime('%Y-%m-%d') if warr.guarantee_date else '',
                     "tanggalJatuhTempo": warr.guarantee_limit_date.strftime('%Y-%m-%d') if warr.guarantee_limit_date else '',
                     "penjamin": warr.name if warr.name else '',
                     "nilaiJaminan": warr.guarantee_value if warr.guarantee_value else 0.01
                })
                iguar += 1
        return guarantee
    ...
    def check_required_fields(self):
        ...
        if not self.exim_codeapi:
            raise ValidationError('Error! Kode API Pengusaha is not found.')
        if not self.exim_status_code:
            raise ValidationError('Error! Kode Status Pengusaha is not found.')
        if not self.exim_nib_entitas:
            raise ValidationError('Error! NIB Pengusaha is not found.')
        ...
        if not self.owner_status_code:
            raise ValidationError('Error! Kode Status Pemilik is not found.')
        # if not self.owner_nib_entitas:
        #     raise ValidationError('Error! Nomor NIB Pemilik is not found.')
        ...
        if not self.sender_identity_type:
            raise ValidationError('Error! Jenis Identitas Pengirim is not found.')
        if not self.sender_identity_number:
            raise ValidationError('Error! Nomor Identitas Pengirim is not found.')
        if not self.sender_status_code:
            raise ValidationError('Error! Kode Status Pengirim is not found.')
