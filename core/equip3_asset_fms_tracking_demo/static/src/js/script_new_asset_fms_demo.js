odoo.define('equip3_asset_fms_tracking_demo.extension_tracking_demo', function(require){
    "use strict";
    ...
    let centerPoint = {lat: -6.200000, lon: 106.816666}; // Jakarta
    let latRange = {min: 0.1, max: 0.2};
    let lonRange = {min: 0.2, max: 0.2};

    if (centerPoint.lat < 0){
      latRange.min *= -1;
      latRange.max *= -1;
    }

    if (centerPoint.lon < 0){
      lonRange.min *= -1;
      lonRange.max *= -1;
    }
    ...
    function get_vehicle_asset(){
        ...
        ajax.jsonRpc("/asset", "call", {}).then(function(data){
            console.log('asssettt newwwwwww 321',data)
            // Load vehicle data
            let map;
            let locations = [];
            let trElement = ""
            ...
            for (let i = 0; i < _.size(data['status']); i++) {
                data['status'][i]['latitude'] = getRandomInRange(centerPoint.lat - latRange.min, centerPoint.lat + latRange.max, 6);
                data['status'][i]['longitude'] = getRandomInRange(centerPoint.lon - lonRange.min, centerPoint.lon + lonRange.max, 6);
                ...
                if(data['status'][i]['status'] == 'Parking'){
                  ...
                  s_text = `<span class="text-success">${'Moving'}</span>`
                  live_status = `<b style="color: black">${timeAgo(data['speed'][i]['last_data'])}</b>
                  `
                }
