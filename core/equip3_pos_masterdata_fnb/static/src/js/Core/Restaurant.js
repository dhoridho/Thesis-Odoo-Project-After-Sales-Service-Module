odoo.define('equip3_pos_masterdata_fnb.restaurant', function (require) {

    const floors = require('pos_restaurant.floors');
    const {Gui} = require('point_of_sale.Gui');
    const {posbus} = require('point_of_sale.utils');
    var sync = require('equip3_pos_masterdata.synchronization');
    ...
    models.PosModel = models.PosModel.extend({
        ...
        async unlock_table() {
            ...
            if (resultUnLock) {
                ...
                this.rpc({
                    model: 'restaurant.table',
                    method: 'write',
                    args: [table.id, {clear_interval: false,date_reserve: false}],
                })
        ...
        async set_table(table, order) {
            ...
            var manager_user = self.env.pos.user;
            if (table && self.env.pos.checkIsLocked(table) && manager_user.role == "manager") {
                this.table_click = table;
                let {confirmed:c, payload: p} = await this.chrome.showPopup('NumberPopup', {
                    title: self.env._t('Manager pin'),
                    isPassword: true,
                }); 
                if (c) {
                    ...
                    if (manager_security_pin.toString() !== entered_security_pin.toString()){
                        ...
                        let order_domain = [
                            ...
                            ['state','=','reserved'],
                            // ['reservation_from','>=',moment().utc().format('YYYY-MM-DD') ],
                            // ['reservation_to','<=',moment().utc().format('YYYY-MM-DD') ],
                        ];
                        await self.rpc({
                            model: 'reserve.order',
                            method: 'search_read',
                            args: [order_domain, ['id','name','state','reservation_from','reservation_from',]],
                            limit: 1,
                        }).then(function(res){
                            if(res.length){
                                self.rpc({
        ...
        play_sound: function () {
            var src = "/equip3_pos_masterdata/static/src/sounds/demonstrative.mp3";
        ...
        sync_request_printer: function (vals) { // TODO: update variable set_dirty of line
            ...
            if (this.config.screen_type && (computeChanges.new.length > 0 || computeChanges.cancelled.length > 0)) {
                if (this.config.screen_type != 'kitchen' && this.config.screen_type != 'takeaway') { // if waiters or cashiers, and order not still on Session, reject it
    ...
    models.Order = models.Order.extend({
        ...
        buildReceiptKitchen(orderReceipt) { // todo: request number and session_id of computeChanges is key unit of request print receipt
            ...
            if(order && order.take_away_order){
                if (this.take_away_order || !orderReceipt.table) { // if clicked to button take away, or pos config have not add table : it mean order is take away
                    ...
                    orderReceipt['take_away_tiket_no'] = this.pos.db.getTakeAwayTicketOrderNumber();
                }
            }
        ...
        for (line_hash in current_res) {
            ...
            if (typeof old === 'undefined') {
                add.push({
                    ...
                    'is_default_timer_active': 0,
                    'default_time_active_time':'',
                    'categ':product.categ,
                    'note': curr.note,
                    'qty': curr.qty,
                    'uom': curr.uom,
                    ...
                    'bom_components': curr.bom_components,
                    'pos_combo_options': curr.pos_combo_options,
                });
        ...
        for (line_hash in old_res) {
            if (typeof current_res[line_hash] === 'undefined') {
                ...
                rem.push({
                    ...
                    'pos_combo_options': old.pos_combo_options,
                });
            }
        }
        ...
        this.orderlines.each(function (line) {
            ...
            if (typeof resume[line_hash] === 'undefined') {
                ...
                if (line['bom_components']) {
                    resume[line_hash]['bom_components'] = line['bom_components'];
                }
                if (line['pos_combo_options']) {
                    resume[line_hash]['pos_combo_options'] = line['pos_combo_options'];
                }
                if (line['variants']) {
    ...
    _super_order.build_line_resume_for_fire_dining = function (categ_ids) {
        // todo:  receipt kitchen line print
        var resume = {};
        var self = this;
        this.orderlines.each(function (line) {
            ...
            let pos_categ_name = product.pos_categ_id[0];
