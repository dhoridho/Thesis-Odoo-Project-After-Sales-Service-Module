...
class SaleOrder(models.Model):
    ...
    def prepare_shopify_customer_and_addresses(self, order_response, pos_order, instance, order_data_line, log_book):
        ...
        invoice_address = order_response.get(
            ...
            order_response.get("billing_address"), order_response.get("email"), partner, "invoice") or partner

        delivery_address = order_response.get(
            "shipping_address") and shopify_res_partner_obj.shopify_create_or_update_address(
            order_response.get("shipping_address"), order_response.get("email"), partner, "delivery") or partner
    ...
    def import_shopify_orders(self, order_data_lines, log_book, is_queue_line=True):
        ...
        for order_data_line in order_data_lines:
            ...
            _logger.info("Done auto workflow process for Odoo order(%s) and Shopify order is (%s)", sale_order.name,
                         order_number)
            if message and order_data_line:
    ...
    def create_shipped_order_refund(self, shopify_financial_status, order_response, sale_order, created_by):
        """ This method is used to create partially or fully refund in shopify order.
            ...
            @return: message
            @author: Meera Sidapara @Emipro Technologies Pvt. Ltd on date 17 December 2021 .
            Task_id: 181380
        """
        message = ""
    ...
    def prepare_vals_shopify_multi_payment_refund(self, order_refunds, order):
        """ This method is used to manage multi payment wise remaining refund amount.
            ...
            @return: True
            @author: Meera Sidapara @Emipro Technologies Pvt. Ltd on date 10 Feb 2022.
            Task_id: 183797
        """
    ...
    def prepare_vals_shopify_multi_payment(self, instance, order_data_queue_line, order_response, log_book_id,
        ...
        """ This method is use to prepare a values for the multi payment.
            @author: Haresh Mori @Emipro Technologies Pvt. Ltd on date 7 September 2021 .
            Task_id:165529 - Multiple shopify payment gateway management for single order
        """
        payment_gateway_obj = self.env["shopify.payment.gateway.ept"]
    ...
    def search_shopify_location_for_update_order_status(self, sale_order, instance, log_book):
        ...
        if not shopify_location_id:
            shopify_location_id = shopify_location_obj.search(
                [("is_primary_location", "=", True), ("warehouse_for_order", "=", sale_order.warehouse_id.id),
                 ("instance_id", "=", instance.id)])
            if not shopify_location_id:
    ...
    def create_shopify_refund(self, refunds_data, total_refund, created_by=""):
        ...
        if self.amount_total == total_refund:
            move_reversal = self.env["account.move.reversal"].with_context(
                ...
                {"refund_method": "refund", "reason": "Refunded from shopify"
                if len(refunds_data) > 1 else refunds_data[0].get("note"), "date": refund_date})
            move_reversal.reverse_moves()
            move_reversal.new_move_ids.message_post(body=_("Credit note is generated by %s as Order refunded in "
                                                           "Shopify.", created_by))
    ...
    def paid_invoice_ept(self, invoices):
        ...
        Changes done by Meera Sidapara on date 11/03/2022.
        """
        self.ensure_one()
