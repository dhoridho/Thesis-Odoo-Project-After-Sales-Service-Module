...
class ShopifyProductTemplateEpt(models.Model):
    ...
    def shopify_sync_products(self, product_data_line_id, shopify_tmpl_id, instance, log_book_id,
        ...
        if shopify_template:
            self.sync_product_with_existing_template(shopify_template, skip_existing_product, template_data, instance,
                                                     ...
                                                     order_data_line_id)
            if not skip_existing_product and instance.sync_product_with_images and shopify_tmpl_id:
            ...
            if shopify_template and instance.sync_product_with_images and shopify_tmpl_id:
                shopify_template.shopify_sync_product_images(template_data)
    ...
    def sync_new_product(self, template_data, instance, product_category, model_id, log_book_id, product_data_line_id,
        ...
        name = template_vals.get("template_title")
        odoo_template = False
        # check for the product tempalte exist or not
        # for variant in variant_data:
        #     sku = variant.get('sku')
        #     barcode = variant.get('barcode')
        #     if instance.shopify_sync_product_with == "sku" and sku:
        #         odoo_product = self.env['product.product'].search([('default_code','=',sku)])
        #         if odoo_product:
        #             odoo_template = odoo_product.product_tmpl_id
        #     if instance.shopify_sync_product_with == "sku" and barcode:
        #         odoo_product = self.env['product.product'].search([('barcode','=',barcode)])
        #         if odoo_product:
        #             odoo_template = odoo_product.product_tmpl_id
        #     if odoo_template:
        #         need_to_update_template = True
    ...
    def is_product_importable(self, template_data, instance, odoo_product, shopify_product):
        ...
        if len(shopify_barcodes) != total_shopify_barcodes:
            message = "Duplicate Barcode%s found in Product %s and ID: %s." % (
                tuple(set(shopify_barcodes)), template_title,
                template_id)
            return message
