...
class ProjectScopeLine(models.Model):
    ...
    name = fields.Char(string="Project Scope", tracking=True)
    ...
    def _check_existing_code(self):
        for record in self:
            code_id = self.env['project.scope.line'].search(
                [('code', '=', record.code)])
            if record.cons_use_code == True:
                if len(code_id) > 1:
                    for res in code_id:
                        raise ValidationError((_("The code already exists with project scope '{}', so please change the code.".format(res.name))))
            else:
                pass
    ...
    def _check_existing_name(self):
        for record in self:
            name_id = self.env['project.scope.line'].search(
                [('name', '=', record.name)])
            if record.cons_use_code == True:
                if len(name_id) > 1:
                    for res in name_id:
                        raise ValidationError(_("The name of this project scope already exists with code '{}', so please change the name.".format(res.code)))
            else:
                if len(name_id) > 1:
                    for res in name_id:
                        raise ValidationError(_("The name of this project scope already exists, so please change the name."))
