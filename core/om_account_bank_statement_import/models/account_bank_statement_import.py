...
class AccountBankStatement(models.Model):
    ...
    def _compute_ending_balance(self):
        ...
        for statement in self:
            ...
            statement.balance_end_real = statement.balance_start + statement.total_entry_encoding
...
class AccountBankStatementImport(models.TransientModel):
    ...
    sample_file = fields.Binary(string='Sample File', readonly=True)

    def download_sample_template(self):
        base_url = self.env['ir.config_parameter'].sudo().get_param('web.base.url')
        # url = base_url + '/om_account_bank_statement_import/static/src/xlsx/bank_statement_import_template.xlsx'
        url_server = os.path.join(base_url, 'om_account_bank_statement_import/static/src/xlsx/bank_statement_import_template.xlsx?download=true')
        # url_server_new = base_url + '/om_account_bank_statement_import/static/src/xlsx/bank_statement_import_template.xlsx?download=true'
    ...
    def _generate_account_xlsx(self):
        output = io.BytesIO()
        workbook = Workbook()
        worksheet = workbook.active
        worksheet.title = 'Statement Import'

        # Set column widths
        column_widths = [12, 20, 30, 20, 12, 10, 20]
        ...
        # Write column headers with specified styles
        headers = ['Date', 'Label/Name', 'Ref', 'Partner', 'Balance', 'Currency', 'Branch']
        ...
        # Sample data
        data = [
            ['2023-07-10', 'Example 01', 'Reference Note A', 'Partner A', 24000, 'USD', 'Branch A'],
            ['2023-07-10', 'Example 02', 'Reference Note B', 'Partner B', -24000, 'IDR', 'Branch A'],
        ]
        ...
        attachment = self.env['ir.attachment'].create({
            'name': 'Statement Import Template.xlsx',
            'datas': out,
            'type': 'binary'
        })
        return attachment
    
    def _generate_account_xls(self):
        output = io.BytesIO()
        workbook = xlwt.Workbook()
        worksheet = workbook.add_sheet('Statement Import')
        
        # Set column widths (in units of 1/256 of the width of the character '0')
        column_widths = [12 * 256, 20 * 256, 30 * 256, 20 * 256, 12 * 256, 10 * 256, 20 * 256]
        ...
        for col_index, header in enumerate(headers):
            worksheet.write(0, col_index, header, bold_style)
        ...
        for row_index, row in enumerate(data, start=1):
            for col_index, value in enumerate(row):
                if col_index == 4:  # Assuming column 4 is for balance
                    if value > 0:
        ...
        out = base64.b64encode(my_utf8)
        attachment = self.env['ir.attachment'].create({
            'name': 'Statement Import Template.xls',
            'datas': out,
            'type': 'binary'
        })
        return attachment
    
    def download_sample_template_xls(self):
        attachement = self._generate_account_xls()
        return {
            'name': _('Statement Import Template'),
            'type': 'ir.actions.act_url',
            'url': '/web/content/%s?download=true' % (attachement.id),
            'target': 'self',
        }

    def download_sample_template_xlsx(self):
        attachement = self._generate_account_xlsx()
        return {
            'name': _('Statement Import Template'),
            'type': 'ir.actions.act_url',
            'url': '/web/content/%s?download=true' % (attachement.id),
    ...
    def get_currency(self, value):
        ...
        return currency if currency else False

    def get_branch(self, value):
        branch = self.env['res.branch'].search([('name', '=', value)])
                ...
                for i in range(len(file_data)):
                    if i == 0:
                        ...
                        values.update({
                            ...
                            'amount': field[4],
                            'currency_id':  self.get_currency(field[5]).id
                        })
                ...
                for row_no in range(sheet.nrows):
                    ...
                    if row_no <= 0:
                        ...
                        values.update({
                            ...
                            'amount': line[4],
                            'currency_id': self.get_currency(line[5]).id
                        })
                        vals_list.append((0, 0, values))
                statement_vals = {
                    'name': 'Statement Of ' + str(datetime.today().date()),
    ...
    def import_file(self):
        for data_file in self.attachment_ids:
            ...
            if file_name.strip().endswith('.csv') or file_name.strip().endswith('.xls') or file_name.strip().endswith('.xlsx'):
                statement = False
                ...
                if file_name.strip().endswith('.csv'):
                    ...
                    for i in range(len(file_reader)):
                        ...
                        if values:
                            if i == 0:
                                ...
                                if not date:
                                    date = field[0]
                                branch_id = self.get_branch(field[6])
                                values.update({
                                    'date': field[0],
                                    'payment_ref': field[1],
                                    'ref': field[2],
                    ...
                    for row_no in range(sheet.nrows):
                        ...
                        if row_no <= 0:
                            ...
                            line = list(map(
                                lambda row: isinstance(row.value, bytes) and row.value.encode('utf-8') or str(
                                    row.value), sheet.row(row_no)))
                            branch_id = self.get_branch(line[6])
                            values.update({
                                'date': line[0],
                                'payment_ref': line[1],
                                'ref': line[2],
                            ...
                            currency = self.get_currency(line[5])
                            if currency.id == journal_id.company_id.currency_id.id:
                                values.update({
                                    'amount': line[4],
                                })
                            else:
                                values.update({
                                    ...
                                    'amount': currency._convert(float(line[4]), journal_id.company_id.currency_id, journal_id.company_id, line[0] or fields.Date.context_today(self))
                                })
                    ...
                    if len(vals_list) != 0:
                        try:
                            # Assuming create_statement method creates account.move.line records
                            # Handle any exceptions that might occur during record creation
                            statement = self.create_statement(statement_vals)
                        except Exception as e:
                            raise ValidationError(_("Error creating account.move.line records: %s" % str(e)))
