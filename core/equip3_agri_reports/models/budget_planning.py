...
class BudgetPlanning(models.Model):
    _inherit = 'agriculture.budget.planning'

    def action_print_pdf(self):
        return self.env.ref('equip3_agri_reports.action_print_agriculture_budget_planning_report').report_action(self)

    def action_print_xlsx(self):
        file_name = 'Activity Budget Planning Report.xlsx'
        ...
        row = 1
        worksheet.write(row, 0, '%s: %s' % (self.env.company.display_name, _('Activity Budget Planning Report')), title_format)

        width = [2 for i in range(14)]
        for record in self:
            row += 2
            ...
            row += 2
            for i, head in enumerate(['ACTIVITY'] + [m.upper() for m in MONTH_NAMES] + ['TOTAL']):
                worksheet.write(row, i, head, header_format)
                width[i] = max([width[i], len(str(head)) + 2])

            row += 1
            for month in record.month_ids:
                for i, key in enumerate(['activity_id'] + ['val_' + m[:3] for m in MONTH_NAMES] + ['total']):
                    if key == 'activity_id':
                        value = month.activity_id.display_name
                    else:
            ...
            for i, key in enumerate(['total'] + ['val_' + m[:3] for m in MONTH_NAMES] + ['total']):
                if i == 0:
                    value = 'Total'
                else:
                    value = '%s %s' % (currency_id.symbol, format_float(sum(record.month_ids.mapped(key)), 2))
                worksheet.write(row, i, value, bold_bordered)
                width[i] = max([width[i], len(str(value)) + 2])
