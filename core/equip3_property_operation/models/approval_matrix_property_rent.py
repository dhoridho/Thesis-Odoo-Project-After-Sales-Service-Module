...
class ApprovalMatrixPropertyRent(models.Model):
    _name = 'approval.matrix.property.rent'
    ...
    approval_matrix_property_rent_line_ids = fields.One2many('approval.matrix.property.rent.line', 'approval_matrix_property_rent_id', string='Approval')
    
    @api.model
    def create(self, vals):
        res = super(ApprovalMatrixPropertyRent, self).create(vals)
        res._check_approval_matrix_line()
        return res

    def write(self, vals):
        res = super(ApprovalMatrixPropertyRent, self).write(vals)
    ...
    def _check_approval_matrix_line(self):
        for line in self.approval_matrix_property_rent_line_ids:
...
class ApprovalMatrixPropertyRentLines(models.Model):
    _name = 'approval.matrix.property.rent.line'
    _description = "Approval Matrix Property Sale"

    @api.model
    def default_get(self, fields):
        res = super(ApprovalMatrixPropertyRentLines, self).default_get(fields)
        if self._context:
            context_keys = self._context.keys()
            next_sequence = 0
            if 'approval_matrix_property_rent_line_ids' in context_keys:
                if len(self._context.get('approval_matrix_property_rent_line_ids')) > 0:
                    next_sequence = len(self._context.get('approval_matrix_property_rent_line_ids')) + 1
            res.update({'sequence': next_sequence})
        return res

    approval_matrix_property_rent_id = fields.Many2one('approval.matrix.property.rent')
