/*!
 * Chart.Funnel.Equal.js
 * A funnel plugin for Chart.js(http://chartjs.org/)
 * Version: 1.0.0
 *
 * Copyright 2023 Rajib Deyana
 */
...
module.exports = function(Chart) {
	...
	Chart.controllers.funnel = Chart.DatasetController.extend({
		...
		update: function update(reset) {
			...
			var n = elements.filter(el => !el.hidden).length;

			let i = 0;
			helpers.each(elements, function (trapezium, index) {
				me.updateElement(trapezium, index, reset, n, i);
				if (!trapezium.hidden){
					i += 1;
				}
			}, me);
		...
		// update elements
		updateElement: function updateElement(trapezium, index, reset, n, i) {
			var me = this,
				chart = me.chart,
				chartArea = chart.chartArea,
				opts = chart.options,
...
module.exports = function(Chart) {
	...
	Chart.elements.Trapezium = Chart.Element.extend({
		getCorners: function () {
			var vm = this._view;
			var corners = [],
				upper = vm.upperWidth > vm.bottomWidth ? vm.upperWidth : vm.bottomWidth,
				bottom = vm.bottomWidth < vm.upperWidth ? vm.bottomWidth : vm.upperWidth,
				i = vm.index,
				t = vm.t,
				m = vm.h / vm.n,
				o = (upper - bottom) / (vm.n * 2),
				g = vm.gap / 2,
				p = i * o;
			...
			// | 0 3 |
			corners = [
				[p + ((vm.w - upper) / 2) + o - g, t + ((i + 1) * m) - g],
				[p + (vm.w - upper) / 2, t + (i * m) + g],
				[vm.w - ((vm.w - upper) / 2) - p, t + (i * m) + g],
				[(vm.w - ((vm.w - upper) / 2)) - o - p + g, t + ((i + 1) * m) - g]
			];
