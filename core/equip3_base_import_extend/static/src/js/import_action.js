...
var DataImport = AbstractAction.extend({
    ...
    onimport: function () {
        ...
        prom.then(function (results) {
            ...
            if (!_.any(message, function (message) {
                    return message.type === 'error' || message.type === 'schedule_import'})) {
    ...
    onresults: function (event, from, to, results) {
        ...
        var schedule_import_message = false
        var messagesSorted = _.sortBy(_(message).groupBy('message'), function (group) {
            ...
            var order = 0;
            switch (group[0].type) {
            case 'schedule_import': order = 0;schedule_import_message = group[0].message; break;
        ...
        if (schedule_import_message){
            var self = this
            self.exit()
            // this.$('.oe_import_error_report').html(
            //     QWeb.render('ImportView.schedule_import', {schedule_import_message: schedule_import_message})
            //     )
            // this.$('.oe_import_error_report').find('#schedule_import_return').on('click', function(){
            //     self.exit()
            // })
            // this.$('.oe_import_error_report').find('#schedule_import_redirect_log').on('click', function(){
            //     self.do_action('equip3_base_import_extend.import_log_action', {
