...
var FormViewDialog = ViewDialog.extend({
    ...
    init: function (parent, options) {
        ...
        if (!options.buttons) {
            if (options.res_model == 'res.users.image') {
                options.buttons = [{
                    text: _t("Close"),
                    classes: "btn-secondary o_form_button_cancel btn-close-face-recognition",
                    close: true,
                    click: function () {
                        if (!readonly) {
            ...
            if (!readonly) {
                options.buttons.unshift({
                    ...
                    classes: "btn-primary btn-save-and-close",
                    click: function () {
                    ...
                    options.buttons.splice(1, 0, {
                        ...
                        click: function () {
                            ...
                            var json_data = $.parseJSON($.ajax({
                                ...
                                data: { 
                                    "user_id": options.context.default_res_user_id,
                                },
                                async: false
                            }).responseText);

                            var res_users_image_amount = json_data.res_users_image_amount;
                            var amount_of_add_face_descriptor = json_data.amount_of_add_face_descriptor;

                            if (res_users_image_amount < amount_of_add_face_descriptor && options.res_id == undefined) {
                                $('.btn-close-face-recognition').css('display','none');
                            } else {
                                $('.btn-close-face-recognition').css('display','initial');
                            }
                        },
                    });
                }
