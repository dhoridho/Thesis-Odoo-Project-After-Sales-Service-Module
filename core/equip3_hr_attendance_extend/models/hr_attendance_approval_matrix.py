...
class AttendanceApprovalMatrix(models.Model):
    _name = 'hr.attendance.approval.matrix'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _description = "Attendance Approval Matrix"
    ...
    approval_matrix_ids = fields.One2many('hr.attendance.approval.matrix.line', 'approval_matrix_id')
    ...
    def search_read(self, domain=None, fields=None, offset=0, limit=None, order=None):
        ...
        result = super(AttendanceApprovalMatrix, self).search_read(domain=domain, fields=fields, offset=offset, limit=limit, order=order)
        return result
    ...
    def read_group(self, domain, fields, groupby, offset=0, limit=None, orderby=False, lazy=True):
        ...
        return super(AttendanceApprovalMatrix, self).read_group(domain, fields, groupby, offset=offset, limit=limit, orderby=orderby, lazy=lazy)
    
    @api.model
    def fields_view_get(self, view_id=None, view_type=None,
                        toolbar=True, submenu=True):
        res = super(AttendanceApprovalMatrix, self).fields_view_get(
...
class AttendanceApprovalMatrixline(models.Model):
    _name = 'hr.attendance.approval.matrix.line'

    @api.model
    def _multi_company_domain(self):
        return [('company_id','=', self.env.company.id)]
    
    approval_matrix_id = fields.Many2one('hr.attendance.approval.matrix')
    ...
    def default_get(self, fields):
        res = super(AttendanceApprovalMatrixline, self).default_get(fields)
        if self.env.context:
