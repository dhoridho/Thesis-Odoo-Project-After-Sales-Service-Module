        ...
        for rec in self:
            for picking in rec.picking_ids:
                if picking.state == 'done':
                    raise exceptions.ValidationError("You cannot cancel the purchase order that receiving notes already received")
            if rec.invoice_status != 'no':
                inv_not_cancel = self.invoice_ids.filtered(lambda p:p.state != 'cancel')
                if inv_not_cancel:
                    raise exceptions.ValidationError("You need to cancel vendor bills before canceling the purchase order")
                else:
                    self.action_purch_cancel_1()
            else:
                self.action_purch_cancel_1()

    def action_purch_cancel_1(self):
        for rec in self:
        ...
        for rec in self:
            ...
            if rec.invoice_status != 'no':
                ...
                if inv_not_cancel:
                    ...
                    self.action_purch_cancel_2()
            else:
                self.action_purch_cancel_2()

    def action_purch_cancel_2(self):
        for rec in self:
            if self._check_stock_installed():
                if rec.sudo().mapped('picking_ids'):
                    if rec.sudo().mapped('picking_ids').sudo().mapped('move_ids_without_package'):
    ...
    def sh_cancel(self):
        ...
        if self.invoice_status != 'no':
            ...
            if inv_not_cancel:
                ...
                self.action_purch_cancel_3()
        else:
            self.action_purch_cancel_3()

    def action_purch_cancel_3(self):
        if self._check_stock_installed():
