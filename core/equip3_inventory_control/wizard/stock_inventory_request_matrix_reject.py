...
class StockCountMatrixReject(models.TransientModel):
    _name = "stock.inventory.request.matrix.reject"
    _description = 'Stock Inventory Matrix Reject'

    reason = fields.Text(string="Reason")

    def action_reject(self):
        inv_id = self.env['stock.inventory'].browse([self._context.get('active_id')])
        user = self.env.user
        approving_matrix_line = sorted(inv_id.approved_matrix_ids.filtered(lambda r:not r.approved), key=lambda r:r.sequence)
