...
class StockCountMatrixReject(models.TransientModel):
    _name = "stock.scrap.request.matrix.reject"
    _description = 'Stock scrap Matrix Reject'

    reason = fields.Text(string="Reason")

    def action_reject(self):
        sc_req_id = self.env['stock.scrap.request'].browse([self._context.get('active_id')])
        user = self.env.user
        approving_matrix_line = sorted(sc_req_id.approved_matrix_ids.filtered(lambda r:not r.approved), key=lambda r:r.sequence)
