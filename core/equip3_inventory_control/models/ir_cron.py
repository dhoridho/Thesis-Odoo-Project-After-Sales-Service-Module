...
class IrCron(models.Model):
    ...
    scrap_log_line_id = fields.Many2one('stock.scrap.request.log.line', string='Product Usage Log Line')
    ...
    def _handle_callback_exception(self, cron_name, server_action_id, job_id, job_exception):
        ...
        if cron.scrap_log_line_id and cron.scrap_log_line_id.state != 'done':
            info = sys.exc_info()
            formatted_info = "".join(traceback.format_exception(*info))
            cron.scrap_log_line_id.write({
                'state': 'failed',
    ...
    def method_direct_trigger(self):
        stock_crons = self.filtered(lambda o: o.scrap_log_line_id)
    ...
    def method_direct_trigger_stock_count_log(self):
        if len(self) > 1:
            raise UserError(_('Product usage cron can only be executed once at a time!'))

        if not self.active:
            raise UserError(_('Cannot run executed product usage cron!'))
    ...
    def _callback(self, cron_name, server_action_id, job_id):
        cron = self.browse(job_id)
        self = self.with_context(trigger_type='scheduler', trigger_log_line_id=cron.scrap_log_line_id.id)
