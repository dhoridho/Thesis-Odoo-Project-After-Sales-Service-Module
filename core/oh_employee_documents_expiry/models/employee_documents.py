...
class HrEmployeeDocument(models.Model):
    ...
    def fields_view_get(self, view_id=None, view_type=None,
        ...
        res = super(HrEmployeeDocument, self).fields_view_get(
    ...
    def custom_menu(self):
        views = [(self.env.ref('oh_employee_documents_expiry.employee_documents_tree_view').id,'tree'),
                 (self.env.ref('oh_employee_documents_expiry.employee_document_form_view').id,'form')]
        if  self.env.user.has_group('hr.group_hr_user') and not self.env.user.has_group('equip3_hr_employee_access_right_setting.group_hr_officer'):
            return {
                ...
                'domain': [('employee_ref.user_id', '=', self.env.user.id)]
            }
        else:
            return {
                'type': 'ir.actions.act_window',
    ...
    @api.constrains('doc_attachment_id')
    def _constrain_file_size(self):
        for rec in self:
            for doc in rec.doc_attachment_id:
                config_file_size = int(self.env['ir.config_parameter'].sudo().get_param(
                    'oh_employee_documents_expiry.file_size', 5))
                max_upload_size = config_file_size * 1024 * 1024
                if doc.file_size > max_upload_size:
                    raise ValidationError(_('File(s) is bigger than %s Mb.') % config_file_size)
