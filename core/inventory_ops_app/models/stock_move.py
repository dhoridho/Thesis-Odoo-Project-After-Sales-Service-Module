...
class StockMove(models.Model):
    ...
    def _compute_temp_barcode(self):
        for record in self:
            if record.product_id.multi_barcode:
                barcode_id = self.env['product.template.barcode'].search([('product_id', '=', record.product_id.id),('uom_id', '=', record.product_uom.id)], limit=1)
    ...
    def unlink(self):
        ...
        if ids_to_delete:
            query = '''
                SELECT 
                  ru.id AS user_id,
                  COALESCE(ru.picking_line_unlink_data, '') AS picking_line_unlink_data
                FROM
                  res_users as ru
                WHERE 
            ...
            for record in users_data:
                ...
                old_value = record.get('picking_line_unlink_data')
                if old_value:
                    new_value = old_value + ',' + ids_to_delete
                else:
                    new_value = ids_to_delete
                self._cr.execute("""UPDATE res_users SET picking_line_unlink_data =%s WHERE id =%s""", (new_value, user_id))
                self._cr.commit()
        return res
            ...
            if move._should_bypass_reservation():
                ...
                if move.product_id.tracking == 'none':
                    ...
                    for lot_dict in move_dict.get('scanned_data', []):
                        if lot_dict['qty'] > 0:
                            lot_id = self.env['stock.production.lot'].search([('name', '=', lot_dict['lot_name']), ('product_id', '=', move.product_id.id)], limit=1)
                            if lot_id:
    ...
    def action_app_put_in_package(self, vals):
        ...
        try:
            self.write(vals)
            self.action_put_in_package()
        except Exception as e:
            error_message = tools.ustr(e)
            self.env.cr.rollback()
        picking_data = self.picking_id.app_stock_picking_retrive_data()
    ...
    def unlink(self):
        ...
        if ids_to_delete:
            query = '''
                SELECT 
                  ru.id AS user_id,
                  COALESCE(ru.move_line_unlink_data, '') AS move_line_unlink_data
                FROM
                  res_users as ru
                WHERE 
                  ru.active != FALSE	AND ru.share = FALSE
            ...
            for record in users_data:
                user_id = record.get('user_id')
                old_value = record.get('move_line_unlink_data')
                if old_value:
                    new_value = old_value + ',' + ids_to_delete
                else:
                    new_value = ids_to_delete
                self._cr.execute("""UPDATE res_users SET move_line_unlink_data =%s WHERE id =%s""", (new_value, user_id))
                self._cr.commit()
        return res
