...
from odoo.addons.equip3_agri_accessright_settings.models.acrux_chat_connector import ca_request
from odoo.addons.equip3_agri_accessright_settings.models.qiscus_connector import qiscus_request


class AgriApprovalMatrix(models.Model):
    _name = 'agri.approval.matrix'
    _description = 'Agriculture Approval Matrix'
    ...
    line_ids = fields.One2many('agri.approval.matrix.line', 'matrix_id',  string='Approval Lines')
    ...
    def get_approval_entries(self, Model, states=None):
        self.ensure_one()
        entry = self.env['agri.approval.matrix.entry']
    ...
    def post_log(self, Model, partner_ids, values):
        ...
        xml_id = 'equip3_agri_accessright_settings.agri_approval_html_body'
    ...
    def send_email_notification(self, Model, partner_id, values):
        self.ensure_one()
        xml_id = 'equip3_agri_accessright_settings.agri_approval_mail_reuse'
        company = self.get_company_model(Model)
    ...
    def send_whatsapp_notification(self, Model, partner_id, values):
        self.ensure_one()
        xml_id = 'equip3_agri_accessright_settings.agri_approval_text_body'
    ...
    def action_reject(self, Model, reason=False):
        ...
        if not self.env.context.get('skip_reject_wizard'):
            return {
                ...
                'res_model': 'agri.approval.matrix.reject',
                'target': 'new',
                'view_mode': 'form',
                'context': {
...
class AgriApprovalMatrixLine(models.Model):
    _name = 'agri.approval.matrix.line'
    _description = 'Agriculture Approval Matrix Line'
    ...
    matrix_id = fields.Many2one('agri.approval.matrix', 'Approval Matrix')
