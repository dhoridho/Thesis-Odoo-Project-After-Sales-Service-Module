...
class HrOvertimeApprovalWizard(models.TransientModel):
    _name = 'hr.overtime.actual.approval.wizard'
    ...
    def submit(self):
        for line_item in self.overtime_id.actual_approval_line_ids:
            if line_item.approver_state == 'draft' or line_item.approver_state == 'pending':
                for user in line_item.approver_id:
                    if self.env.user.id in user.ids:
                        sequence_matrix = [data.sequence for data in self.overtime_id.actual_approval_line_ids]
                        sequence_approval = [data.sequence for data in self.overtime_id.actual_approval_line_ids.filtered(
