...
from odoo.http import content_disposition, dispatch_rpc, request, serialize_exception as _serialize_exception, Response, JsonRequest

_logger = logging.getLogger(__name__)


class PosIntegrationsWhatsappController(http.Controller):
    ...
    @http.route([ 
        '/pos/whatsapp/<string:ttype>/<string:filename>'
    ...
    def content_image(self, xmlid=None, model='ir.attachment', id=None, field='datas',
        ...
        if ttype not in ['receipt']:
            return request.not_found()

        if ttype == 'receipt':
            model = 'ir.attachment'
            field = 'datas'
            id = filename.split('-Receipt')[0]
    ...
    def _content_image(self, xmlid=None, model='ir.attachment', id=None, field='datas',
        ...
        return PosIntegrationsWhatsappController._content_image_get_response(
    ...
    def _content_image_get_response(
        ...
        if not image_base64:
            if placeholder is None and model in request.env:
                ...
                placeholder_content = PosIntegrationsWhatsappController.oloutlet_placeholder(image=placeholder_filename)
            else:
                placeholder_content = PosIntegrationsWhatsappController.oloutlet_placeholder()
